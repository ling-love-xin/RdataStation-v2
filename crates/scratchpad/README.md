...
| `src/scratchpad_view.rs` | 面板视图（`ScratchpadView`）：工具栏 / 搜索 / 树 / 引用 / 回收站 / 撤销栏 / 状态行、剪贴板与多选、键盘导航、脏点回显、**冲突条**、**拖起文件行**（载荷 `shared::InsertFileDrag`）；另提供中央编辑区的两套只读投影（搜索结果面板 / 冲突 Diff 面板）；含纯函数辅助与其单测 |
...
| **拖入编辑器插入**（拖草稿行落到编辑器文本区 → 内容插到光标处；载荷 `shared::InsertFileDrag`，落点现读） | 命中跳转到具体行（需编辑器跳行端口） |
| 懒加载、排序（名称/大小/时间）、文件名过滤、**虚拟列表**（只渲染可视区）、空态大图标 + 按钮 | 提升/存档只读/取回/版本（Phase D，依赖 `analytics_resource`） |
| 内容搜索（正则/大小写）+ **命中高亮** + **全部替换**（结果与替换栏落中央编辑区）+ **点命中标题打开该草稿** | 命中跳转到具体行（需编辑器跳行端口） |
...
- 对外宣传 / 新人先读：`docs/architecture/scratchpad/scratchpad-showcase.html`（自包含单页：特点 · 流程 · 架构 · 状态）。
