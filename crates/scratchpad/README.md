...
| 文件 | 职责 |
| --- | --- |
| `src/models.rs` | 域模型：`ScratchpadEntry` / `FileMeta` / `ExternalReference` / `ExternalReferenceStatus` / 搜索 / Diff / 替换 |
| `src/state.rs` | `ScratchpadState`：持有 store（按项目初始化），供宿主/窗口使用 |
| `src/store.rs` | `ScratchpadStore`：列表（懒加载按目录）、CRUD、**递归复制**、回收站入口、内容搜索（子串/正则/大小写 + **命中区间**）、替换（正则/字面量 + 大小写）、Diff、外部引用（含**重新定位**）、可分析文件、路径防护、旧布局迁移 |
...
| 已实现 | 待补 |
| --- | --- |
| 模块根 + 内部态隔离 + 旧布局迁移 | 双击打开到编辑区、脏点、冲突 Diff（Phase C） |
| 项目级回收站（来源/原路径/还原/清空） | 双击打开到编辑区、脏点、冲突 Diff（Phase C） |
| 列表/新建（含模板）/重命名/删除/移动/复制（文件与**文件夹递归**） | 拖放导入 / 拖入编辑器（Phase C） |
...
| 外部引用（添加文件或目录 / 自定义别名 / 改名 / 打开 / 移除 / 可用性探测 / **失效后重新引用**）、文件元数据（数据源绑定） | 引用目录的展开浏览（当前只作入口） |
