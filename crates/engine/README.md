...
| 目标 | 命令 | 基线 |
| --- | --- | --- |
| 库单测 | `cargo test -p rds-engine --lib -j 2` | **446 通过 / 24 忽略** |
...
| 全量 | `cargo test-all` | 以重测为准（2026-09-19 第二次复跑：**84 个目标 1990 通过 / 51 忽略 / 0 失败**；含本机诊断 `zz_fixture_probe` 1 项，项目自身套件为 83 目标 / 1989 项。全量台账见 `../../docs/architecture/module-status.md`） |
