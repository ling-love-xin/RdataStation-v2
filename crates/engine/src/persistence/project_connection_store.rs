...
use crate::persistence::project_db::ProjectDatabaseManager;
...
impl ProjectConnectionStore {
    ...
    pub async fn enable_driver(&self, driver_id: &str) -> Result<(), CoreError> {
        let global_db = crate::migration::get_global_db_manager()
    ...
    pub async fn project_add_plugin(
        &self,
        plugin: &crate::persistence::plugin_store::ProjectUsedPlugin,
    ) -> Result<(), CoreError> {
        let conn = self.db_manager.sqlite_pool().acquire().await?;
        crate::persistence::plugin_store::project_add_plugin(conn.inner()?, plugin)?;
        Ok(())
    }
    ...
    pub async fn project_remove_plugin(&self, code: &str, version: &str) -> Result<(), CoreError> {
        let conn = self.db_manager.sqlite_pool().acquire().await?;
        crate::persistence::plugin_store::project_remove_plugin(
            conn.inner()?,
            code,
            version,
        )?;
    ...
    pub async fn project_get_plugins(
        &self,
    ) -> Result<Vec<crate::persistence::plugin_store::ProjectUsedPlugin>, CoreError> {
        let conn = self.db_manager.sqlite_pool().acquire().await?;
        let result = crate::persistence::plugin_store::project_get_plugins(conn.inner()?)?;
        Ok(result)
    }
    ...
    pub async fn project_update_plugin_enabled(
        ...
        let conn = self.db_manager.sqlite_pool().acquire().await?;
        crate::persistence::plugin_store::project_update_plugin_enabled(
            conn.inner()?,
            code,
            version,
            enabled,
    ...
    pub async fn project_set_plugin_config(
        &self,
        config: &crate::persistence::plugin_store::ProjectPluginConfig,
    ) -> Result<(), CoreError> {
        let conn = self.db_manager.sqlite_pool().acquire().await?;
        crate::persistence::plugin_store::project_set_plugin_config(conn.inner()?, config)?;
        Ok(())
    }
    ...
    pub async fn project_get_plugin_configs(
        ...
        code: &str,
        version: &str,
    ) -> Result<Vec<crate::persistence::plugin_store::ProjectPluginConfig>, CoreError> {
        let conn = self.db_manager.sqlite_pool().acquire().await?;
        let result = crate::persistence::plugin_store::project_get_plugin_configs(
            conn.inner()?,
            code,
            version,
        )?;
