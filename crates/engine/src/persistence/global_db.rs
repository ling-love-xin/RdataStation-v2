...
use crate::persistence::auth_store;
use crate::persistence::driver_store::{self, DataSourceType, Driver, DriverFile};
use crate::persistence::env_store;
use crate::persistence::network_store;
use crate::persistence::plugin_store;
use crate::persistence::sql_template_store::SqlTemplateStore;
use crate::persistence::workbench_context_store::WorkbenchContextStore;
...
impl GlobalDuckdbConnection {
    ...
    pub async fn new(db_path: PathBuf) -> Result<Self, CoreError> {
        let conn = crate::DuckDBManager::open_file_with_retry(&db_path.to_string_lossy())?;
...
impl GlobalDatabaseManager {
    ...
    pub async fn save_global_connection(
        ...
        let final_password = match input.password.or(url_password.as_deref()) {
            Some(p) if !p.is_empty() => shared::crypto::encrypt_password(p).map_err(|e| {
    ...
    pub async fn update_global_connection(
        ...
        let password_encrypted = match input.password {
            Some(p) if !p.is_empty() => {
                Some(shared::crypto::encrypt_password(&p).map_err(|e| {
                    CoreError::common(CommonError::General(format!("密码加密失败: {}", e)))
                })?)
            }
            _ => {
