...
impl MigrationStrategy for V1ToV2Migration {
    ...
    fn migrate(&self, conn: &Connection) -> Result<(), CoreError> {
        ...
        // 确保迁移历史表存在（正常流程由 connection_metadata 迁移创建；
...
mod tests {
    ...
    fn test_cache_version_manager() -> Result<(), CoreError> {
        ...
        // 模拟旧库状态：仅 cache_version（version=1）+ columns（V7ToV8 策略
        // 会 ALTER columns 加 is_primary）。CacheVersionManager 是纯代码版本
        // 记录层（真实 DDL 在 connection_metadata SQL 迁移链中）
        conn.execute(
        ...
        // 执行迁移：从 version=1 到 CURRENT_CACHE_VERSION，每个版本策略记 1 条
