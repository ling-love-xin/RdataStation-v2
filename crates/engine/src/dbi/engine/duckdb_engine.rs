...
use crate::dbi::engine::ExecutionEngine;
use shared::error::CommonError;
use shared::error::CoreError;
use shared::error::DatabaseError;
...
impl DuckDBEngine {
    ...
    pub fn conn(&self) -> Result<duckdb::Connection, CoreError> {
        let _arc = crate::duckdb::DuckDBManager::get_or_create_in_memory()?;
        // 返回连接的克隆（duckdb::Connection 不实现 Clone，需要重新打开）
        Err(shared::error::CoreError::common(
            shared::error::CommonError::General(
    ...
    async fn execute_query(&self, sql: &str) -> Result<QueryResult, CoreError> {
        ...
        let batch = if row_count > 0 {
            crate::duckdb::row_to_arrow::duckdb_rows_to_arrow(&columns, &row_data)?
        } else {
...
#[allow(dead_code)]
fn duckdb_value_to_value(row: &duckdb::Row, index: usize) -> shared::models::Value {
    if let Ok(Some(v)) = row.get::<_, Option<i64>>(index) {
        return shared::models::Value::Int(v);
    }

    if let Ok(Some(v)) = row.get::<_, Option<f64>>(index) {
        return shared::models::Value::Float(v);
    }

    if let Ok(Some(v)) = row.get::<_, Option<String>>(index) {
        return shared::models::Value::Text(v);
    }

    if let Ok(Some(v)) = row.get::<_, Option<Vec<u8>>>(index) {
        return shared::models::Value::Bytes(v);
    }
