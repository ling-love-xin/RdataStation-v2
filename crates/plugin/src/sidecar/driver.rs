//! TODO(migration): v1 死代码（未编译）。DriverFactory 旧接口（id/name/kind/default_port/create_pool）
//! 与 v2 trait（descriptor/create）不匹配，待按 v2 接口重写后恢复编译。
...
use engine::driver::{DriverFactory, DriverKind};
...
impl Database for SidecarDatabase {
    async fn query(&self, sql: &str) -> Result<QueryResult, CoreError> {
        ...
        let result = client
            ...
            .await
            .map_err(|e| CoreError::database(shared::error::DatabaseError::Driver {
    ...
    async fn query_with_params(
        ...
        let json_params: Vec<Value> = params
            .into_iter()
            .map(|v| match v {
                shared::models::Value::Null => Value::Null,
                shared::models::Value::Bool(b) => Value::Bool(b),
                shared::models::Value::Int(i) => Value::Number(i.into()),
                shared::models::Value::Int64(i) => Value::Number(i.into()),
                shared::models::Value::Float(f) => Value::Number(
                    serde_json::Number::from_f64(f).unwrap_or_else(|| 0.into()),
                ),
                shared::models::Value::String(s) => Value::String(s),
                shared::models::Value::DateTime(dt) => Value::String(dt.to_string()),
                shared::models::Value::Blob(b) => Value::Array(
                    b.into_iter()
    ...
    async fn begin_transaction(&self) -> Result<Box<dyn Transaction>, CoreError> {
        Err(CoreError::database(shared::error::DatabaseError::Driver {
            db_type: "sidecar".to_string(),
    ...
    async fn ping(&self) -> Result<(), CoreError> {
        ...
        match result {
            ...
            Err(e) => Err(CoreError::database(shared::error::DatabaseError::Driver {
                db_type: "sidecar".to_string(),
                operation: "ping".to_string(),
                source: e.to_string(),
            })),
...
impl DbPool for SidecarPool {
    async fn acquire(&self) -> Result<Box<dyn Database + Send + Sync>, CoreError> {
        let port = self.manager.port().ok_or_else(|| {
            CoreError::database(shared::error::DatabaseError::Driver {
                db_type: "sidecar".to_string(),
...
impl DriverFactory for SidecarDriverFactory {
    ...
    async fn create_pool(
        &self,
        _config: engine::driver::DriverConnectionConfig,
