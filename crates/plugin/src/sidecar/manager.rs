...
impl SidecarManager {
    ...
    /// 获取状态锁（带错误处理）
    fn lock_status(&self) -> Result<std::sync::MutexGuard<'_, SidecarStatus>, SidecarError> {
        self.status
            .lock()
    ...
    /// 获取子进程锁（带错误处理）
    fn lock_child(&self) -> Result<std::sync::MutexGuard<'_, Option<Child>>, SidecarError> {
        self.child
            .lock()
            .map_err(|e| SidecarError::Internal(format!("Mutex poisoned: {}", e)))
    }
    ...
    /// 获取端口锁（带错误处理）
    fn lock_port(&self) -> Result<std::sync::MutexGuard<'_, Option<u16>>, SidecarError> {
        self.port
            .lock()
            .map_err(|e| SidecarError::Internal(format!("Mutex poisoned: {}", e)))
    }
    ...
    /// 获取停止信号锁（带错误处理）
    fn lock_stop_tx(&self) -> Result<std::sync::MutexGuard<'_, Option<oneshot::Sender<()>>>, SidecarError> {
        self.stop_tx
            .lock()
            .map_err(|e| SidecarError::Internal(format!("Mutex poisoned: {}", e)))
    }
    ...
    /// 获取健康检查任务锁（带错误处理）
    fn lock_health_check_task(&self) -> Result<std::sync::MutexGuard<'_, Option<JoinHandle<()>>>, SidecarError> {
