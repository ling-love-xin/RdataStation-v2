...
| **详情 tab：字段卡片（生成器分类子菜单 / 搜索对话框 + 编辑 / 智能 / 删除 + 列搜索框）+ 预览表格（组件库 `DataTable`，与编辑器结果集同一套原语；列头可排序 / 单元格可悬停看全文 / 右键可复制值·列·行）** | 预览筛选（取样会变成长度未知的子集）；结果表 tab 的列只读，要筛先回草稿；预览排序**不是**就地重排（会重查临时表，见架构 §9-I26）；「复制此列」只给取样内的值（整列请落库 / 导出后查） |
