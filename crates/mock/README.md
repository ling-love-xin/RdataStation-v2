...
| **详情 tab：字段卡片（生成器分类子菜单 / 搜索对话框 + 编辑 / 智能 / 删除）+ 预览表格（组件库 `DataTable`，与编辑器结果集同一套原语）** | 预览**列头排序已开**（点列头 = 按该列**重查**临时表取全局前 N 行，不是就地重排；右键菜单同一套；内存库忙时不排队，给一句可读提示）；列宽跟随内容自适应（现为组件默认 100px + 可拖）；单元格悬停显示全文（行号槽不给） |
