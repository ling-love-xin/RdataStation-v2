...
| **工作日历就在列上**（不共享、不内置节假日表）：「仅工作日」+ 工作周掩码（`1111100` = 周一~周五）+ 跳过日期（节假日）+ 上班日期（调休，优先）；`date_time_between` 另有「仅工作时段（09:00~18:00）」 | 工作时段窗口自定义（现固定 09:00~18:00）；把日历扩到 `date` / `date_time` / `date_time_before` / `date_time_after` |
