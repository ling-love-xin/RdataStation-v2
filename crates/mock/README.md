...
- 列依赖 `dependency` 当前只承载**跨表引用**（`dep_type == ForeignKey`）；生成器按 `columns` 顺序取值，**没有拓扑排序，也不解释依赖表达式**（`Expression` / `Template` / `Sequence` / `Weighted` 与 `source_columns` / `expression` / `weights` 是 v1 遗留空壳，待拍板：做掉或删掉）。
...
| **列编辑对话框（列名 / 类型 / 参数 / 空值率 / 唯一 / 恢复智能默认）** | 列依赖编辑（`Expression` / `Template` / `Weighted` 待拍板：做表达式还是删空壳） |
