...
/// 导航状态的**落库防抖窗口**（毫秒）：展开 / 选中 / 搜索词在窗口内的连续变更攒成一次写。
///
/// 为何需要：展开与箭头键漫游都是高频动作，而落库是 UI 线程上的 SQLite 写——
/// 逐个写会把点击响应放大成一串磁盘操作（架构 §11 #25 ① 点名的「展开态写库无防抖」）。
/// 600ms 是「手指离开键盘就落」与「连按不写库」之间的取值。
pub const NAV_STATE_SAVE_DEBOUNCE_MS: u64 = 600;
