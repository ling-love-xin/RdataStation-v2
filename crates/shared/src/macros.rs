...
macro_rules! core_err {
    ($msg:expr) => {
        $crate::error::common_err($msg)
    };
    ($fmt:expr, $($arg:tt)*) => {
        $crate::error::common_err(format!($fmt, $($arg)*))
    };
}
...
macro_rules! conn_err {
    ($conn_id:expr, $kind:expr) => {
        $crate::error::CoreError::connection($kind)
    };
}
...
macro_rules! query_err {
    ($sql:expr, $reason:expr) => {
        $crate::error::query_err($sql, $reason)
    };
    ($sql:expr, $fmt:expr, $($arg:tt)*) => {
        $crate::error::query_err($sql, format!($fmt, $($arg)*))
    };
}
...
macro_rules! storage_err {
    ($store:expr, $op:expr, $reason:expr) => {
        $crate::error::storage_err($store, $op, $reason)
    };
    ($store:expr, $op:expr, $fmt:expr, $($arg:tt)*) => {
        $crate::error::storage_err($store, $op, format!($fmt, $($arg)*))
    };
}
...
macro_rules! invalid_arg {
    ($param:expr, $reason:expr) => {
        $crate::error::invalid_arg($param, $reason)
    };
    ($param:expr, $fmt:expr, $($arg:tt)*) => {
        $crate::error::invalid_arg($param, format!($fmt, $($arg)*))
    };
}
...
macro_rules! not_supported {
    ($feature:expr) => {
        $crate::error::not_supported($feature)
    };
}
...
macro_rules! timeout {
    ($operation:expr, $duration_ms:expr) => {
        $crate::error::timeout($operation, $duration_ms)
    };
}
