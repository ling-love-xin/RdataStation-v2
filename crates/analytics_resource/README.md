...
| `src/service.rs` | 归档服务：归档（首次）/ 再归档（指纹未变即幂等、变了才增版本）/ 取回（检出）/ **还原到历史版本**（用旧内容生成新版本；无副本拒、指纹相同幂等）/ **取回历史版本** / **撤销归档**（本体移回 + 硬删行）/ **移入回收站**（批量，本体进 `ProjectTrash` + 登记行软删）/ **从回收站还原**（`origin` 校验 + 同名避让后登记行跟改）/ **按原相对路径还原**（索引修复那条）/ **永久删除与清空**（只动本模块的条目）编排 + **归档时把标签 / 分组真的落上**（`apply_labels` / `link_tags`：标签按名找、找不到就建；附属项失败记进 `ArchiveOutcome.notes` 不回滚主操作）+ **历史内容保留策略**（`KeepVersions`：全留 / 只留元数据 / 保留最近 n 份，两处裁剪合并为 `prune_copies`）+ `ResourcesChanged` 广播 | ✅ Phase 0 + Phase 3 + P0.8 + P2.4 + Phase 2 第八刀 |
...
| `src/dialogs/archive.rs` / `checkout.rs` / `pick.rs` / **`version.rs`** / **`index_repair.rs`** / **`trash.rs`** / **`tag.rs`** / **`group.rs`** | 归档确认 / 取回（检出）/ 草稿多选 / **版本历史** / **索引修复** / **回收站** / **标签** / **分组名**对话框：种子（宿主备好的来源、字典与只读信息）+ 表单（**显示名 / 别名 / 分组下拉（`group_picker`，选择住 `Rc<RefCell<_>>`）/ 标签 / 保留份数**；文件名 / 是否打开；勾选列表；版本表格 + 选中后动作栏；三分组 + 行内动作；条目表格 + 行内动作 + 清空；标签勾选 + 新建并打上 + 行内 ⋯（重命名 / 删除）；分组名单输入）+ 校验与解析（纯函数）+ `open_*_dialog`；后五者的状态可被宿主换行（`set_rows` / `set_options`）与收放忙态。回收站对话框**只列 `origin = "resources"` 的条目**，别人的只给一句说明 | ✅ Phase 1 + Phase 3 + P0.8 + Phase 2 三 / 八刀（执行由宿主注入的 `on_submit` / `on_action` 接手） |
...
| **版本历史**【Phase 3 第一刀】`dialogs/version.rs` + `present::build_version_rows` + 宿主接线：当前版本与历史行同列（副本缺失行露出来）；**行本体走 `list::ListItem`**（hover / 选中由组件承担，不再手搓）；**还原 = 生成新版本**（不原地回滚）、**取回该版本为草稿**、**删除内容副本**（`AlertDialog` 确认）；动作后宿主换行，不关窗；还原后的裁剪按设置项（见下行） | 详情面板的"最近 3 条"明细（需批量取数） |
...
| **面板**【Phase 1 + Phase 2 第一 / 二 / 三刀 + 2026-09-20 UI 收尾】`ResourcesPanel`：面板头（标题图标 + `＋ ▾` / `⋯` 四项）/ 工具栏（搜索·筛选（含**标签维**）·排序）/ **`List` 虚拟化行（kind 图标 + 强度徽标 + 尾部字段）+ 右键菜单** / **批量多选（单击 / `Ctrl` / `Shift` / 双击手势 + `Ctrl+A`，多选时单选项置灰、删除带数量）** / **分组折叠区（会话级折叠）** / 状态行 / 两种空态；`present.rs` 把索引行转成快照（含逐行详情与标签）；**workbench 接线已落**（`panels/resources.rs` 装配 + `services/resource_jobs.rs` 后台取数 + `components/resource_host.rs` 端口）；**右栏「存档详情」已接**（`RightPanel::Archive`，宿主观察面板实体做选中联动） | `F2` 重命名（待重命名入口）、标题点击折叠（需与 M4/M5 面板头一起做）、折叠状态持久化（P2.4 后半） |
| **行态与控件口径**【2026-09-20 UI 收尾 / 开发方案 V1–V12】：行距钉在 `ui::ROW_HEIGHT`（24px，`list_row()` 压掉 `ListItem` 默认 `py_1`——组件只量一个样本行定全局行距）；悬停不覆盖选中；分组头展开指示走共用原语 `tree::disclosure_slot` + `disclosure_icon`（字符 `▸/▾` 退场）+ 色条 `primary`（与 M4 同角色）；**停在分组头上按 `Enter` 也能折叠 / 展开**；撤销 / 复制 / chip `×` 一律 `Button::ghost().xsmall()`（键盘可达 + 命中区 20px）；加载骨架走组件 `Skeleton`；固定描边统一 `ui::HAIRLINE` | 行尾「相对时间 → 悬浮显示绝对时间」要等 div 级 tooltip 基建（0.6.1 的 `.tooltip()` 只挂组件上） |
...
| **归档 / 取回**【Phase 1 + Phase 2 第八刀】`dialogs/{archive,checkout,pick}.rs`：对话框 + 校验 + 冲突提示（`resources/x-2.sql`）；**归档表单带显示名 / 别名 / 分组 / 标签 / 保留份数**，且这几项**真的落库**（标签按名找、找不到就建；分组走移动语义；附属项失败写进回执，不回滚归档）；workbench 侧真执行（`resource_host` + `resource_jobs` 的 `Archive` / `Checkout` 作业 + 重名避让 + 回执 + 顺手打开）；**草稿箱入口**（面板头 `＋ ▾` + 草稿多选，来源连接与出处自动带出）；**归档可撤销**（`undo_archive` + 5 秒撤销栏） | 草稿箱右键入口、默认分组、批量归档时的逐项改名、`Ctrl+Z` |
...
| **标签**【Phase 2 第一 / 三刀】`tag.rs`（改名 / 删除补齐 + 批量查询）+ 打标 / 去标 + **标签对话框**（行内真 `Checkbox` 勾选 + 新建并打上 + 差集提交 + 行内 **⋯：重命名 / 删除**）+ 详情面板 chips（× 即去标，是语义按钮）+ **筛选菜单的标签维**，标签字典随快照下发（不另查库） | 标签颜色（现不使用用户填的 hex；颜色一律 token）、按标签排序、批量打标签（多选态） |
