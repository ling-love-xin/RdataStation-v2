...
| `src/resource_view.rs` | 左 Dock 面板：面板头 / **工具栏（搜索·筛选·排序）** / 提示行 / **行列表（`list::List`：虚拟化 + 组件化 hover·选中·键盘漫游，选中以面板的行 id 为准）** / **行右键菜单（打开·取回·移入回收站）** / 状态行 / **两种空态（空库 vs 无匹配）**；宿主动作经 `ResourcesHost` | ✅ Phase 1 五刀（详情面板接入·行图标·批量多选待下一批） |
...
| **面板**【Phase 1】`ResourcesPanel`：面板头 / 工具栏（搜索·筛选·排序）/ **`List` 虚拟化行 + 右键菜单** / 状态行 / 两种空态；`present.rs` 把索引行转成快照；**workbench 接线已落**（`panels/resources.rs` 装配 + `services/resource_jobs.rs` 后台取数 + `components/resource_host.rs` 端口） | 详情面板接入、行图标、批量多选、五个对话框与动作真实现 |
...
- 验证：`cargo test -p rds-analytics-resource -j 2` → **66 项单测**（16 存储 + 5 领域 + 11 本体 + 7 归档服务 + 7 索引修复 + 6 筛选/排序 + 4 面板 + 5 详情 + 5 呈现）+ `tests/panel_window.rs` **6 项窗口测试**；`cargo check -p rds-analytics-resource --all-targets -j 2` 零告警。
