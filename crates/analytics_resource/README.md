...
| `src/filter.rs` | 工具栏**数据层**（纯函数，零 GPUI 依赖）：`ResourcesFilter`（关键字 / 种类 / **标签（id 多选，并集）** / 只看异常；`toggle_kind` 把"全选"规范化为不限、`toggle_tag` / `drop_tag` 管标签条件）+ `SortField`/`SortOrder`（`label` / `arrow` / `flipped`）+ `apply_view`（筛选→排序，同键名称兜底且不随方向翻转）；`is_empty()` 决定面板显示哪一种空态 | ✅ Phase 1 + Phase 2 第一刀 |
...
| `src/ui.rs` | 视图尺寸常量（与 `workbench/ui.rs` 同源同值，但**在本 crate 声明**：依赖方向不允许反向读 workbench） | ✅ 14 项 |
...
| **标签**【Phase 2 第一刀】`tag.rs`（改名 / 删除补齐 + 批量查询）+ 打标 / 去标 + **标签对话框**（勾选 + 新建并打上 + 差集提交）+ 详情面板 chips（× 即去标）+ **筛选菜单的标签维**，标签字典随快照下发（不另查库） | 标签颜色（现不使用用户填的 hex；颜色一律 token）、按标签排序 / 分组、批量打标签（多选态） |
...
- 验证：`cargo test -p rds-analytics-resource -j 2` → **111 项单测**（15 存储 + 5 领域 + 13 本体 + 17 归档服务 + 7 索引修复 + 8 筛选/排序 + 9 面板 + 5 详情 + 14 呈现 + 6 对话框（归档/取回/草稿多选）+ 3 版本对话框 + 3 索引修复对话框 + 3 回收站对话框 + 3 标签对话框）+ `tests/panel_window.rs` **14 项面板窗口测试** + `tests/dialog_window.rs` **8 项对话框窗口测试**；编辑器侧 `cargo test -p rds-editor --lib -j 2` **217 项**（含 `persist` 的只读打开用例）；`cargo check -p rds-workbench --all-targets -j 2`、`cargo check -p rds-app -j 2` 与 `cargo check -p rds-analytics-resource --all-targets -j 2` 零告警。
