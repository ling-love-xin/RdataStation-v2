...
//!
//! 结构为简单 JSON 数组（最新前），去重（同 SQL 移动到最前），上限 20 条；
//! 文件不存在 / 损坏时按空历史处理（load 不报错，append 会重建）。
//! 目录可注入（`*_at` 变体），便于测试与未来多项目隔离。
//!
//! **已被 `editor::history` 取代（B8）**：真正的历史是引擎的 `history_store`（带耗时 / 成败 /
//! 失败原因 / 行数），右 Dock 的历史面板读它。这里只剩旧的 20 条纯文本记录，
//! 界面上已无调用点（保留 API 与它的测试，不在本次改动里删文件）。
