...
fn migration_routes_settings_to_config_and_skips_new_layout() {
    ...
    let dests: Vec<&Path> = items.iter().map(|(_, d)| d.as_path()).collect();
    ...
    assert!(
        !items.iter().any(|(s, _)| s == &src.join("data")),
        "新布局目录不能被当成旧数据"
    );
    assert!(!items.iter().any(|(s, _)| s == &src.join("logs")));
}
...
fn migration_does_not_overwrite_existing_destination() {
    ...
    migrate::copy_item(&src.join("settings.json"), &dest, &mut report);
...
fn migration_copies_directories_recursively() {
    ...
    let mut report = migrate::MigrationReport::default();
    migrate::copy_item(&src, &dest, &mut report);
