...
| **quick_open** | Metadata search and command palette | ✅ | Three prefixes (`>` commands / `#` / `@`) · the name tier runs on `metadata_index` · 100k-object latency **158 ms → 1.5 ms** | Full-document tier (FTS not wired) · scratchpad file source |
...
- **Full-text metadata search**: the FTS write path has a real defect (its insert statement references a table that doesn't exist in the migrations), which must be fixed before "search by comment or data type" can ship.
