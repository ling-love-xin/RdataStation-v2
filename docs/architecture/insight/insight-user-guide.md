...
### 3.5 对比两次分析（**列表已可用，对比面板尚在 Phase 5.2**）
...
目前能做的：**存快照 + 看历史 + 认版本**。列表满了 10 条时下方会写明「只列出最近 N 条」（更老的版本还在库里，只是没列出来）。
...
⬜ 待落地：点任一版本进入对比面板，逐字段显示 `旧 → 新 (±差值)`，并用三色标注**增 / 减 / 不变**；以及按天数清理。
...
- [x] 存储用量为后端真实统计（不是前端估算）
- [ ] 选任一历史版本进入对比，逐字段显示差值，增/减/不变三色正确（Phase 5.2）
