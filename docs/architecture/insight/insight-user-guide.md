...
- **临时表口径**：分析对象是**内存库里的临时表**（结果集表，或「查看统计」时按表名登记的表）。洞察自己为一次分析建的样本表**用完即删**（`tmp_i_` 前缀、建表即登记，分析结束就收）——所以不存在「洞察的表过期」这件事。只有**源表**不在内存库里了（宿主关了结果集 / 会话结束），面板才会报「结果集已失效或已过期，请重新执行查询」。
