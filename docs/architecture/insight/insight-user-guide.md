...
| **尚未落的防线（待拍板，见架构 §12 Q1）** | 说明 |
| --- | --- |
| 规则 SQL 静态门 | 拟只放行单条 `SELECT` / `WITH`，禁分号与 `ATTACH` / `COPY` / `INSTALL` / `LOAD` / `read_*` 等——**防呆层**，不是安全边界 |
| 项目规则信任门 | 项目规则跟着仓库走，建议首次发现时要求确认信任（默认不自动生效） |
| 沙箱执行 | ✖ 不计划；已核查「禁用扩展 / 只读连接」会连带砸掉连接 DuckDB 数据源、打开 CSV/Parquet、装扩展等自身能力（同一个内存单例） |

> **现状（请如实理解）**：规则模板里的 SQL **原样交给 DuckDB 执行**——可以 `ATTACH` / `COPY` 到本地路径等。规则文件被视为**可信的本地文件**，与执行任意脚本同级别（`insight-architecture.md` K1 / Q1）。
