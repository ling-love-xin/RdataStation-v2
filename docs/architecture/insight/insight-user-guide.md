...
| **尚未落的防线** | 说明 |
| --- | --- |
| 项目规则信任门 | 项目规则跟着仓库走（`.RSmeta/insight-rules/`），**克隆一个不信任的仓库再打开项目 = 把它的 SQL 拿到你本机执行**；建议首次发现项目规则时要求确认信任（架构 §12 Q1 ③，待拍板） |
| 完整沙箱 | ✖ 不计划；已核查「禁用扩展 / 只读连接」会连带砸掉连接 DuckDB 数据源、打开 CSV/Parquet、装扩展等自身能力（同一个内存单例） |

> **现状（请如实理解）**：通过静态门的规则仍**原样交给 DuckDB 执行**——防呆挡的是误写与顺手抄来的危险语句，**不是安全边界**（DuckDB 没有官方解析沙箱，黑名单天然有漏）。规则文件仍应被视为**可信的本地文件**（`insight-architecture.md` K1 / Q1）。

> 实务建议：只运行你理解或信任来源的规则文件；把项目规则目录纳入代码评审，与对待源码同样严格；不要随手打开来源不明的项目目录。
