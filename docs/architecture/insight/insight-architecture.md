...
| D59 边界实证（扩展源） | `workbench/tests/insight_source_real.rs`（`extension_provided_source_is_analyzable`：Oracle 经 community 扩展 `oracle_scanner` 读入 → 取样 → 列画像 / 表探查）——证明洞察只依赖「能在自己内存库里跑的一句只读 SQL」，与数据是文件 / `ATTACH` / **扩展表函数**无关（D61） |
...
| 结构洞察方言 | `insight/src/schema_analyzer.rs`（`SchemaDialect` / `schema_filter` / `display_name`；列名比较**大小写不敏感**——MySQL 元数据列名是全大写） |
