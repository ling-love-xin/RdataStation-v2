...
| 并发受限 | 并发超上限 | 行内提示「正在分析中，请稍候」（**不出现** v1 的英文 `Too many concurrent insight operations, please retry`） |
| 错误 | 临时表失效 / 连接断开 | 错误文案 + 「重试」；临时表 TTL 过期时提示「结果已过期，请重新执行查询」（DuckDB 临时表 TTL 30 分钟） |
