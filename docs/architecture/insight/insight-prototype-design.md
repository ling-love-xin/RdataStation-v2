...
> ② 导出的**内容在面板侧算好**（与界面同源的视图模型，D36）：宿主只弹保存对话框、写文件、给状态栏回执，
> 不必知道 JSON 的分组键与 Markdown 的转义（D60）。原型画的两个 `⤓` 图标并成一个「导出 ▾」菜单（两项）。
> ③ 下钻**不建临时表**：走 D58 的源取样通道（`SampleSource`），洞察侧自己取 500 行样本（D59）——
> 原设计里「宿主登记临时表」的说法在 D58 之后不再需要（面板只报「看哪张表」，宿主给「在哪条连接上查哪张表」）。
...
| 错误 | 源表（结果集临时表）被回收 / 连接断开 | 文案 + 「重试」**由 `InsightService::describe_error` 给出**：结果集失效或过期 → 「结果集已失效或已过期，请重新执行查询」（**不给重试**，要重新执行查询）；连接抖动 → 「连接不可用，请检查数据源后重试」（给重试）；其余原文照给。不展示 `CoreError` 的 `[code]` 内部错误码 |
...
│ 搜索规则…            [ ＋ 新建项目规则 ]  [ ⟳ 重新加载 ]   │
├────────────────────────────────────────────────────────────┤
│ ⚠ 该项目带了 2 条规则，尚未加载（未信任）。                │  ← 信任门横幅（Q1 ③）
│   规则 SQL 会在本机执行…… 规则：a / b   目录：…            │
│   [ 信任并加载 ]  [ 不加载 ]                                │
├────────────────────────────────────────────────────────────┤
...
| 信任门横幅 | 未信任的项目规则**不装配**（Q1 ③ / D53）：横幅列出条数 / 前 6 个 id / 目录 / 其中解析失败数，两个按钮决定「信任并加载」或「不加载」；已拒绝时改说「你此前选择了不加载」+「改为信任并加载」。首次遇到时叠一个确认框（对话框层是栈式的）——**问一次就够**，之后只留横幅 |

**实现期新增的两项**（K7 的落地形态 + Q1 ③ 的落地形态）：

1. 全局分组在目录尚不存在时，空态给「创建目录并新建规则」——目录在**首次写入时**创建，不在启动时预设空目录。
2. **项目规则信任门**：项目层的装载以「用户是否信任该项目」为前提（默认不信任），首次碰到弹确认、决定入全局库；未信任时项目组仍列出行（它们的确在磁盘上），但**不参与任何分析**。
