...
> 状态：**Phase 5 完成 + 规则侧收口 + 临时表一致化（K16 ①~④）+ 规则 SQL 静态门（Q1 ①）**（2026-09-17）——Phase 0–5 全部完成（快照历史 · 版本对比 · 存储清理）；Q6/Q7 落地（解析期校验，K11/K12 结案）；Q2/Q3/K15 处置（删 engine 死副本 + 下线两条跑不通的内置规则，现 **16 条**）；**洞察中间表改走 `engine::duckdb::analysis`**（`tmp_i_` 前缀 · 建表即登记 · 用完即删 · 惰性清理真正 DROP）+ **内存闸与可观测**（D51）；**规则 SQL 静态门**（D52：只放行单条 `SELECT` / `WITH`，拒多语句 / DDL / 文件与远端表函数——防呆，不是安全边界）。测试 **205 项 + 集成 13 项**全绿。下一步：**宿主侧欠账**（表入口 `Shared::open_insight_table` + 导航右键「查看统计」、Schema 导出按钮与下钻）与待拍板项（**Q1 ③ 项目规则信任门** · Q4/Q5 保留期与双写补偿 · **K16 余项：结果集侧 `tmp_q_` 回收时机**）。逐项进度见 `insight-dev-plan.md` §0。
>
...
| `insight-architecture.md` | **为什么这样设计 / 怎么运转**：概念模型与八条不变式 / 分层与 crate 归属 / 状态所有权（单一写入者）/ 六条数据流 / **D1–D52 决策表** / 并发与资源 / 降级矩阵 / 测试策略 / 实现位置映射 / **§11 已知问题 K1–K16（权威：含无 SQL 沙箱（静态门已落地，信任门待拍板）· 清理剪断版本链 · 临时表回收（K16：洞察侧已修（D50/D51），结果集侧未接））** / §12 待确认 Q1–Q7（Q3 / Q6 / Q7 已定） |
...
| 待确认 | **Q1 ③ 项目规则信任门**（静态门已落地 = D52；信任门需要先拍板产品语义）· 存储清理保留天数与双写补偿（Q4/Q5）· **K16 余项：结果集侧 `tmp_q_` 回收时机**（内存闸与可观测已落地，见 D51） |
