...
> 状态：**Phase 5 完成 + 规则侧收口**（2026-09-17）——Phase 0–5 全部完成（快照历史 · 版本对比 · 存储清理）；Q6/Q7 落地（解析期校验，K11/K12 结案）；Q2/Q3/K15 处置（删 engine 死副本 + 下线两条跑不通的内置规则，现 **16 条**）。测试 **202 项 + 集成 13 项**全绿。下一步：**宿主侧欠账**（表入口 `Shared::open_insight_table` + 导航右键「查看统计」、Schema 导出按钮与下钻）与三个待拍板项（**Q1 规则 SQL 安全边界** · Q4/Q5 保留期与双写补偿 · **K16 临时表回收**——TTL/上限目前是死代码）。逐项进度见 `insight-dev-plan.md` §0。
>
...
| `insight-architecture.md` | **为什么这样设计 / 怎么运转**：概念模型与八条不变式 / 分层与 crate 归属 / 状态所有权（单一写入者）/ 六条数据流 / **D1–D49 决策表** / 并发与资源 / 降级矩阵 / 测试策略 / 实现位置映射 / **§11 已知问题 K1–K16（权威：含无 SQL 沙箱 · 清理剪断版本链 · 临时表 TTL 是死代码）** / §12 待确认 Q1–Q7（Q3 / Q6 / Q7 已定） |
...
| 待确认 | **规则 SQL 安全边界（架构 §12 Q1）** · 存储清理保留天数与双写补偿（Q4/Q5）· **临时表回收（K16：TTL/上限目前没有调用者）** |
