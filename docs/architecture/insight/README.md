...
> 状态：**Phase 5 完成 + 规则侧收口 + 临时表一致化（K16 收口）+ 规则安全边界（Q1 全落地）+ 快照收尾（Q4/Q5/K14 定案）**（2026-09-17）——Phase 0–5 全部完成；Q6/Q7 落地；内置规则 **16 条**；**临时表三块全收口**（D50 · D51 · D54）；**规则安全边界三层落地**（D52 + D53）；**快照收尾三件**（D55 双写回滚 · D56 保留期 30 天 · D57 剪链标注）。测试 **218 项 + 集成 13 项**全绿。下一步：**宿主侧欠账**——「结果集 / 源表 → 洞察」这条路还没接（`create_duckdb_temp_table` / `open_insight_column` 均零调用）。逐项进度见 `insight-dev-plan.md` §0。
>
...
| `insight-architecture.md` | **为什么这样设计 / 怎么运转**：概念模型与**九条不变式** / 分层与 crate 归属 / 状态所有权（单一写入者）/ 六条数据流 / **D1–D57 决策表** / 并发与资源 / 降级矩阵 / 测试策略 / 实现位置映射 / **§11 已知问题 K1–K16（权威：K1 规则安全边界与 K16 临时表回收、K14 剪链标注均已处置）** / §12 待确认 Q1–Q7（**Q1 / Q3 / Q4 / Q5 / Q6 / Q7 已定案**） |
