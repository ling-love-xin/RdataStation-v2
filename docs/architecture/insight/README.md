...
> 状态：**Phase 5 完成 + 规则侧收口 + 临时表一致化（K16 收口）+ 规则安全边界（Q1 ①②③ 全落地）**（2026-09-17）——Phase 0–5 全部完成（快照历史 · 版本对比 · 存储清理）；Q6/Q7 落地；Q2/Q3/K15 处置（现 **16 条**内置规则）；**临时表三块全收口**（D50 洞察中间表 · D51 内存闸与可观测 · D54 结果集表命名与回收口）；**规则安全边界三层落地**（D52 静态门 + D53 信任门 + 如实声明）。测试 **216 项 + 集成 13 项**全绿。下一步：**宿主侧欠账**——「结果集 → 临时表 → 洞察」这条路还没接（`create_duckdb_temp_table` / `open_insight_column` 均零调用）；待拍板：Q4/Q5 保留期与双写补偿。逐项进度见 `insight-dev-plan.md` §0。
>
...
| `insight-architecture.md` | **为什么这样设计 / 怎么运转**：概念模型与**九条不变式** / 分层与 crate 归属 / 状态所有权（单一写入者）/ 六条数据流 / **D1–D54 决策表** / 并发与资源 / 降级矩阵 / 测试策略 / 实现位置映射 / **§11 已知问题 K1–K16（权威：K1 规则安全边界与 K16 临时表回收均已收口）** / §12 待确认 Q1–Q7（**Q1 已定案** / Q3 / Q6 / Q7 已定） |
...
| 待确认 | 存储清理保留天数与双写补偿（Q4/Q5）· 规则安全边界若**再严一档**：`insight_rule_trust` 加规则集内容指纹（现绑定项目路径，见 D53 取舍） |
