...
> 状态：**Phase 5 完成 + 规则侧收口 + 临时表一致化（K16 ①~④）+ 规则安全边界（Q1 ①②③ 全落地）**（2026-09-17）——Phase 0–5 全部完成（快照历史 · 版本对比 · 存储清理）；Q6/Q7 落地（解析期校验，K11/K12 结案）；Q2/Q3/K15 处置（删 engine 死副本 + 下线两条跑不通的内置规则，现 **16 条**）；**洞察中间表改走 `engine::duckdb::analysis`**（`tmp_i_` · 用完即删）+ **内存闸与可观测**（D51）；**规则 SQL 静态门**（D52：只放行单条 `SELECT` / `WITH`）+ **项目规则信任门**（D53：未信任不装配，首次弹确认，决定入全局库）。测试 **216 项 + 集成 13 项**全绿。下一步：**宿主侧欠账**（表入口 `Shared::open_insight_table` + 导航右键「查看统计」、Schema 导出按钮与下钻）与待拍板项（Q4/Q5 保留期与双写补偿 · **K16 余项：结果集侧 `tmp_q_` 回收时机**）。逐项进度见 `insight-dev-plan.md` §0。
>
...
- **基线**：`cargo test -p rds-insight` 当前 **216 项**全绿（迁移基线 53：`rule_executor` 13 / `schema_analyzer` 16 / `insight_engine` 10 / `quality_scorer` 7 / `rule_registry` 7；Phase 0 新增 38；Phase 1 两批新增 29；Phase 2 两批新增 23；Phase 3 三批新增 31；Phase 4 一批新增 10；Phase 5 三批新增 15；规则校验补强新增 3；规则 SQL 静态门新增 3；**项目规则信任门新增 11**），另有**集成测试 13 项**（`cargo test -p rds-insight --test column_profile_e2e`：真实 DuckDB 临时表 → 规则统计 / 表探查 / 评估全表 / 多列规则 / **快照历史 · 版本对比 · 清理（真项目目录）** → 视图模型），**新增功能不得减少**。临时表一致化（D50/D51）的 engine 支撑另有 10 项：`cargo test -p rds-engine --lib -- duckdb::analysis duckdb::manager duckdb::temp_table`。
...
| `insight-architecture.md` | **为什么这样设计 / 怎么运转**：概念模型与**九条不变式** / 分层与 crate 归属 / 状态所有权（单一写入者）/ 六条数据流 / **D1–D53 决策表** / 并发与资源 / 降级矩阵 / 测试策略 / 实现位置映射 / **§11 已知问题 K1–K16（权威：含规则安全边界（K1 已收口：静态门 + 信任门）· 清理剪断版本链 · 临时表回收（K16：洞察侧已修（D50/D51），结果集侧未接））** / §12 待确认 Q1–Q7（**Q1 已定案** / Q3 / Q6 / Q7 已定） |
...
| 待确认 | 存储清理保留天数与双写补偿（Q4/Q5）· **K16 余项：结果集侧 `tmp_q_` 回收时机**（内存闸与可观测已落地，见 D51）· 规则安全边界若要**再严一档**：`insight_rule_trust` 加规则集内容指纹（现绑定项目路径，见 D53 取舍） |
