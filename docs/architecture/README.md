...
| `analytics_resource/analytics-resource-showcase.md` | 资产库 / 分析存档**一页看懂（可贴版）** | 关键数字 / 三个问题 / 核心闭环（归档→取回→再归档，mermaid）/ 架构分层（mermaid）与关注点落点 / 七个特点 / 归档对话框五字段 / 界面骨架 · 键鼠 / 能力清单（✅ 与 ○ 分列）/ 硬约束六条 / 质量与验证 / 二期路线；适合贴进 PR、wiki 与聊天 |
| `analytics_resource/analytics-resource-showcase.html` | 资产库 / 分析存档**一页看懂（视觉版）** | 同一套内容的卡片化排版 + 面板 / 详情线框 + 明暗切换；自包含可离线打开，与原型页同一套设计语言 |
