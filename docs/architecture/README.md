...
| `insight/insight-user-guide.md` | 洞察模块**使用手册** | 入口（四个）/ 界面导览与怎么看数字（类型徽标 · 质量色阶 · 采样口径 · TTL）/ 典型流程六条（含**规则管理对话框与它的两处行为**、**多列分析的列序语义**、**快照历史 / 版本对比 / 清理旧快照**）/ **§4 规则编写指南（对外契约）**：三层作用域 · 字段全表 · `applies_to` 与 `value_type` 取值表 · 质量门控判定语义与两个已知坑 · 两份可照抄示例 · 参数替换与**安全边界（如实说明无沙箱）** · **§4.8 内置规则 18 条一览** / FAQ 排查 / USIT 验收清单 |
| `insight/insight-dev-plan.md` | 洞察模块开发方案 | 已确认决策 9 项 / **§0 进度记录（Phase 0 两批 + Phase 1 六批 + Phase 2 两批 + Phase 3 三批 + Phase 4 一批 + Phase 5 三批 + 规则校验补强）** / 现状盘点 / **§2 五项实证缺陷** / **§3 目标 crate 边界（4 类文件归位）** / **§4 规则三层作用域与索引表设计** / Phase 0–5 任务与落点 / 测试场景 T1–T14 / 风险 R1–R7 / 实现位置映射 / 验证命令 |
