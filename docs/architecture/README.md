...
| `module-status.md` | 模块状态与测试基线（**实测版**） | **全仓唯一一份「刚跑过」的台账**：工作区 `84 目标 / 1983 通过 / 0 失败 / 51 忽略`（另有「项目自身套件 83 目标 / 1982 项」口径）· 逐包基线（lib + 集成）· 工作台 32 个集成目标明细 · 模块接通状态 · 零调用清单 · **复现命令（含 rustup shim `Permission denied` 的绕行）** · 本轮实测发现（本机诊断脚本入库事件 / 失败目标的环境归因 / **两次复跑的漂移修正表**）· 维护约定 |
