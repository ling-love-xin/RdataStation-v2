...
| `insight/insight-showcase.md` | 洞察模块**一页看懂（可贴版）** | 关键数字 / 三个问题 / 五 Tab / 五步旅程（入口→取样→算→看→留档）与分层架构（mermaid） / 八个特点 / 规则 TOML 示例 / 入口矩阵 / 质量与验证 / 二期路线；适合贴进 PR、wiki 与聊天 |
| `insight/insight-showcase.html` | 洞察模块**一页看懂（视觉版）** | 同一套内容的卡片化排版 + 面板骨架线框 + 主题切换；自包含可离线打开，与原型页同一套设计语言 |
...
| `mock/mock-showcase.html` | Mock 数据生成**宣传 / 概览页**（自包含 HTML · 明暗双主题） | 给不了解这个模块的人**一页看懂**：英雄区四个数字（143 变体 / 0 行源数据 / 4 出口 / 100 万行上限）/ 痛点对照（手写脚本 · 导生产库 · Mock）/ 六个特点卡 / 四层架构图（依赖只向下）/ 生成六阶段流程与出口分支 + 清单状态图例 / 生成器目录 15 分类与两大亮点（分布族 · 工作日历）/ 两处排版缩略图（右 Dock + 中央 tab 带真实字段与预览）/ 三条硬边界与生成前护栏 / 质量与规模实测表（测试基线 · 目录自检 · 代码规模）|
