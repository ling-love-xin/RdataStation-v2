...
| `references/README.md` | **外部参考索引**（同类项目） | 七个参考仓（navop / zqlz / dbflux / fluxDB / dbui / sqlab / Rdata-Sidecar）的位置与**许可红线**（哪些能抄）/ **按模块查**（编辑器内核 · 结果网格 · 执行与取消 · 导航与元数据 · 驱动与插件 · 类型映射 · 分析 · 连接与隧道 · 设置主题 · 打包发布 · CI 测试 · 文档与 agent 工作流）/ 跨项目共同结论 / 五处坑 / **可落地项 R1–R10** |
| `hardening-plan.md` | **健壮性调整单（按模块 · 可直接交给 AI 会话）** | 目标是不崩/不静默/可复现/可回归/可诊断，而非功能对齐 · §1 通用约定与提交规矩 · **H-1 工程基线**（测试进门禁 750 项实测 / fmt 与告警分两步 / cargo-deny / 实机容器化 / 性能基线 / 构建隔离）· H-2 引擎与驱动（类型保真 · 错误映射台账 · 取消矩阵 · 临时表残留 · 声明同步）· H-3 导航与缓存 · H-4 编辑器与结果集 · H-5 插件（能力协商握手化 · 三道闸 · 孤儿进程实测 · 崩溃可见性 · 安装完整性）· **H-6 mock / H-7 洞察 / H-8 分析存档**（三个优势模块的可信化）· H-9 连接与凭据 · H-10 项目与路径 · H-11 UI 与主题 · H-12 发布与更新 · §14 差异点守住什么 · §15 五批顺序 · §16 许可与素材 |
...
| `data-layer-wiring-matrix.md` | 数据层**接线台账** | **有什么、谁在用、缺在哪**：判据四级（活 / 半接线 / 零调用 / 有意不做）/ 驱动接口面 / **L1 十三组 key 与 L2 十四张表的消费者逐行状态** / 持久化层方法面规模与纪律推论 / 缓存事实源纪律 / **缓做三项调查结论（稳定 ID 设计已回避 · L2 行类型转换点仍 1 处 · `dbi` 层 2100 行死代码与三套扩展清单）** / 本轮接线清单（含顺带修的两个坏 SQL）/ 已知缺口 6 条 |
| `crate-ownership-proposal.html` | crate 归属 | app / workbench / settings / Feature / shared / 资产 归属示意 |
| `module-status.md` | 模块状态与测试基线（**实测版**） | **全仓唯一一份「刚跑过」的台账**：工作区 `85 目标 / 2005 通过 / 0 失败 / 51 忽略`（另有「项目自身套件 84 目标 / 2004 项」口径）· 逐包基线（lib + 集成）· 工作台 32 个集成目标明细 · 模块接通状态 · 零调用清单 · **复现命令（含 rustup shim `Permission denied` 的绕行）** · 本轮实测发现（本机诊断脚本入库事件 / 失败目标的环境归因 / **两次复跑的漂移修正表**）· 维护约定 |
...
| `quick_open/quick-open-dev-plan.md` | Quick Open **开发方案** | 进度记录（**Phase 0 两刀 + Phase 1 四刀已落地**：模块骨架 / `List` 化 / 键盘三态 / 打开即聚焦 / 命中高亮 / 单字符门槛 / **FTS 写侧接线 + `#` 全文档档** / 元数据名称档异步接线 / 截断不静默 / 命令目录外提 / **草稿箱文件源** / **名称档索引直出提速（158 ms → 1.5 ms）**）/ 现状盘点 / 5 条决策 / 任务表 / T1–T9 测试场景 / R1–R5 风险 / 验证命令（含 shim 损坏绕行）/ 实现位置映射 |
| `quick_open/quick-open-showcase.md` | Quick Open **一页看懂（可贴版）** | 15 节：一句话 + 它替你做到的事 / 为什么不是一个「搜索框」/ 三档模式（默认 · `>` · `#`）与 2·3 字门槛 / 三个高光 / 四个剧本 / 一次搜索全流程（ASCII + 落点表）/ 结果区怎么读 / 名称档两段式与索引直出（含 10 万对象实测对照表）/ 全文档 FTS5 trigram（含语料边界与两处**已修**底座缺陷）/ 键盘与选中 / 异步与状态 / 架构（ASCII 分层 + 五条纪律）/ 尺寸与主题 / 质量与证据 / 边界与待办 / 文档地图；适合贴进 PR、wiki 与聊天 |
...
| `connection/connection-showcase.md` | 连接模块**一页看懂（可贴版）** | 与编辑器宣传页同一体例的 12 节：一句话 + 它替你做到的事 / 为什么不再是一个填 URL 的弹窗（V1 逐条对照）/ 三个高光 / 四个剧本（新建·连续配置·项目快照·连不上定位）/ 一次保存全流程（mermaid 时序 ×2）/ 两层目录 / 五 Tab / 安全与凭据 / 零造数据 / 架构（mermaid 分层）/ 质量与证据 / 边界 / 文档地图 |
| `connection/connection-showcase.html` | 连接模块**一页看懂（视觉版）** | 首屏是**新增数据源连接对话框线框**，可点着切 MySQL / PostgreSQL / SQLite / DuckDB——驱动短名、地址标签、表单与「网络」Tab 显隐、结果行落点全部联动；作用域三态内联可点；吸顶导航滚动高亮 + KPI / 对照表 / 高光 / 剧本 / 流程条 / 分层 / 质量 / 边界，与 md 一一对应，明暗双主题、离线可开 |
...
| `database/database-navigator-showcase.md` | 数据源管理 / 数据库导航**一页看懂（可贴版）** | 15 节：一句话 + 它替你做到的事 / 为什么它不是一个「连接列表」（V1 对照）/ 三条轴各归一地问 / 四个高光（密度 · 事实只读 · 记得住 · 十万表只给一页）/ 四个剧本 / 一次展开全流程（mermaid）/ 连接行解剖 / **三级缓存与取数管线（含 L2 内布局与池化）** / **大库是怎么扛住的（索引分页 · 计数 · 内省级别 · 跨连接搜索 + 仍缺项触发条件）** / 刷新预热预取 / 属性面板 / 键盘右键拖拽 / 架构 / 质量与证据（含测试基线 440+51）/ 边界 / 文档地图；适合贴进 PR、wiki 与聊天 |
| `database/database-navigator-showcase.html` | 数据源管理 / 数据库导航**一页看懂（视觉版）** | 吸顶导航滚动高亮 + 首屏线框（**新增索引搜索结果区示意**，可切归属域 facet、可开关标签 / 归属域列）+ KPI 五枚（三级缓存 · 首屏 200 条 · 一份索引三用 · 每文件一次固定开销 · 测试 491 项）+ 15 节与可贴版一一对应；自包含可离线打开，与项目 / 草稿箱 / 编辑器 / 洞察宣传页同一套设计语言 |
| `database/metadata-cache-vs-dbeaver-datagrip.md` | 元数据缓存**与 DBeaver / DataGrip 深度对比** | 两款产品的缓存取向（会话内内存 vs 持久化+内省级别 vs 本仓的“每连接 SQLite + 可管理”）/ 10 维度对照表 / 本仓三条硬约束（为何不照抄）/ 缓存侧补齐（L1 回填 · 连接池接线 · 表视图分键）/ **导航侧分页消费（§4.5，表/视图各自分块 + 加载更多）** / **搜索两档都已接：名称档 `metadata_index`（2026-09-18）+ 内容档 `metadata_fts` trigram（2026-09-19，§4.6，含两处旧硬伤已修的记录）** / 仍缺项按“对 10 万表体验贡献”排序并附**触发条件** |
...
| `editor/editor-user-guide.md` | SQL 编辑器**使用手册** | 能力与期次标注 / 入口 / 界面导览（工具栏三层 · 编辑区 · 结果区 · 状态栏）/ 典型流程 / 一次操作落到哪 / 已注册快捷键 / 只读四道防线 / FAQ 排查 / **USIT 验收清单** |
...
1. `overview.md` → 2. `crate-ownership-proposal.html` → 3. 按需进入 `layout/` / `quick_open/` / `theme/` / `settings/` / `connection/` / `database/` / `project/` / `scratchpad/` / `editor/` / `insight/` / `analytics_resource/` / `mock/` / `federation/` / `release/`
