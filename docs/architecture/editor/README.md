...
> **宣传页（一页看懂）**：[`editor-showcase.html`](editor-showcase.html)（视觉版，明暗双主题、离线可开）· [`editor-showcase.md`](editor-showcase.md)（可贴版，适合贴进 PR / wiki）
>
...
> 状态（2026-09-18）：**Phase 0 + 1a 已完成，1b（"合格的 SQL 客户端"）已收口**——执行闭环（执行族五项 / 分段抓取 / 中断与超时 / 事务 / 结果区 / 历史）、**三档执行通道**（源库 / 本地加速 / 联邦全部真接通）、导出五档（含 Parquet / XLSX 经 DuckDB `COPY`）、B9 补全（打字即补 + `Ctrl+Space` + 模板片段）、B10 格式化 / 转译 / 执行计划、B15 本地 DuckDB 分析（预置菜单 + 自定义 SQL + 血缘摘要）均已落地；**分析模式的笔记本视图属 1c，按计划搁置**。已知问题与排期的**唯一权威**是架构文档 §12。
>
...
| **折叠候选自算、机制用现成的** | 折什么由我们的纯函数算（括号组 / `CASE`·`BEGIN`·`IF` 块 / 多行注释与字面量），折起来的样子、chevron、点击折叠、开关都用内核现成的；**候选随文本重喂**（内核不会替我们平移） | 架构 D26、`fold.rs` / `view/fold.rs` |
...
| 折叠（候选纯函数 / 喂内核） | `crates/editor/src/fold.rs`（✅ 纯函数：括号组 / `CASE`·`BEGIN`·`IF` → `END` / 多行注释与字面量；档位与词法边界都在模块头）+ `crates/editor/src/view/fold.rs`（✅ 喂候选：构造一次 + 每次文本变更后重喂） |
...
10. `cargo` 命令固定 `-j 2`（并发链接重型 crate 会 OOM（DuckDB 已改动态链接））。
...
```sh
# 模块回归（编辑器服务层 + 引擎 SQL 层；含 Phase 0 原语：切分 26 / 高亮 13 / 格式化 9 / 历史 4；B17 折叠纯函数 14）
...
| `editor-user-guide.md` | **怎么用**：能力与期次标注 / 入口 / 界面导览（工具栏三层 · 编辑区 · 结果区 · 状态栏）/ 典型流程（执行族 · 结果集六件事 · 格式化转译执行计划 · 补全模板 · 事务中断 · 三通道 · 历史 · 草稿箱闭环）/ 一次操作落到哪 / 快捷键（已注册的）/ 只读与安全 / FAQ 排查 / **USIT 验收清单** |
| `editor-prototype.html` | 可交互示意稿（明暗双主题 × 三模式；含执行、结果集上限、单元运行与 stale 演示） |
| `editor-showcase.html` / `editor-showcase.md` | **宣传页（一页看懂）**：特点 / 流程 / 架构 / 质量证据一页扫完；改了实现请顺手核对页里的关键数字 |
...
| Phase 1b | 执行闭环（"合格的 SQL 客户端"，并关闭 M4 遗留的"查看数据不自动执行"）。**进度**：✅ B16（关闭口径 + 新建入口 + 工具栏执行级）· ✅ B11 + B12（**编辑器成为唯一的 SQL 编辑器**：导航四处改走 `QueryRequest`，旧 `EditorPanel` 的 SQL 区块与内联执行闭包已删，M1 草稿拦截改读 `EditorShared`；“查看数据”现在打开即执行）· ✅ B14（筛选 / 排序本地那档 + 下发源库那档都收圆）· ✅ B8（历史面板在右 Dock）· ✅ B7 / B5 / B5b / B6 / B2 / B3 / B4（导出 / 结果区 / 分段抓取 / 错误定位 / 执行族 / 中断 / 事务）· ✅ **B10 格式化 / 转译 / 执行计划（三个切片全完成）**：格式化与转译共用脚本骨架（**脚本一条不丢**）· 工具栏「格式化」+ `Ctrl+Shift+F` · 「⋯ 更多 ▾ ▸ 转译为」十个目标 · 执行计划**按通道取方言**且落新结果集贴标题 · 🟡 B13 执行通道（**切片一 / 二 / 三前半已完成**：三档互斥 + 门控真值 + 随会话持久化 + 切通道旧结果标灰与提示 + **加速档真接通**（`ATTACH … READ_ONLY` / `USE` 解析表名 / 四类执行全分流）+ **「重新挂载源库」入口** + **历史带通道字段**；余：联邦外部源入口）· ✅ **B1（连接绑定，切片一 + 收尾 2026-09-18）**：文档属性 + 工具栏选择器 + 状态栏 + 执行真的用它 + **绑定跨重启**（`editor_contexts.connection_id` 写真值；恢复时校验连接还在不在，不在就回退并说原因）；⬜ 余：只读锁 ⓘ（要连接侧写策略来源） · ✅ **项目只读对执行的拦截已补回（2026-09-18：`editor/src/project.rs` 端口 + 提交前拒写，判据与通道闸同一份）** · ✅ **B9 补全（两切片全完成 2026-09-18）**：打字触发 + **`Ctrl+Space` 手动触发**（与打字那条同一份候选）· 内建 provider（零自建下拉）· 候选来自导航缓存（后台预载 + 按通道限定名：联邦是 `别名.schema.表`）· **模板片段**（「⋯ 更多 ▾ ▸ 插入模板」，插到光标处并选中 `{table}`）；⬜ 余：缓存未命中时回落实时内省 · 🟡 **B15 DuckDB 分析（切片一 / 二 + 血缘已完成 2026-09-18）**：结果集「⚗ 分析 ▾」→ 本地内存 DuckDB 上对**已抓到的行**跑聚合（**不重跑源库**，上限 2 万行并如实说明）→ **新结果集**（原结果一行不动、不抢选中）；预置菜单 = 计数行数 / 逐列分组计数，底部「自定义分析 SQL…」弹输入框（预填模板，`{table}` 指桥接出来的临时表，空白在本地就拒）；结果工具栏左段带**来源摘要**（`原查询` / `下发筛选` / `排序下发` / `取下一段` / 标题中的执行计划 / 分析）· ✅ **B7 导出两切片全完成（2026-09-18）**：CSV / JSON / INSERT 编辑侧编码 + **Parquet / XLSX 经 DuckDB `COPY`**（一次性线程 + 回执，不堵 UI：XLSX 首次要装 excel 扩展；`HEADER true` 必须显式传）；⬜ 余：列类型化输出（CSV / JSON 仍是展示文本）· 沉淀为分析单元（1c 已搁置） |
