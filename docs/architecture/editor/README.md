...
| Phase 1a | ✅ **A1–A15 全部完成**（A13 的“关折叠”只差内核开关）：服务层 · Dock 标签面板 · 内核视图 · SQL 高亮 · 模式切换矩阵 · 只读两维度 · 状态栏 · 脏状态 · 持久化 + workbench 接线 + 聚焦已存在标签 · Actions 与快捷键 · 查找 / 替换（内核 + 组件库，零自建）· 会话持久化（真 SQLite 实测）· 大文件档位（真 200MB 稀疏文件实测）· 最小执行 + 结果网格（真机四库实测）· ui_contract 契约。⬜ 余：A9 对话框收尾（另存为 / 关闭三态 / 模式切换确认 / 系统文件对话框）如 1b 一并做 |
