...
> 状态：**Phase 0 已完成 + Phase 1a 进行中（2026-09-15）**——地基地基部分落地：语句切分（`engine/src/sql/split.rs`）、SQL 高亮（`engine/src/sql/highlight.rs`，13 项回归）、格式化（sqlglot generator）、历史字段真实化、`crates/editor` 骨架；**编辑器已进工作台中央 Dock**（多文档标签、脏点、状态栏、SQL 着色、`Ctrl+S` / `Ctrl+/` / `Ctrl+W` 均已实测生效）。已知问题与排期的**唯一权威**是架构文档 §12。
>
...
| Phase 1a | ✅ 已到：A1 服务层 · A2 面板（Dock 标签侧）· A3 内核视图 · A4 SQL 高亮 · A5 模式切换矩阵 · A6 只读两维度 · A7 状态栏 · A8 脏状态 · A9 持久化 + workbench 接线 + 聚焦已存在标签 · A10 Actions 与快捷键（`ctrl-s` / `ctrl-/` / `ctrl-w`）／余：A11 查找 · A9 对话框收尾（另存为 / 关闭三态 / 模式切换确认 / 系统文件对话框）· A14 最小执行 + 结果网格 |
