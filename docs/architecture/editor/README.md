...
| Phase 1a | ✅ 已到：A1 服务层 · A2 面板（Dock 标签侧）· A3 内核视图 · A4 SQL 高亮 · A5 模式切换矩阵 · A6 只读两维度 · A7 状态栏 · A8 脏状态 · A9 持久化 + workbench 接线 + 聚焦已存在标签 · A10 Actions 与快捷键 · A11 查找 / 替换（用内核 + 组件库能力，零自建）· **A12 会话持久化（光标 / 选区 / 模式跨重启，真 SQLite 实测通过）** · A14 最小执行 + 结果网格（真机四库实测通过）／余：A9 对话框收尾（另存为 / 关闭三态 / 模式切换确认 / 系统文件对话框）· A13 大文件档位 · A15 契约 |
