...
| B13 | **执行通道**：工具栏「执行位置」指示器（源库 / 本地加速 / 联邦）+ `channel_status(conn_id) -> Result<(), Reason>` 可用性判定 + 不可用原因 + 通道随文档持久化 + 切通道失效提示 | 🟡 **切片一已完成（2026-09-17，见 §0）**：`channel.rs`（三档互斥 + 能力边界 + 门控端口 + 纯函数菜单/文案/写拒绝）· `Document.channel` 随会话持久化（引擎 `editor_contexts.channel` 列）· 工具栏「执行位置 ▾」+ 状态栏通道段 + 结果集通道徽标（`·旧`）+ 切通道后顶部一行提示 · 加速 / 联邦拒写源库且**不打扰执行器** · 宿主门控按“谁能先改”报第一条缺口。⬜ 余：**切片二**——加速档真接 `engine::duckdb::federation::attach_data_source` + Secret（扩展安装走外部编译库、指定目录），接上后把 `editor_channels::execution_supports_accelerated` 翻成 `true`；**切片三**——联邦外部源注册入口（菜单项“注册新源…”）· 历史标记通道 · 执行计划按通道 | `crates/editor/src/channel.rs` + `view/host.rs` + `services/editor_channels.rs` + `engine::duckdb::{federation,extensions}` | 集成：三通道各跑一次（⬜ 待切片二）· 未开加速的连接下拉置灰并给原因（✅）· 写语句在加速通道被拒（✅）· 切通道后旧结果标灰 + 提示（✅） |
...
30. 通道门控：未开本地加速的连接 → 执行位置中加速项置灰且行尾有原因；联邦无源时给“注册新源…”入口（✅ 2026-09-17 切片一：置灰 + 行尾原因已落实，且**宿主门控只报第一条过不去的条件**——连接开关 → 版本能力 → 运行时就绪；“注册新源…”入口随切片三）
31. 通道切换：源库 → 加速后旧结果集标灰 + 顶部提示；分析模式下全部单元标 `stale`（✅ 2026-09-17 切片一：旧结果保留 + 标签缀 `·旧` 转静色 + 顶部一行“旧结果来自 X”；分析模式的单元 `stale` 随 1c）
32. 通道写限制：加速通道下 `INSERT` 被拒并提示切回源库（不得落本地副本）（✅ 2026-09-17 切片一：判据在引擎语句类型上（DML/DDL），**提交前**就拒、执行器一次都没被叫；本地临时对象仍放行）
