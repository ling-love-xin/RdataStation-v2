...
| B5 | 结果区完整化：结果集标签条（上限 5 + 淘汰 + **通道徽标 + 血缘摘要**）、工具栏（行数/耗时/连接/**筛选 + 下发开关**/**分析**/导出/刷新/复制）、截断提示、`affected_rows` 展示 | ✅ **已完成（2026-09-17，见上一行）**：标签条（B2）+ **影响行数**（引擎侧六个驱动全给真值）+ 结果工具栏（来源连接 / 截断提示 / 复制 / 刷新，另半属 B15 / B14 / B7）+ 可拖拽分栏。⬜ 余：标签的**通道徽标**（B13）与**血缘摘要**（B14/B15）；工具栏的筛选 / 下发开关 / 分析 / 导出各自随所属步骤落地（**没实现就不摆按钮**） | `view/widgets/result_sets.rs` + `result_grid.rs` + `store.rs` | 窗口/真机：滚动流畅；DML 显示影响行数（✅ 六驱动）；标签显示通道与血缘（⬜ B13 后） |
...
| B6 | 错误回填：错误位置解析（多家驱动格式）→ 诊断 + 定位 + 聚焦；无位置则只提示 | ✅ **已完成（2026-09-17，见首行）**：`diagnostics.rs`（纯函数解析，认不出就不给位置）+ 引擎侧 PG 两条路径的结构化位置与真实消息 + 面板「诊断 / 定位 / 聚焦」三件事。⬜ 余：错误卡片（与 B3 的「已中断」终态合流）· 其余驱动的结构化位置（文本里已带，暂不重复做） | `execution.rs` + `view/` + `driver/utils.rs` | 单测：≥8 种真实错误文本（✅ 10 类）；真机：故意写错列名可定位（✅ 六驱动） |
