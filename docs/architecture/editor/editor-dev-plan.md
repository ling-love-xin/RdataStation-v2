...
| B14 | **筛选下发源库**：筛选框 + `▢ 下发源库` 开关 → `re_execute_with_filter` → **新结果集**（lineage = 原结果 + 条件 + 通道） | ✅ **已完成（2026-09-17；三个切片见 §0 的三行）**：本地筛选与本地排序 · 下发源库（引擎改写 + 方言差异在宿主 + 新结果集 + 去 LIMIT 提示）· **表格右键菜单**（按值筛选 / 复制此值 / 冻结列用原生 `Column.fixed`）。⬜ 余：右键「排序下发 SQL」（与筛选下发同构）· 值预览弹层与值编辑 · 血缘摘要（依赖 B13 通道信息） | `execution.rs` + `engine/src/sql/filter.rs` + `view/results/grid.rs` + `services/editor_exec.rs` | 集成：本地筛选不重查（✅）· 下发产生新结果集且原结果保留（✅ 真机六驱动）· 含 `LIMIT` 的原查询给出提示（✅ 状态栏） |
