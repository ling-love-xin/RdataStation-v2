...
| B7 | **导出**：CSV / JSON / INSERT / Parquet / XLSX（后者经 DuckDB 临时表） | 🟡 **切片一完成（2026-09-17，见首行）**：CSV / JSON / INSERT 三格式 + 两档范围（仅已抓取 / 抓全量后导出）+ 路径选择端口（宿主 `rfd`）+ 默认名从 SQL 猜表名。抓全量是**多轮取段**，失败 / 切结果集 / 提交被拒不落半截文件。⬜ 余：**Parquet / XLSX**（要经 DuckDB `COPY … TO … (FORMAT …)` 与对应扩展，属切片二）· 导出当前筛选后的行集（B15） | `execution.rs` / `export.rs` | 集成：**CSV / JSON / INSERT 落盘可回读（✅ 单测 + 窗口测试）**；Parquet / XLSX（⬜ 切片二） |
...
| B10 | 格式化 / 转译 / 执行计划接线（用 P0.3 的格式化实现；EXPLAIN 按方言生成） | 🟡 **切片一（格式化）已完成（2026-09-18，见 §0）**：`format_with_report` 按语句区间**原位回填**（区间外一个字节不动）+ `editor/src/format.rs` 的 `plan`（选区优先 / 光标映射 / `changes` 口径）+ 工具栏文档级「格式化」与真注册的 `Ctrl+Shift+F`（写回保留撤销历史）· 方言只从 `ConnectionOption.db_type` 来。⬜ 余：切片二方言转译 · 切片三执行计划 | `commands.rs` + `engine/src/sql/{formatter,engine}.rs` + `editor/src/format.rs` + `engine::sql::SqlEngine` | 格式化结果可往返解析（✅ sqlite/duckdb 真机）；EXPLAIN 在 4 类库均返回（⬜ 切片三）。**硬约束（已实测，§12 #19）**：转译**必须**先按 `sql/split.rs` 切分再逐条转译——`transpile` 对脚本会**静默丢弃**第二条及以后的语句；并先写“脚本不得丢语句”的回归；执行计划的**权威来源是源库 EXPLAIN**（本地 `plan` 仅作降级，DDL 不支持） |
