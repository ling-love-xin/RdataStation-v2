...
| B14 | **筛选下发源库**：筛选框 + `▢ 下发源库` 开关 → `re_execute_with_filter` → **新结果集**（lineage = 原结果 + 条件 + 通道） | 🟡 **切片一完成（2026-09-17，见首行）**：**本地筛选 + 本地排序**（视图层，不重查；300ms 防抖；工具栏明示“已筛选 N / M 行”；导出跟随筛选）· 列头点击排序（原生 `Column.sort` + `perform_sort`，数值优先）。⬜ 余：`▢ 下发源库` 开关与重查（拼 `WHERE` → 新结果集 + 血缘）· 右键「按值筛选」与「排序下发 SQL」· 冻结列 | `execution.rs` + `view/results/grid.rs` + `export.rs` | 集成：本地筛选不重查（✅ 单测：数据行一行不动）· 下发后新结果集且原结果保留（⬜ 后半）· 含 `LIMIT` 的原查询给出提示（⬜ 随后半） |
