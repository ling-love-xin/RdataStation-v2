...
| B5 | 结果区完整化：结果集标签条（上限 5 + 淘汰 + **通道徽标 + 血缘摘要**）、工具栏（行数/耗时/连接/**筛选 + 下发开关**/**分析**/导出/刷新/复制）、截断提示、`affected_rows` 展示 | 🟡 **B2 已落地其中一半（2026-09-16）**：结果集标签条（`view/widgets/result_sets.rs`，组件库 `TabBar`）+ **上限 5 + 淘汰最旧（未选中项）+ 失败项 `danger` 点** + 选中项切换；⬜ 余：标签的**通道徽标**（依赖 B13 的通道状态）与**血缘摘要**（依赖 B14/B15 的派生结果集）、结果工具栏（筛选/下发开关/分析/导出/刷新/复制）、截断提示、`affected_rows` 展示 | `view/widgets/result_sets.rs` + `result_grid.rs` + `store.rs` | 窗口/真机：滚动流畅；DML 显示影响行数；标签显示通道与血缘 |
