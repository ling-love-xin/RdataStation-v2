...
| A4 | **SQL 高亮**：注册 grammar + 方言选择 + 主题角色映射（缺角色复用最接近标准 token） | **方案已按 API 核实修正（2026-09-15）**：组件库自带的高亮器**不含 SQL grammar**（只有 go/js/json/rust/markdown/…），因此**不走 tree-sitter**（与 D12 一致）；正道是 gpui-base 的 **`DocumentRangeSemanticTokensProvider`**（外部语义着色）：`legend()` 给出 token 类型名、`semantic_tokens()` 返回 delta 编码区间，**颜色由活跃 `HighlightTheme` 按名字解析**（主题一切换自动重着色，天然零裸色值）。落点：`crates/editor/src/view/highlight.rs`（`engine::sql::highlight::highlight_spans` → 语义 token） | 待实现：① `TokenClass` → 主题词汇（`keyword`/`type`/`function`/`string`/`number`/`comment`/`operator`/`punctuation`；`Parameter` → `variable`；`Identifier` **不上色**）② 大文件档位（超阈值不上色，A13）③ 真机核对明暗两套可辨 |
