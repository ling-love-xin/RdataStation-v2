...
| B14 | **筛选下发源库**：筛选框 + `▢ 下发源库` 开关 → `re_execute_with_filter` → **新结果集**（lineage = 原结果 + 条件 + 通道） | ✅ **已完成（2026-09-17；切片一见“本地筛选与本地排序”那行，切片二见首行）**：本地筛选（视图层、300ms 防抖、导出跟随）· 本地排序（原生列头）· **下发源库**（引擎改写 `filter.rs`：包一层 + WHERE + 去 LIMIT + ORDER BY 提到外层；方言差异在宿主；结果落新结果集；去掉 LIMIT 有提示）。⬜ 余：右键「按值筛选」与「排序下发 SQL」（随右键菜单一起做）· 冻结列的列级开关 · 血缘摘要（依赖 B13 通道信息） | `execution.rs` + `engine/src/sql/filter.rs` + `view/results/grid.rs` + `services/editor_exec.rs` | 集成：本地筛选不重查（✅）· 下发产生新结果集且原结果保留（✅ 真机六驱动）· 含 `LIMIT` 的原查询给出提示（✅ 状态栏） |
