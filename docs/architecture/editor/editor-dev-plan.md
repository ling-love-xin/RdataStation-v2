...
| A5 | **模式判定与切换**：规则表（§1.2）+ 切换矩阵（§1.3，含确认对话框与内容变换）；能力表驱动 chrome | ✅ **逻辑已完成**：`mode.rs` —— 判定（记忆 > 扩展名 > 文本）+ **切换矩阵**（`plan_switch` 纯函数：确认类型 / 内容变换 / 提示语；禁止静默切换）+ 内容变换（`sql_to_cells` 走 `engine::sql::split`、`cells_to_sql`、`cells_from_text` / `cells_to_text`，文本层分隔标记 `-- %%`）；视图：`EditorHostPanel::sync_mode` 按模式刷新（着色 / 只读）。⬜ 待接：确认**对话框**（需对话框宿主，随 A9 打开/保存入口一并接） | 单测 **11 项**（矩阵逐项 + 拆分粒度二选一 + 字符串内分号不被误拆 + 单元文本往返 + 空输入）+ 窗口测试：切模式后 `sync_mode` 与渲染正常 |
...
| A9 | 保存 / 另存为 / 外部修改检测 / 关闭三态确认（保存失败二次确认） | ✅ 持久化层 + **workbench 接线已完成**：`persist.rs`（读 / 写 / mtime 外部修改检测 / `open_file` / `save_document` / `save_as`）· `EditorHostPanel::save` · `WorkbenchView` 新增 `editor_service` + `editor_hosts`，中央 tab 组用 `DockLayout::tabs().panel_view(旧面板).panel_view(编辑器面板)` 接入，并新增 `open_in_editor(path)`（走 `DockArea::add_panel`）· **聚焦已存在标签已完成**（`focus_self` 走 `TabGroup::select_tab`，并同步服务层当前文档）。⬜ 余：另存为 / 关闭三态对话框 · 模式切换确认对话框 · 系统文件对话框（打开 / 另存为） | 单测 8 项（持久化）+ `cargo check --workspace --all-targets` 零告警（editor → workbench → app 链跑通） |
...
| A10 | Actions 与快捷键：`Ctrl+S` / `Ctrl+Enter`（最小执行=执行全部）/ `Ctrl+/` / `Ctrl+F` / `Ctrl+Shift+F` | ✅ **可独立完成的部分已完成**：`commands.rs`（Action 声明）· `edit.rs`（行注释纯函数，13 项单测）· 面板 `key_context("editor")` + `track_focus` + 三个 `on_action` · 状态栏“失败原因”段 · `close_document_in_dock`（宿主执行关闭）· app 层注册 `ctrl-s` / `ctrl-/` / `ctrl-w`。**5 项真按键盘的窗口测试**（写盘 / 报原因 / 注释可逆 / 只读拒绝 / 宿主关闭）。⬜ 余：`Ctrl+Enter`（A14 执行）· `Ctrl+F` `Ctrl+Shift+F`（A11 查找 / B10 格式化）——**未实现不注册、不宣传** | 真机：已注册键全部生效（**禁止“只宣传未注册”**）；单测层面已用 `simulate_keystrokes` 逐键验证 |
