...
| B16 | **接住 Dock 标签 ✕ 的关闭请求**（A9 尾巴）：让标签 ✕ / 项目切换等所有关闭路径都走 `request_close_document`（三态确认）；允许的实现：自绘标签条、或对脏文档让 `closable(cx)` 返回 `false`（至少不静默丢）· 顺带把工具栏其余控件按原型 §2.2 分层补齐（执行族 ▾ / 格式化 / 历史 / ⋯更多 / 执行位置 / 连接） | `view/host.rs` + `crates/workbench`（容器侧） | 窗口测试：脏文档点 ✕ 不得直接丢改动（要么弹确认、要么无 ✕）；工具栏每个按钮都有真动作（**不允许“只宣传不实现”**） |
