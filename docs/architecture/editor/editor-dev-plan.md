...
| B13 | **执行通道**：工具栏「执行位置」指示器（源库 / 本地加速 / 联邦）+ `channel_status(conn_id) -> Result<(), Reason>` 可用性判定 + 不可用原因 + 通道随文档持久化 + 切通道失效提示 | 🟡 **切片一 + 切片二已完成（2026-09-17 / 09-18，见 §0）**：切片一 = 三档状态 + 门控端口 + 随会话持久化 + 切通道旧结果标灰与提示 + 提交前拒写；切片二 = **加速档真接通**（`duckdb/accel.rs`：按源缓存专用 DuckDB 连接 · `ATTACH … READ_ONLY` · `USE` 解析表名 · 第一段/下发/排序/取下一段全部分流 · 执行也进历史 · 门控真值含扩展可用性）。⬜ 余（切片三）：联邦外部源注册入口（菜单项“注册新源…”）· 「重新 ATTACH」界面入口 · 历史标记通道与执行计划按通道（正规字段）· 加速会话临时表与分析会话共享 | `crates/editor/src/channel.rs` + `view/host.rs` + `services/{editor_channels,editor_exec}.rs` + `engine/src/duckdb/accel.rs` | 集成：三通道各跑一次（🟡 源库 + 加速已通，联邦待切片三）· 未开加速的连接下拉置灰并给原因（✅）· 写语句在加速通道被拒（✅ 双保险）· 切通道后旧结果标灰 + 提示（✅） |
