...
| 18 | ⚪ | **Dock 无“关闭前否决”钩子**（已静态核实 2026-09-15）：`DockArea` 订阅 `TabGroupEvent::ClosePanel` 后直接 `remove_panel_id`；`Panel::closable(cx)` 是唯一闸门（静态许可，不能问用户）；`remove_panel` / `with_renderer` 均为 `pub` | 决定了“标签 ✕ 能否弹未保存确认”——需要自绘标签条才能做到 | 见 §13 #15：默认走“草稿兜底”（关闭即落草稿），需要弹窗时再上自定义标签条。**2026-09-16 补充（A9）**：三条关闭路径现在口径不一——`Ctrl+W` 走宿主 `request_close_document`（**三态确认**）；标签 ✕ 仍走 Dock 直移除（**草稿兜底**：`on_removed` 落一份会话，重开时内容从会话回来的，不是磁盘）；`CloseProject`/退出类同理。1b 处理：要么接住 Dock 的关闭请求（需容器侧钩子）并把 `request_close_document` 复用上去，要么在 `closable(cx)` 里对脏文档返回 `false`（收起 ✕）——**不能继续默默依赖草稿兜底**（用户看不出“关一下到底丢没丢东西”） |
