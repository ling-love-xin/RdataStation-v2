...
| 14 | 🟡 | **加速通道新鲜度**（ATTACH 快照 vs 实时）无表达、无重建入口 | 用户可能拿旧快照做决策 | **已修正并落地一半（2026-09-18，B13 切片二）**：实测**数据是实时的**（源库插一行，同一 DuckDB 会话立刻看得到），但**表清单**在 `ATTACH` 时定型——所以界面写的是「源库只读」而**不是“快照”**，引擎侧已有 `AccelSession::refresh`（= 重新 ATTACH）；⬜ 余：界面入口（切片三）。⬜ 另：加速会话的临时表与分析会话（1c）暂不共享 |
