...
| 联邦查询（注册外部源 / 建外部表） | `engine::services::sql_service.rs`（`register_external_database` / `create_external_table`）+ `engine/src/duckdb/federation.rs` | 零调用 |
| DuckDB Secret（本地加速通道） | `connection/src/secret.rs`（`CREATE PERSISTENT SECRET` + `secret_directory`） | 已接（M3 保存连接时按 `use_duckdb_fed` 门控注册） |
...
| D21 执行通道（源库/加速/联邦）与门控 | `crates/editor/src/channel.rs`（新增）+ `connection/src/secret.rs` + `engine/src/duckdb/federation.rs` |
...
| 13 | 🔴 | **通道可用性判定无来源**：`use_duckdb_fed` 只是“是否注册 Secret”的开关，但“Secret 是否注册成功 / 能否 ATTACH”没有可查询状态 | 通道门控只能靠猜，UI 给不出准确的“不可用原因” | 1b 前定义 `channel_status(conn_id) -> Result<(), Reason>`（读连接管理器 / Secret 注册记录） |
| 14 | 🟡 | **加速通道新鲜度**（ATTACH 快照 vs 实时）无表达、无重建入口 | 用户可能拿旧快照做决策 | 结果集徽标 + 状态栏“快照”提示 + “重新 ATTACH”入口（原型 §5.7） |
...
| 24 | 🟡 | **键位被编辑内核占用的风险**（**读源码 + 实测，2026-09-15**）：`gpui-base` 在 `Input` context 里绑了 `ctrl-f` → `input::Search`（以及 `ctrl-h` / `ctrl-a` / `ctrl-z` …，`input/base/state.rs:132` 起）。绑定按 **context 深度**优先（`Input` 比面板的 `editor` 深），所以 `ctrl-f` 会被内核先拿到；而 0.6.1 的 `Search` **没有 handler**（只有匹配引擎与高亮渲染，写会话的 `search_session` 是 `pub(super)`）→ 目前 `ctrl-f` 在编辑器里是**空按键**。好消息：`Window::dispatch_key` 会**逐个尝试匹配上的 binding**，前一个没人处理就继续到下一个（所以 `workbench` 的 `FocusNavSearch` 仍能收到） | 误以为“绑上就生效”；将来内核补上 handler 时，已注册的 `ctrl-f` 会静默改变归属 | 改键位前先看 `gpui-base/src/input/base/state.rs` 的 `init`；A11 做查找时**优先接内核的 `Search` 会话**（若仍无 handler 则提 issue），不要另造一套；已在本 crate 的 `commands.rs` 文件头写明 |
...
| 26 | 🟡 | **执行通道没有连接绑定**（**1a 划定范围，2026-09-15**）：`EngineQueryRunner` 调 `SqlService::execute(None, …)`，即 `DEFAULT_CONN_KEY = "active"` → **连接管理器里的活动连接**。用户在导航里选了哪个库就发到哪个库，但编辑器界面上**没有地方看得出这件事** | 多连接场景下“这条 SQL 发到哪了”不可知；与原型 §5.7「执行位置（源库/加速/联邦）」相差一档 | 1b 第一件事：文档/标签级连接绑定 + 工具栏右侧「执行位置」；在那之前若误发到别的库，结果只能从结果区的内容自己推断（不静默报错，但也不足以防错） |
