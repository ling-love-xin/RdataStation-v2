...
| 特点 | 含义 | 出处 |
| --- | --- | --- |
| **业务在 crate、视图也在 crate** | 递归复制 / 搜索 / 替换 / 引用重定位等语义与**面板视图**都在 `crates/scratchpad`（37 项单测：域逻辑 + 面板纯函数 + 后台任务）；workbench 只剩宿主端口实现与左 Dock 装配 | 架构 §7.1 |
...
# 单测（37 项：模块根隔离 / 路径防护 / 回收站来源 / 跨模块还原拒绝 / 旧布局迁移 / 引用状态与重定位 /
