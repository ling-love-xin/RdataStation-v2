...
> 状态：**设计已定稿 + 目录已落位（2026-09-18）**——`crates/engine/src/duckdb/federation/` 已建成
> （`legacy` 是迁移前的旧实现；`registry` / `session` / `bridge` 目前只有职责与接口草案）；
> 实现按 `federation-dev-plan.md` 的分期推进，**第一期未开工**。
...
| 源登记与状态快照 | `.../federation/registry.rs`（🟡 第一期） |
| 多源会话与挂载 | `.../federation/session.rs`（🟡 第一期） |
...
| 通道档位与门控（编辑器侧） | `crates/editor/src/channel.rs` + `crates/workbench/src/services/editor_channels.rs`（✅ 已有 Federated 档与门控端口） |
...
7. **状态如实、命名如实**：徽标写“联邦”不写“快照”；桥接数据标“拉取于 HH:MM 的副本”。
8. **零裸值**（视图层）：颜色走主题 token、尺寸进 `ui.rs`（契约测试会拦）。
...
# 多源真机（第一期起）：MySQL / PG / SQLite / DuckDB 四源互挂
cargo test -p rds-workbench -j 2 --test editor_exec_real -- --nocapture
```
...
第一期（多源挂载）的开工条件已齐：目录落位 ✅ · 选型实测 ✅ · 旧实现迁移 ✅。
任务清单见 `federation-dev-plan.md` §2。
