...
> 状态：**设计定稿 + 目录落位已完成（2026-09-18）；第一期未开工**——进度见 §0 表（最近在前）。
...
| 多源挂载 | ⛔ 无 | 旧的 `federation/legacy.rs` 是另一套模型（进程内状态、无只读、无凭据加密），正被取代 |
...
| T1.2 | **源登记**：连接表加“用作联邦源”标记（幂等补列）；`SourceRegistry` 快照（别名 / 状态 / 表数 / 刷新时间）+ 别名重名检测 | `.../federation/registry.rs` + `engine/src/persistence/workbench_context_store.rs`（或连接所在库） | 单测：标记往返 · 快照是内存读（无 I/O） · 重名检测给出冲突别名 |
| T1.3 | **多源会话**：`FederatedSession::ensure/mount/refresh`——逐源 `ATTACH … (READ_ONLY)`，**失败不中断**（记原因继续）；主源 `USE`；按源 `DETACH`+`ATTACH`；会话资源边界（`memory_limit` / `temp_directory` / `max_temp_directory_size`） | `.../federation/session.rs`（复用 `accel` 的 `MOUNT_LOCK` 与回执模式） | 真机：四类源互挂 · 一个源故意写错密码 → 其余源仍可用且该源带原因 · 重挂后新表可见 · 写源库被拒（双保险） |
