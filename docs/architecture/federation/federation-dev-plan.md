...
> 状态：**第一期（多源挂载）执行路径已通（2026-09-18）**——引擎侧 + 工作台侧均落地，
> 编辑器里选「执行位置：联邦」能跑跨源 join（徽标 `联邦`、历史带参与源）。
> 剩余：源清单浮层与「用作联邦源」标记存储（见 §2 的 T1.2 / T1.6）。
...
| T1.2 | **源登记**：连接表加“用作联邦源”标记（幂等补列）；`SourceRegistry` 快照（别名 / 状态 / 表数 / 刷新时间）+ 别名重名检测 | `.../federation/registry.rs`（✅ 形状已落）+ `engine/src/persistence/workbench_context_store.rs`（⬜ 存储未接） | **本期口径（待 T1.2 替换）**：候选源 = 已连接 + 开「DuckDB 本地加速」+ 驱动可挂（开关的含义就是“可以被 DuckDB 直连”，凭据/Secret 也挂在它上）；单测：快照是内存读 · 别名重名给唯一名（✅ 已测）· 源清单组装（✅ `plan_from_infos` 三例） |
...
| T1.5 | **执行路径**：`QueryRunner` 的 federated 分支在联邦会话上跑（复用 `run`/`run_filtered` 的分流形状）；历史带**参与源清单** | `crates/workbench/src/services/editor_exec.rs` + `engine/src/persistence/history_store.rs`（✅ 本期） | ✅ 单测：源清单组装 / 不够源时拒 / 门控原因；历史 `sources` 字段往返（✅）；真机：走 `federation_probe` + `federation_credentials_probe`（端到端在窗口里跑，见 `editor_exec_real`） |
