...
| **跨连接搜索** | 搜索框 ≥ 2 字符 | 后台扫每个连接自己的索引（中缀匹配）；单连接 50 条 / 总量 300 条封顶；渲染上限 100 条 | 交互操作不该被超大 schema 拖死；**没有缓存的连接不建文件**（不会因为敲了个字就落一堆空缓存） |
...
| 缺什么 | 影响 | 什么时候必须做 |
| --- | --- | --- |
| **FTS 内容搜索**（注释 / 数据类型 / 源码） | 今天搜的是**名称**，不是全文 | 出现「注释里有这个词」的抱怨时。前置：修 FTS 写侧（它的视图插入引用了不存在的 `views` 表），并让注释先进缓存 |
| **虚拟列表** | 首屏只取 200 条已经够用，但 1 万条同屏仍未验证 | 真实 10 万表库实测出现滚动卡顿时 |
| **搜索结果「在树中定位」** | 搜到了能在属性面板看，但不能一键展开并高亮到树上那一层 | 用户开始用搜索当主要入口时 |
...
导航侧单测    cargo test -p rds-database --lib    →  42 passed / 0 failed
