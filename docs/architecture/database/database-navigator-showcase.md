...
>
> 本页是**可贴版**（纯文本 + 表格，适合贴进 PR / wiki）；视觉版见 [`database-navigator-showcase.html`](database-navigator-showcase.html)
> （自包含、明暗双主题、离线可开，首屏可在归属域 facet 间切换、可开关「显示标签 / 显示归属域」）。
> 两版章节一一对应；视觉版把面板线框放在首屏，本页把它拆进第 6 节。

---

## 一句话

**十几个库，一眼找到那一个——展开就自动连上，双击就看细节。**

它不是一个「连接列表」：组织（分组）、检索（标签 / 筛选）、事实（属性面板）各占一个位置，互不打架；
对象树逐级懒加载，元数据有缓存，展开连接时若未建连会**先自动建连**。
...
| 这一条是哪来的 | 归属域是**事实属性**：`P` 项目 / `G` 全局 / `GP` 共享，行内**右对齐固定列**（可关）+ 筛选，不参与组织 |
| 屏幕上会不会太挤 | **行内常驻 ≤ 3 个元素**：徽标 + 名称 + 归属域列；`+`（加标签）与 `✎`（编辑）只在 hover / 选中时显 |
| 状态一眼看得出来吗 | 徽标**双通道**：颜色 = 状态（灰 = 不可用 / 彩 = 可用），形状 + 内叠 2 字母 = 数据库类型；hover 300ms 出类型 / 状态 / 驱动 |
| 展开会不会卡 | **render 期零 I/O**：加载 / 属性 / 预热 / 预取全在后台工作线程；大 schema 首批 200 条 + 「加载更多」+ 邻接预取 |
...
### HIGHLIGHT 3 · 展开不卡，错了有原因

树加载、属性、预热、预取全部进**单一后台工作线程 + 结果队列**；渲染期不做 I/O。
大 schema 靠类别文件夹分页顶住，首次展开表文件夹后还会顺手**预取前 20 张表的列**。

`首批 200 条 + 加载更多` · `预热可取消 · 有进度` · `节点级失败可读原因 + 重试`
...
2. **再精确**：搜索框支持 facet 语法，与 chips 互补 —— `type:postgres` / `scope:global` / `tag:prod`。
   `type:` `scope:` `tag:`
3. **把常用固定下来**：拖连接行到**分组头**即归组；拖到「未分组」头 = 移出全部；分组头之间拖 = 调分组顺序。
   `拖到分组头` `Alt+↑↓`
...
```
① 面板就绪          归属域 chips + 筛选 + 搜索常驻；分组 / 标签本地小读走 defer_in      NavView
② 展开连接根        未建连先自动建连；建连失败只给可读原因 + 重试，不排加载            NavHost ▸ 连接服务
③ 请求下一级        只提交路径（连接 / 库 / schema / 表…），不阻塞 UI                 nav_jobs 入队
④ L1 内存           本会话取过的节点直接命中                                          NavigatorService
⑤ L2 每连接 SQLite  跨重启缓存；命中即渲染，不必碰数据库                              cache-aside 回写
⑥ L3 实时内省       驱动按能力位内省（有无独立 Schema 层由驱动声明）；结果回写 L2       MetadataService ▸ MetadataBrowser
⑦ 回填渲染          后台线程产出结果，主线程轮询取回；类别首批 200 条 + 加载更多        结果队列
⑧ 顺手备好下一层    表文件夹首次展开后预取前 20 张表的列；连接成功后可提交预热          预取 / 预热
```
...
## 7. 四级取数管线（cache-aside）

| 级 | 是什么 | 说明 |
| --- | --- | --- |
| **L1 内存** | 会话级 | 本会话已取过的节点；命中即返回，零磁盘零网络 |
| **L2 每连接 SQLite** | 跨重启元数据缓存 | 按存储域分区：项目 / 共享连接落 `project.db`，全局连接落 `global.db`；可迁移、带版本 |
| **L3 实时内省** | **不是缓存** | 驱动 `MetadataBrowser` 直连目标库；结果按 cache-aside 回写 L2 |
| **层级** | 随数据库类型 | 有没有独立 Schema 层由驱动能力位声明，渲染层不分叉 |
...
| 动作 | 粒度与行为 |
| --- | --- |
| **刷新** | `⟳` 刷新当前选中连接；`⋯ → 刷新全部元数据` 清掉已加载子节点后重载仍展开的连接根；节点右键刷新只重建该范围。**保留展开态**，重新渲染时按需再懒加载 |
| **预热** | 连接成功后提交后台内省 catalogs / schemas 写入 L2；面板头显示「预热 n/m」，**可取消**；进度是原子量，主线程轮询重绘 |
| **邻接预取** | 表 / 视图文件夹首次展开后，后台预取**前 20 张表**的列；失败静默（预取是加成，不是前置条件） |
| **大 schema 兜底** | 类别文件夹首批 200 条 + 行尾「加载更多」 |
| **缓存清理** | 唯一入口：`⋯ → 缓存管理…`。断开 / 刷新 / 删除连接都**不删**缓存 |
...
| 文件 | 职责 |
| --- | --- |
| `nav_view.rs` | **NavView** 实体：面板头 / 分组树 / 连接行 / 对象树 / 右键 / 拖拽 / 行内编辑 |
| `nav_host.rs` | **NavHost** 端口（宿主实现）：连接清单与选中 · 项目根 · 提示与重绘 · 视图偏好 · 连接生命周期 · 需要窗口的对话框 |
| `nav_store.rs` | 标签 / 分组 / 排序 / 导航状态落库——只依 `engine` + 项目根，**不进端口** |
| `nav_jobs.rs` | 后台任务：加载 / 预热 / 预取 / 生成 SQL / 测试连接（单工作线程 + 结果队列 + 原子进度） |
| `navigator_service.rs` / `metadata_service.rs` | 层级与懒加载编排 / 内省调用 |
| `model.rs` / `cache.rs` | 节点模型与 L2 缓存读写 |
| `property_panel.rs` / `commands.rs` | 属性面板投影 / 面板键盘动作 |
...
| 维度 | 做法 |
| --- | --- |
| **契约测试兜底** | 结构尺寸必须等于设计值（常量单一来源 `workbench_shell::ui`）；视图源码**零裸色值、零裸 px**，下沉后仍登记在同一份扫描清单；共享状态字段白名单 |
| **纯函数优先** | 可测判定都拍成纯函数：搜索 facet 解析 / 类型徽标映射 / 限定名拼接 / 节点子键 / 归属域推导；导航模型与缓存往返均有单测（`model.rs` / `cache.rs`） |
...
| 对象树**懒加载与两级缓存**、刷新、预热、预取 | SQL 编辑器 / Mock / 洞察的**本体** → 各自模块（本面板只给右键入口） |
