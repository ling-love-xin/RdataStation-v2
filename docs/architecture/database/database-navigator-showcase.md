...
| 缺什么 | 影响 | 什么时候必须做 |
| --- | --- | --- |
| **搜索结果「在树中定位」** | 搜到了能在属性面板看，但不能一键展开并高亮到树上那一层 | 用户把搜索当主要入口时；键已统一（`ObjectRef::key()` 与树节点同构），底座已备 |
...
```
引擎侧单测    cargo test -p rds-engine --lib      → 440 passed / 0 failed（24 ignored）
导航侧单测    cargo test -p rds-database --lib    →  51 passed / 0 failed
