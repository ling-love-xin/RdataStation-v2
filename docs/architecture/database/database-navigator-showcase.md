...
| 没展开到的地方能搜吗 | **跨连接名称搜索**：搜索框扫每个连接自己的元数据索引（中缀匹配，敲 `ord` 命中 `order_items`），命中行落在列表最前（与树同一片滚动区，`↑↓` 可达），单击即看属性 |
...
**面板头常驻五个动作**：`＋` 新建数据源 · `⊞` 新建分组（文件夹 + 图标）· `⟳` 刷新选中连接 · `断开` · `⋯`
（刷新全部元数据 / 缓存管理… / 显示标签 / 显示归属域）。面板头高 36px（`PANEL_HEADER_HEIGHT`，与全 App 一致）。

**面板底状态行**：`N 已连接 · M 离线 · 元数据更新于 X`（无连接不显示；离线为 0 / 未取过元数据时对应段不写）。

**分组头**：色条 + 名称 + **健康度·计数（合并为一个元素）** + 一键全折叠；全连只报数（安静态），有未连写 `已连 c/n`，有失败追加 `· N 失败`（红）。

**树内类别靠形状**（表 / 视图 / 列 / 过程 / 序列 / 触发器 / schema / 文件夹各一个图标），**颜色只承载状态**（失败 `danger`，其余中性）——与连接行「颜色 = 状态」同一套语言。
...
| **跨连接搜索** | 搜索框 ≥ 2 字符 | 后台扫每个连接自己的索引（名称档中缀匹配；内容档走 FTS，≥ 3 字）；单连接 50 条 / 总量 300 条封顶；列表内最多渲染 100 条命中（超出由标题下的说明行交代） | 交互操作不该被超大 schema 拖死；**没有缓存的连接不建文件**（不会敲个字就落一堆空缓存） |
| **搜索范围诚实** | 有未拉取余量时 | 命中行显示「筛选仅覆盖已加载」；有命中时不会出现「没有匹配的数据源」（命中行在列表里，行集非空就轮不到空态） | 搜不到 ≠ 库里没有——差异必须说出口 |
...
| ~~**虚拟列表**~~ | ~~首屏只取 200 条已经够用，但「1 万条同屏」未验证~~ | ✅ **已落**（2026-09-20）：`v_virtual_list` 只画视口内的行，行集合由 `NavRow` 扁平静态给出（含搜索结果行）。余项：真机 10 万表库的滚动体感、S4 清单里的拖拽 / 右键 / 行内编辑未过 | 真实 10 万表库实测出现滚动卡顿时 |
...
| 文件 | 职责 |
| --- | --- |
| `nav_view.rs` | **NavView** 实体：面板头 / 分组树 / 连接行 / 对象树 / 右键 / 拖拽 / 行内编辑 / 搜索结果行（并入虚拟列表） |
