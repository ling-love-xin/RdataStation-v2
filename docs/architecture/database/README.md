...
| **归组 / 排序可拖** | 连接行拖到**分组头** = 归组（「未分组」头 = 移出全部）；拖到**连接行** = 插到该行之前；每次整容器写 `0..n`（不是相对插入） | 原型 §6.1、手册 §4.3、dev-plan §0（B3） |
...
| 待拍板 / 待做 | 分组本身的拖拽排序（`connection_groups.sort_order` 已就绪）；「未手动排序按名称」在存储侧落实现（§11#20） |
