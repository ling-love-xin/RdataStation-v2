...
| **归组 / 排序可拖** | 连接行拖到**分组头** = 归组（「未分组」头 = 移出全部）；拖到**连接行** = 插到该行之前；分组头拖到分组头 = **分组排序**；另给 `Alt+↑/↓`（连接）与右键上下移（分组）。每次整容器写 `0..n` | 原型 §6.1、手册 §4.3、dev-plan §0（B3） |
...
| **表 / 视图可直接拖进编辑器** | 拖拽载荷只带限定名 + 短名；落 **SQL 区**→插到光标处、落其它位置→追加；两处都聚焦 SQL 区 | 原型 §6.1、手册 §4.9、dev-plan §0（B7） |
...
8. `cargo` 命令固定 `-j 2`（并发链接重型 crate 会 OOM（DuckDB 已改动态链接））。
