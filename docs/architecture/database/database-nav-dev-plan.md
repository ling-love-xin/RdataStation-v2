...
原计划里的第四项（抽一个「行与键同序」的共用容器，替掉导航的 `rows` + `nav_order` 与草稿箱的二次 flatten）。
实施 A+B 后重估，**结论：不划算，不采纳**，理由写在这里以免重复提议：

1. **导航的漫游序列不是「行 + 键」**：`nav_order: Vec<NavOrderItem>` 带了过滤器（分组头不可漫游）
   与额外载荷（`conn_id` / `path` / `property` / `has_children`），通用容器盖不住，存两份反而多一处口径；
2. **导航的定位查询已经是一行**：`rows.iter().position(|r| r.key() == key)`（`nav_scroll_to_key`），
   容器能省的只是这行；
3. **草稿箱的二次 flatten 是借用生命周期问题**（`keys` 要在 `rows` 不借用时用），容器解决不了；
4. 净效果：新增一层抽象、删不到 40 行、却让两处都多一个跟面板无关的概念。

> 判断依据与当初拒绝「抽通用树组件」同源：**只抽自变量少、语义确定的东西**（A/B 两层），
> 「行集合怎么来」各面板差异是真实的。

### 并行写冲突记录（2026-09-20，已处置）

`crates/scratchpad/src/scratchpad_view.rs` 在同一时段被两个写入者改写，出现**字节错位交织**：
`ScratchpadEdit` 枚举区与搜索结果 `unique_files` 区文本互相嵌套，
`cargo check -p rds-scratchpad` 报 6 个解析错误，`rds-workbench` / `rds-app` 随之编译不过。
与 HEAD 做 token 多重集对比：**HEAD 独有 331 个 token 缺失**——说明内容确有丢失，不是纯格式化，
无法靠缝合恢复。

**处置（经确认执行）**：

1. 损坏内容先备一份取证副本：`.rds/tmp/scratchpad_view.rs.corrupted-20260920.bak`（4117 行，gitignored）；
2. `git checkout HEAD -- crates/scratchpad/src/scratchpad_view.rs` 回到干净版本（丢掉那 266/209 行未提交改动）；
3. 在干净文件上重做草稿箱侧的共用原语接入（4 处）：`tree::active_bar` · `tree::indent_spacer` ·
   `tree::disclosure_slot` + `disclosure_glyph`（顺带清掉两处 `chevron` 字符变量）·
   `scratchpad_row_height` 改返 `tree::RowHeight` + `tree::row_sizes`；
4. 验证：`rds-scratchpad` 37 测试全绿、`cargo check --workspace --all-targets` 无错、`ui_contract` 7/7。

**教训（写入本仓工作纪律）**：同一文件不要在两个会话里同时写——本仓的视图文件（nav_view / scratchpad_view
/ resource_view）都很长，两个写入者各自整文件重写就会交织出不可解析的文件；
若必须并行，请先 `git status` 看一眼该文件是否已被别人改过。
...
> V11 常量变更（2026-09-20）：新增 `ui::ICON_SIZE_XS`（0.75rem = 12px，图标阶梯第三档）；`NAV_ADD_TAG_SIZE`（16px）改名并改为 `NAV_ROW_ACTION_SIZE`（1.25rem = 20px，`+` / `✎` 共用）。三处已同步：`ui.rs` / `ui-design-spec.md` §2.1 / `ui_contract.rs`。
...
| C4 | 大 schema 分页 + **虚拟列表** | `navigator_service.rs` + `nav_view.rs`（视图已下沉到 `crates/database`） | ✅ **分页 + 虚拟滚动已接**（2026-09-20）：`get_object_chunk` 索引分块 + 「加载更多」行 + `v_virtual_list` 虚拟滚动（S1–S3 见 §2.5；S4/S5 待做） |
...
### 2.5 C4 实施规格与实施记录：导航树虚拟列表

**目标**：树不再靠「首屏 200 + 加载更多」限流，而是**扁平静态 + 虚拟滚动**；顺带解掉「滚到眼前」——
需要一个**可编程滚动入口**（导航树以前是自绘递归，没有）。

**状态：S1–S3 已落，S5 的滚动部分已接**（2026-09-20）：`843b5e1`（扁平静态 + 契约）· `2f51176`（改渲染）·
`scroll_to_item` 接线（定位 / 键盘漫游）。

#### 决策：用 `gpui-base::v_virtual_list`，**不用** `gpui-component::List`

原计划是 `List` + `NavTreeDelegate`（新文件 `nav_tree.rs`）。动手前核实两边的实际能力后改掉了：

| | `List`（gpui-component） | `v_virtual_list`（gpui-base，已采用） |
| --- | --- | --- |
| item 渲染期拿得到面板吗 | 拿不到（`render_item` 在面板被借用时跑，回头读面板会 panic）→ **行数据得拷一份进委托** | 拿得到（闭包签名 `Fn(&mut V, Range<usize>, &mut Window, &mut Context<V>)`） |
| 行高 | 按一个被测量的 item 定全表行高（文档明写「Every item should have same height」） | 按索引给高度（`Rc<Vec<Size<Pixels>>>`），支持不齐 |
| 鼠标事件 | 组件自己消费（拖拽是最大不确定项） | 不管事件（行还是行，现有 `on_drag` / `on_click` / 右键菜单全部原样） |
| 选中 | 组件维护选中索引，需镜像 + 防重入守卫 | 不维护（选中仍由 `selected_key` 一处管） |

代价：滚动 / 定位要自己写（`VirtualListScrollHandle::scroll_to_item`，列在 S5）；
行高要自己算准（`v_virtual_list` **不回写**测量结果，见下）。

#### 已就绪的三块（重构没从零开始）

1. **扁平静态**：`NavView::collect_nav_rows` 一次算出 `Vec<NavRow>`（六类行：分组头 / 连接 / 引用 /
   对象 / 「更多」/「已定位」，带 `key / conn_id / depth / 载荷`）。它是顺序**唯一权威**——
   `render_*` 不再递归、不再推顺序，键盘漫游序列（`nav_order`）是它的**投影**。
2. **可见性规则已集中**：`nav_node_matches`（本地过滤）· `child_total` vs 已加载（`pending_more`）·
   `folder_limit`（渲染窗口）· `jumped`（定位窗口）· `skip`（过滤时不得整支隐藏）。
3. **副作用跟着顺序走**：展开即排队加载、索引搜索排程都在 `collect_nav_rows` 里——
   留在旧渲染路径上，切换后就会静默失效。
...
| # | 做什么 | 状态 | 验收 |
| --- | --- | --- | --- |
| S1 | `NavRow` 六类行 + `collect_nav_rows`（复刻原递归遍历的顺序与副作用）+ 抽 `NavConnFilter`（树与索引搜索共用一份连接级判据） | ✅ `843b5e1` | 8 阶段等价验收：收集序与渲染序逐步相等（分组 / 引用行 / 分页 / 定位窗口 / 过滤 / 无匹配） |
...
| S4 | 拖拽（对象→编辑器、连接行→分组）· 右键菜单 · 行内编辑器——**逐个真机确认**（测试盖不到鼠标；仓库已记载 `simulate_click` 全套跑不可靠，所以不写点击模拟） | ⏳ 待做 | **逐条清单见 §2.6**（18 条，记 ✅/❌） |
...
`v_virtual_list` 按给定高度**定义**每行槽位（`AvailableSpace::Definite`），**不会**反过来测量并写回：
估小 → 下一行压上来，估大 → 多留一段空隙。所以：

- 每种行高在 `crates/workbench_shell/src/ui.rs` 有常量（`NAV_ROW_GROUP` / `NAV_ROW_CONNECTION` /
  `NAV_ROW_TREE`），渲染里的 `h(rems(…))` 与 `nav_row_height` **成对维护**；
...
- **行高不齐** → 已按上节钉成常量；新增行类时必须同时加常量与分支。
- **回归面大** → 已补：可见行顺序 · 定位链路（3 条）· 行被真的画出来（本切片）；`ui_contract` 两份清单**无需**改动
  （没有新建视图文件，渲染仍全在 `nav_view.rs`，已在扫描清单里）。
...
### 2.6 S4 真机验收清单（逐条过，别用“应该没事”结案）

为何不能自动化：仓库已有两条明确记载（`analytics_resource/tests/dialog_window.rs`、
`editor/src/view/tests.rs`）——`simulate_click` **单跑能中、全套跑就中不了**；拖拽更没有可靠的模拟方式。
所以 S4 靠人过一遍，每条记下结果（✅ / ❌ + 现象）。

**准备**：一个带多 schema + 大表文件夹（≥ 500 张表）的库；一个已建好的分组；`⋯ → 显示标签 / 显示归属域` 打开。

| # | 操作 | 期望 | 结果 |
| --- | --- | --- | --- |
| 1 | 单击连接行 | 选中高亮；再单击展开 / 折叠 | |
| 2 | 点行首三角、按 `→` / `←` | 与 1 同一行为（键盘与鼠标不各管一套） | |
| 3 | 双击表 / 列 / 视图行 | 右侧属性面板打开且字段是该对象 | |
| 4 | 右键连接行 | 菜单：连接/断开 · 编辑 · 复制名称 · 刷新元数据 · 在 SQL 编辑器中打开 · 查看洞察 | |
| 5 | 右键分组头 / 对象行 | 分组：重命名 / 新建 / 删除 / 上移 / 下移；对象：查看数据 / 生成 SQL / 复制限定名 / 结构洞察 / 生成 Mock / 查看统计 | |
| 6 | 拖表 / 视图行 → SQL 编辑器 | 插入**限定名**，幽灵显示短名 | |
| 7 | 拖连接行 → 另一个分组头 | 归组（该连接在两个组里出现；非主组是引用行 `∈ 主组名`） | |
| 8 | 拖连接行 → 组内某行之前 | 组内排序；拖分组头 → 分组头：分组排序 | |
| 9 | 行尾 `+` 打标签 | 行内标签编辑器出现（行**变高**而不是压住下一行），回车保存、重开仍在；**未 hover 时行尾不应该能点到任何东西**（v8：`+`/`✎` 用 `invisible()` 隐藏） | |
| 10 | 右键「移动到分组…」 | 归组编辑器出现，勾选即时生效 | |
| 11 | 右键「复制连接（模板）…」 | 复制编辑器出现，回车提交、取消可关 | |
| 12 | 大表文件夹点「加载更多」 | 拉下一页，余量数字变小，新行接在末尾；**筛选态**下标签含「筛选仅覆盖已加载」 | |
| 13 | 定位到一个屏外的大表（搜索结果行「定位」/ Quick Open `⌥↵`） | 链路逐层展开、选中目标、**列表自动滚到目标（居中）**；窗顶有「已定位第 N 条（本窗 M）· 回开头」 | |
| 14 | 点窗顶「回开头」 | 回到首页量行，余量数字恢复 | |
| 15 | 键盘 `↓` 一直往下 | 选中一路往下，**出屏即自动滚**；能走到引用行 / 「更多」行 | |
| 16 | 滚轮 / 滚动条拖拽 | 正常滚动；行与行不重叠（钉高估准了）、不因主题字号变化而错位 | |
| 17 | 切明暗主题 | 行色 / 选中底 / 分组头色正常；**树内类别靠形状辨认**（明暗两套下都不依赖颜色） | |
| 18 | 无连接项目启动 | 空态标题 + 说明 + 「新建连接」可点 | |
| 19 | 项目里有连接，但在搜索框里输入一个不存在的词 | 空态给「没有匹配的数据源」+ **当前筛选清单** + 「清除筛选」；点一下搜索框与 facet 一起清（§V12） | |

> 真发现问题的重点怀疑对象：行高估算（与渲染成对维护，见 §2.5 硬约束）、
> 拖拽落点（`on_drop` 仍在行上，未被列表接管）、行内编辑器块高（内部滚动而不是被截断）。
...
| 大 schema（10 万+ 表）渲染卡顿 | 已落：`get_object_chunk` 分页 + `v_virtual_list` 虚拟滚动（只画视口内的行，见 §2.5）；列内联仅在 ≤50 列时展开 |
