...
**状态：S1–S3 已落**（2026-09-20）：`843b5e1`（扁平静态 + 契约）· `2f51176`（改渲染）。
...
| S5 | `scroll_to_item` 落地（定位收尾滚到目标）· `page_limit` 退场 · `load_more` 替「加载更多」行 | ⏳ 待做 | 定位→目标在可视区（新窗口测试） |
...
**收益**：① 十万行同屏不再靠分页限流；② 得回可编程滚动（S5 用）；③ 顺序与副作用单源，
过滤 / 分组 / 定位的判据只有一份。
