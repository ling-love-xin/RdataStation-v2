...
| 18 | 🟡 | 拖拽与手动排序 | 表 / 视图 → 编辑器插入限定名**已做**（2026-09-16：`on_drag` + SQL 区/编辑区双落点，API 见 `panels.rs::NavDragPayload`）；**归组拖拽与组内外手动排序仍未做**（`ConnectionOrgStore::set_member_order` 已就绪，缺 UI 入口）。 |
