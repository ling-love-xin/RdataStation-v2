...
- 大 schema 客户端分页：每批 `ui::NAV_FOLDER_PAGE_SIZE`（200），行尾「加载更多」。
...
- **分页**：类别文件夹首批 200 条，超出显示「加载更多」（`ui::NAV_FOLDER_PAGE_SIZE`）。
...
| 18 | ✅ | 拖拽与手动排序 | 已做（2026-09-16）：① 表 / 视图 → 编辑器插入限定名（SQL 区插光标处 / 编辑区兜底追加）；② 连接行 → 分组头归组（含「未分组」= 移出全部）；③ 连接行 → 连接行= 组内插到该行之前；④ 落库每次**整容器写 `0..n`**（`set_member_order_all`）。**仍未做**：分组本身（`connection_groups`）的拖拽排序。 |
| 19 | ✅ | 新建分组无描述表单 | 已做（2026-09-16）：`components/group_form_dialog.rs` 新建 / 编辑共用（名称 + 描述）；新建预填自动去重默认名；名称必填 + 内联提示；入口面板头 `🗂＋` / 分组右键（新建 / 编辑分组…）/ 组内联编辑器。 |
| 20 | 🟡 | 缓存首版限制（C3–C6） | 命中判据「非空即有数据」（空列表会回落实时内省）；刷新 `prune_schema` 连带失效兄弟文件夹缓存；列级删除不剪枝；每次 `NavCache::open` 新建连接 + 跑幂等迁移。**另一项**：「未手动排序的按名称升序」只在**渲染侧**生效，存储侧 `list_group_members` 仍按 `sort_order, connection_id` 返回（`sort_order` 无“未设置”哨兵，两态不可分）。详见 `dev-plan` C3 说明。 |
