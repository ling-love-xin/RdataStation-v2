...
| 17 | 🟡 | 导航侧菜单缺口 | ✅ 已补（2026-09-14）：**测试连接**（导航侧独立会话探测，与对话框同源、不注册池不写库）；表 / 视图 **生成 SQL ▸**（INSERT / UPDATE / DELETE 模板，列走后台任务 + L2，无主键退化为 `WHERE 1 = 0`）。⬜ 仍缺：复制连接（模板）· 共享至项目 / 取消共享 · 删除连接（删除目前只在编辑区）。 |
