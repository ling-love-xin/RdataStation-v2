...
> 状态：**方案 A 已实现（v5）；v6/v7 降密与徽标语义已实现（V2–V10，2026-09-13）；v8/v9/v11 视觉通道、面板框与结果区并入列表已实现（V11–V12、V15，2026-09-20）** · 关联文件：`database-navigator-prototype.html`（可交互原型）、`database-nav-dev-plan.md`（开发方案）
...
### v8 修订点（视觉通道与行态统一，2026-09-20）

| # | 问题 | 修订 |
| --- | --- |
| 1 | 树内节点类别用 **8px 色块**表达，与 v7「颜色 = 状态」同屏打架（同一个 `info` 在连接行是「连接中」、在表行是「这是表」） | 类别改**形状**：按 §3 图标角色表逐项落资产路径（表 `table` / 视图 `eye` / 列 `columns-3` / 例程 `square-function` / 序列 `list-ordered` / 触发器 `zap` / Schema `folder-tree` / 类别文件夹 `folder`(开)/`folder-open`(合)）；**颜色只留给状态**（失败 `danger`，其余中性 `muted`） |
| 2 | 选中态不一致：悬停底色**覆盖**选中底色；树行没有左侧 2px 激活条（`TREE_ACTIVE_BAR` 全仓其它树都有）；「加载更多 / 已定位 / 引用」进键盘漫游但**无任何选中反馈** | 全树统一：悬停只在**未选中**时生效；选中行 = `list_active` 底 + 左侧 2px `list_active_border` 条（内缩 `TREE_ACTIVE_BAR_INSET`）；行态判据收敛到一个 `nav_row_selected` |
| 3 | 行尾 `+` / `✎` 用 `opacity(0)` 隐藏：**看不见但仍能点到**（hover 槽位宽也常驻）；`+` 16×16、`✎` ≈20×15 命中区偏小 | 改用 `invisible()`（不绘制、不登记 hitbox）+ `group_hover` 显；两个按钮统一 20×20（`NAV_ROW_ACTION_SIZE`，旧名 `NAV_ADD_TAG_SIZE`） |
| 4 | 命中区过小：分组头「全折叠」10×10（且 `⇅` 字形漏写 `text_xs`）、搜索命中「定位」≈24×15 | 全折叠 24×24（行高上限）+ `chevrons-up-down` 图标；「定位」抬到 20px 高并有 hover 底 |
| 5 | 虚拟列表**钉死行高**，但树行名称 / 引用行 `∈ 主组名` / 列元信息 / 「加载更多」/「已定位」/ 搜索命中元信息**均未截断** → 长文案换行撺破定高槽位 | 逐处补 `min_w_0` + `overflow_hidden` + `text_ellipsis`（或 `flex_none`）；「已定位」文案压短为「已定位第 N 条（本窗 M）· 回开头」 |
| 6 | 图标语言混用：`🗂＋`（全库唯一 emoji）、`▸/▾`、`✎`、`⇅`、`✓`、文本 `+` 与 `IconName` 资产共震 | 统一走资产路径（`folder-plus` / `chevron-*` / `pencil` / `chevrons-up-down` / `check` / `plus`）；`⋯` 菜单的选中态改用 `PopupMenuItem::checked`（不再拼 `✓ ` 前缀） |
| 7 | 「更多」行点击后外观不变、再点静默无反应 | 取数中改文案「加载中…」并转中性色 |
| 8 | 图标字号无 12px 档常量，行内小图标各写 `rems(0.75)` | 登记 `ui::ICON_SIZE_XS`（0.75rem = 12px，图标阶梯的第三档） |

> 未改（已知遗留，详见 `database-navigator-architecture.md` §11 #25）：选中 / 滚动 / 搜索词未持久化（§6.4）。（面板头、底部状态行、分组头计数、筛空空态已在 **v9 修订点**补上；结果区并入列表见 **v11 修订点**。）

### v9 修订点（密度、面板框与空态，2026-09-20）

| # | 问题 | 修订 |
| --- | --- | --- |
| 1 | 面板头 30px（裸 `rems(1.875)`）是全仓唯一一档：编辑器工具条 / 设置页 / 资产库面板头都是 36px | 改取 `ui::PANEL_HEADER_HEIGHT`（2.25rem = 36px），与 App 其余面板对齐（§2 布局图） |
| 2 | §2 布局图里的**底部状态行从未实现**（「N 已连接 · M 离线 · 缓存 X」三个事实无处承载；状态栏只显示当前连接与 DuckDB） | 面板底补 24px 状态行（`text_xs` muted，不占行级密度预算）：`N 已连接 · M 离线 · 元数据更新于 X`。三条口径：无连接不显 / 离线为 0 不写 / 未取过元数据不写后段。时间取**最近一次加载成功回填**（不读缓存文件 mtime：render 期禁止 I/O） |
| 3 | 分组头把数量显示两次：`1/2 · 2`（分母就是总数，再跟一个 `2` 读起来像重复） | 两个元素**合并为一个**，三档：全连 → 只报数（安静态）；有未连 → `已连 c/n`；有失败 → `已连 c/n · N 失败`（整段 `danger`） |
| 4 | 被筛空时只有一句「没有匹配的数据源。」：不说**谁**把它筛空的，也没有恢复入口（chips 与搜索框是两个独立入口） | 空态升级为「标题 + 当前筛选清单 + 「清除筛选」按钮」：清单按面板自上而下列举（归属域 → 类型 / 驱动 / 标签 → 搜索词原样带引号）；按钮**同时**清搜索框与 facet（只清一边，点完还是空） |

> 状态行与行徽标共用一条「已连接」判据：本会话建过连（`nav.connected`）或连接记录本身标为可用（`ConnectionItem::connected`）。

### v11 修订点（搜索结果并入列表，2026-09-20）

| # | 问题 | 修订 |
| --- | --- | --- |
| 1 | 结果区是虚拟列表**之外**一个自带 128px 上限的滚动块：① 命中行 `↑↓` 走不到（键盘漫游序列从行集合投影，它不在行集合里）；② 没有选中反馈（点过的行下一帧仍是普通行）；③ `NAV_SEARCH_MAX_ROWS` = 100 条与 128px 可视高度不匹配——一条 22px，100 条要在这个小窗里**二段滚动**；④ 树空、命中有时，下面还要多一块空态文字 | 命中行改成一个行类型（`NavRow::SearchHit`）**并入虚拟列表**：排在最前（与旧「结果区在树顶」同观感），与其他行共用同一片滚动区与同一套行态（悬停不盖选中、选中 = `list_active` 底 + 左侧 2px 激活条）、进 `↑↓` 漫游序列，`Enter` = 打开属性面板（与单击同一件事）；`NAV_SEARCH_SECTION_MAX` 与那个二段滚动窗一起退场 |
| 2 | 结果区标题行（“搜索中 / 无命中 / 共 N 条 / 定位失败原因”）如果也搬进列表，会随树滚走 | 标题**留在列表之外**（`render_search_header`）：不滚动、也没有高度上限，只有标题一行与偶发的说明行（「仅显示前 N 条」/ 定位失败原因） |
| 3 | 命中行的身份只用 `ObjectRef::key`：它**不带类别**，同 schema 下例程与表可以同名 → 元素 id 同层重复（串状态）、业务键重复（`↑↓` 在原地来回弹） | key / 元素 id 一律带上**位次**：`search:{位次}:{引用}` 与 `nav-search-{位次}-{引用}`（`search_hit_row_key` / `search_hit_row_id` 单一来源）；`ObjectRef::key` 的口径挪到 `SearchHit::key`（`nav_jobs.rs`），行、元素 id、Quick Open 三处共用一份 |
| 4 | 已定位成功时不再需要那块独立空态判断（“有命中就别说没有匹配”） | 空态去掉 `search_hits.is_empty()` 这道显式挡板：命中已在列表里（有命中则行集合非空），矛盾那一条自然不成立 |

> 上限语义变更：`NAV_SEARCH_MAX_ROWS`（100）现在是「最多**渲染**多少条命中」（DOM 元素数护栏），不再是「一个 128px 小窗里塞多少条」；超出仍由标题下的说明行交代。
...
| 4 | 行内 `+` 用于添加标签 | 行尾 `+` = **标签快捷入口**（展开行内标签编辑，**只做标签**）；**归组走右键「移动到分组…」**，不新增第二个行内按钮（§6.1） |
| 5 | 标签显示 / 隐藏在「更多」处设置 | `⋯` 增「**显示标签**」开关（持久化 `settings.json`）；开启后在**名称下一行**以「最多 2 个小 chip + `+N`」呈现（§2.2） |
...
| 3 | 行内 🗂 与右键归组重复 | **移除行内 🗂**；行尾 `+` **只做标签**，归组统一走右键「移动到分组…」+ 键盘（§6.1 / §6.2） |
...
左 Dock `LeftPanel::Database` 面板：顶部**归属域 facet（全部 / 项目 / 全局 / 共享）+ 附加筛选（类型 / 驱动 / 标签）+ 搜索**，下面是**数据源管理 + 对象树**；数据源按**自定义分组**一级组织，可属于多个分组（主组全亮 + 其它组引用样式），标签只做筛选、不进树；连接行常驻「**徽标（颜色 = 状态 / 形状 = 类型）+ 名称 + 归属域短码（右对齐列）**」，行尾 hover 出 `+`（加标签）与 `✎`（编辑），**标签显示在名称下一行**且默认不显示（可在 `⋯` 开启）；展开连接时若未建连会**先自动建连**；续展开对象树（schema → 表 / 视图 / 存储过程 → 列）；双击对象打开**靠右停靠的 DBeaver 式属性面板**（事实的唯一权威展示位）。**本模块只管理数据源，不涉及分析资产。**
...
│ 数据源              [＋][⊞][⟳][⚡][⋯]       │  ← 面板头 36px（`PANEL_HEADER_HEIGHT`）
├ 筛选（常驻）─────────────────────────────────┤
│ [🔍 名称 / 标签 / type: / scope: …] [筛选▾ 2]│
│ [全部]  项目   全局   共享                    │  ← 归属域 facet（常驻唯一 facet）
├ 树主体（滚动、虚拟化 >50）───────────────────┤
│ ▾ 核心库                 已连 1/2       ⇅     │  ← 分组头：色条+名+健康度·计数（合并）+全折叠
│     ▾ ◈ 生产 PG               GP    +      │  ← 连接：徽标（色=状态·形=类型）+ 名称 + 域码列
│       ▾ 📁 analytics                        │
│         ▾ ▦ 表                     23       │
│           ▾ orders              1.2M        │  ← 选中（左侧 2px 激活条）
│               order_id     PK  bigint       │
│               customer_id      bigint · FK  │
│         ▸ 👁 视图 / ƒ 过程 / ≡ 序列          │
│     ▸ ◈ 本地 MySQL            P             │
│ ▾ 报表               已连 0/1 · 1 失败        │  ← 有失败：整段转 danger
│     ▸ ◈ 报表 PG               G             │
│ ▾ 未分组                     1               │  ← 全连：只报数（安静态）
│     ▸ ◇ 测试 SQLite           G             │
├ 底部状态（`text_xs` muted）──────────────────┤
│ 3 已连接 · 1 离线 · 元数据更新于 12 分钟前    │
└──────────────────────────────────────────────┘
```

> 图中 `◈` = 徽标（彩色 → 已连接；形状随类型）；`◇` = 同形状但灰色（未连接）。`+` 仅 hover / 选中时显现。
> 树内**类别靠形状**（`▦` 表 / `👁` 视图 / `ƒ` 过程 / `≡` 序列 …），颜色只用于状态（失败 `danger`，其余中性）。
> 底部状态行三条口径：无连接时不显示；离线为 0 时不写；没取过元数据时不写「更新于」那一段。
...
- **行尾 `+` = 标签快捷入口**（hover / 选中显），点击展开行内标签编辑；**归组仍走右键**（§6.1）。行操作仅 `+` 与 `✎`；**连接 / 断开只走右键菜单**（不占行内）。
- **标签默认不显示**：显示在**名称下一行**（独立行，不挤占名称 / 归属域列）；`⋯ → 显示标签` 开启后以「最多 2 个小 chip + `+N`」呈现（§2.2）。
...
> 实现状态（v6/v7，2026-09-13）：**已实现** —— 行内瘦身 + 双通道徽标（含 hover 卡）、归属域右对齐固定列 + `⋯` 开关、标签行内可选显示（`⋯ → 显示标签`）+ 行尾 `+`、分组头健康度 / 全折叠、行操作悬停显示、多组引用样式 + 显式主组（`connection_group_members.is_primary`，右键 `设为主组 ▸`）、`筛选 ▾` facet 弹层 + 搜索 facet 语法（V7）。
> 实现状态（v8，2026-09-20）：面板头第二个按钮的 `🗂＋` 字形换成 `folder-plus` 图标（emoji 字宽随字体变，与同排 `IconName` 按钮对不齐）；树内类别图标按 §3 形状化；选中 / 悬停 / 截断 / 命中区统一（见 v8 修订点）。
> 实现状态（v10 打磨，2026-09-20）：**归属域短码 tooltip 已补**（§2.3 表里那三句人话，300ms 延迟，与徽标 hover 卡同一套；`nav_scope_hover_card` / `nav_scope_tooltip`）；**搜索框前导放大镜已补**（`Input::prefix`，与连接对话框的驱动筛选同法）；**「连接中」徽标脉冲已补**（§2.3：`Animation::new(1.5s).repeat()` + `bounce(ease_in_out)`，只改透明度且只挂在 `Connecting` 状态上，见 §2.3 的动效口径）。
...
- 视图状态：**展开态按连接**持久化（`navigator_state.expanded_keys`）；**选中 / 搜索词**是面板级（同一张表的保留行，见 §6.4）；**滚动由选中锚点恢复**；归属域筛选 + 附加 facet 进 `settings.json`（UI 偏好）；分组折叠态是内存态（不落库）。
...
- 归组：**右键「移动到分组…」**（唯一入口，v6 移除行内 🗂）+ 组内外手动排序；支持多选组、新建分组。
- 标签：**行尾 `+`** 编辑（唯一入口）；多值；仅供筛选 / 搜索（`tag:prod`）。
- **标签显示（v7 修订）**：默认**不显示**；`⋯ → 显示标签` 开启后，在**名称下一行**（独立行）以「**最多 2 个小 chip + `+N`**」呈现。字号取**最初版小字**（`text_xs`，与行内文字同尺寸），不挤占名称行，也不影响归属域列对齐。
...
> **动效口径（2026-09-20 落定，首个动效）**：「连接中」的脉冲 = `Animation::new(ui::NAV_BADGE_PULSE_MS).repeat()`
> + `with_easing(bounce(ease_in_out))` + `|badge, delta| badge.opacity(1.0 - delta * 0.5)`：
> 只改透明度（`div` 不能缩放），缓动**首尾同值**（循环接缝不跳，且 `reduce_motion` 停帧时落在 1.0 = 外观正常）。
> **只给这一种暂态挂**——循环动画会让窗口每帧重绘，而稳态（已连 / 未连 / 失败）另有静态表达（颜色 + hover 卡），
> 不靠动画说明状态（上游动效规范：动效解释变化，不做常驻装饰）。判定抽成纯函数 `nav_badge_pulses`（带单测），
> 免得后来把动画顺手挂到稳态上。全量口径见 skill `gpui-kit-dev` §动效。
...
| 单击箭头 | 展开 / 折叠（懒加载；连接根未建连时**先自动建连**） |
| **双击对象** | **右侧属性面板**（DBeaver，§7） |
| **双击连接** | 连接 / 断开切换 |
| 悬停 / 选中连接行 | 行尾浮出 **`+`（加标签）** 与行操作（断开、刷新、⋯）；常驻零 affordance |
| **拖拽表 / 视图到编辑器** | 插入**限定名**（`db.table` / `schema.table`，同段去重）：落 **SQL 区**→插到光标处；落编辑区其它位置→追加到草稿末尾 |
| **拖拽连接行到分组头** | 归组：拖到自定义分组头 = 加入该组（**保留**其它归属，多对多）；拖到「未分组」头 = **移出全部分组** |
| **拖拽连接行到连接行** | 组内手动排序：插到目标行**之前**（跨组拖 = 先归组再定位） |
| **拖拽分组头到分组头** | 分组排序：插到目标分组之前；拖到「未分组」头 = 排到**最后**（未分组永远在末位） |
| `Alt+↑` / `Alt+↓` | 选中**连接**在所属容器内上移 / 下移一位（改顺序不改光标）；对象节点不适用 |
| 右键 | 上下文菜单（6.2）——**归组入口之一**（拖拽已可归组） |

> **v9 拖拽落点语义**（实现）
> - 只有**表 / 视图**行可拖（其余节点无载荷）；拖拽幽灵显示**短名**，插入的是**限定名**；
> - 载荷只带「限定名 + 短名」两个字符串，**不带连接句柄与执行计划**——拖拽不承诺语义，由落点决定动作；
> - **SQL 区**（`use_duckdb_fed` 连接才可见）有独立落点，拖入时高亮描边；插到光标处需该区**已聚焦**（未聚焦时光标停在 0，插入会把表名顶到语句前面，故退化为追加）；
> - 编辑区其它位置是**兜底落点**（拖到面板任意处都不丢），一律追加；
> - 两种落点都会**聚焦 SQL 区**；SQL 区不可见时额外给一条面板通知，说明名字已进草稿（解除 `use_duckdb_fed` 限制后此提示自然消失，见架构 §11#16）。
> - 插入文本带尾随空格，连续拖多个表不会粘成一个标识符。

> **v6/v7**：移除行内 🗂 归组按钮（与右键菜单重复）；行尾 hover 只显 **`+`（加标签）与 `✎`（编辑）**，**连接 / 断开只走右键**；标签显在**名称下一行**。行操作全部可由键盘 / 右键到达，悬停显隐只影响“可见性”，不影响可达性（C7 键盘导航已覆盖）。

### 6.2 右键菜单

**连接节点**：连接 / 断开 · ✅ **测试连接**（导航侧：独立会话探测，结果落面板提示） · 编辑连接… · 查看属性 · 移动到分组… · 设为主组 ▸ · ✅ **复制连接（模板）…**（行内输入新名，**不带密码**） · ✅ **共享至项目**（仅 `G_`） / ✅ **取消共享**（仅 `GP_`） · ✅ **删除连接**（二次确认） · 复制名称 · 刷新元数据（+ 通用模块入口）。标签编辑走行尾 `+`；归组还能直接**拖到分组头**（§6.1）。

**通用模块入口**（所有节点都有：连接 / Catalog / Schema / 文件夹 / 表 / 视图 / 列 / 例程…；位于菜单**底部、分隔线之后**）：**在 SQL 编辑器中打开**（选中该节点所属连接 + 清空导航 / 结果残留 + 聚焦中央编辑区）· **查看洞察**（右 Dock 展开到 `Insight`）。两者经 `SidebarEvent::{OpenSqlEditor, OpenRightPanel}` 由宿主处理，面板不直接改布局状态。

**表 / 视图专属**：另有 **生成 Mock 数据**（右 Dock 展开到 `Mock`），夹在通用项之间；其余节点不出现该项。

**表 / 视图**：查看数据（`SELECT * … LIMIT 200` 注入编辑区）· ✅ **生成 SQL ▸**（INSERT / UPDATE / DELETE 模板；列由后台任务取、命中 L2 不发查询；**只注入不执行**）· 查看属性 · 复制名称 / 限定名 · 刷新元数据（+ 通用模块入口 / Mock）。

**分组节点**：新建分组（**表单**：名称 + 描述） · 重命名分组（行内） · **编辑分组…**（表单：名称 + 描述） · **上移分组 / 下移分组**（边界置灰） · 删除分组（**不删成员连接与缓存**） · 在此新建连接 · 折叠 · **折叠其他**（v6）。「未分组」只给「新建分组」。

**列 / 索引 / 约束 / 例程**：查看属性 · 复制名（+ 通用模块入口）。
...
- ~~FTS 全量搜索（≥2 字符）：`MetadataCacheOps::search_fts`，snippet 高亮；结果落**中央编辑区**专用面板。~~
  **已按实现更新**（2026-09-19）：名称档（`search_index`，≥ 2 字）落**导航面板列表最前**（命中行 = `NavRow::SearchHit`，
  2026-09-20 并入虚拟列表，见 v11 修订点）；
  内容档（`search_fts`，trigram，≥ 3 字）在 **Quick Open 的 `#` 档**（`workbench/src/quick_open/`），
  两档都不落编辑区，命中行单击开属性面板。
- `↑↓` 选择、`Enter` 打开（✅ 已实现：`↑↓` 走 `nav_order`，命中行自 v11 起就在序列里；`Enter` 与行单击同一件事 = 开属性面板）；300ms 防抖、上限 500（防抖在 Quick Open 侧，导航面板目前不防抖：`nav_search_query_ready` ≥ 2 字即排一次索引搜索）。
  **未实现**：`Esc` 清空搜索框（`nav_view` 里目前没有任何 `Escape` 绑定）。
...
- 写入防抖 **600ms**（`ui::NAV_STATE_SAVE_DEBOUNCE_MS`）；状态与缓存一样**不随断开/删除清除**。

**实现订正（2026-09-20，V18）——一张表里的两类行**：

v5 把「来源标签页」合并成一棵分组树后，**选中与搜索框变成了面板级唯一的东西**（一个搜索框管所有连接、同一时刻只有一个选中行），按连接各存一份会互相覆盖、也说不清该听谁的。于是：

| 状态 | 落点 | 字段 |
| --- | --- | --- |
| 展开态 | 按连接一行（`conn_id` = 真实连接 id） | `expanded_keys` |
| 选中 / 搜索词 | **面板级保留行**（`conn_id` = `PANEL_STATE_CONN_ID` = `__panel__`） | `selected_key` / `filter_text` |
| 滚动 | **不存偏移**：由选中锚点恢复（加载后把选中的那一行滚进视口，行没到位时由 `nav_pending_scroll` 跨帧兑现） | — |
| 归属域 / 类型 / 驱动 / 标签（facet） | `settings.json`（UI 偏好，跨项目；已如此） | `navigator.filters` |

- 为何滚动不存像素偏移：行集合是**按需懒加载**的（展开态恢复后子节点还在陆续回来），偏移在恢复时对不上；而「我选中的那条」既是稳定的锚点，也正是用户记忆里的位置。
- 面板级状态是**项目隔离**的结构化状态：有项目根落项目库；**没项目根就没有落点**（读回 `None`、写为 no-op）——生产上应用启动即绑定当前项目（§2.1）。
- **哪些选中不记**：搜索命中行（`search:{位次}:…`）、分组头（`group:`）、引用行（`ref:`）、「更多」/「已定位」（`#more`/`#jump`）都是**视图临时行**（`nav_selection_is_persistable` 是否决判据，带单测）；记住它们只会让下次启动的选中落到一行看上去无关甚至不存在的行上。
- 展开态写库**防抖**（旧实现每次点击写一次，并在写展开态时用 `..Default::default()` 把其它字段抹掉——V18 一并修正）。
...
| 23 | **行内 `+`（v7）** | `+` = **标签快捷入口**（只做标签）；**归组走右键「移动到分组…」**（不新增第二个行内按钮） | v7 |
| 24 | **标签显示（v7）** | 默认**关**；`⋯ → 显示标签` 开启后在**名称下一行**（最初版小字 `text_xs`）以「**≤2 chip + `+N`**」呈现 | v7 |
...
| 32 | **拖拽落点语义（v9）** | 表 / 视图行可拖；载荷 = 限定名 + 短名；**SQL 区（已聚焦）→ 光标处**，**编辑区其它位置 → 追加**；两处都聚焦 SQL 区；SQL 区不可见时给通知（§11#16 未解前） | v9 |
| 33 | **归组 / 排序拖拽（v9）** | 连接行可拖：落**分组头** = 归组（自定义组为加入、未分组为**移出全部**）；落**连接行** = 插到该行之前。跨组拖 = 先归组再定位。已在组内且只落到组头 → **不改位置**；拖到自己身上 = 不变 | v9 |
| 34 | **容器顺序落库（v9）** | 每次写**整个容器的 `0..n`**（不是相对插入），避免序号空洞；「未分组」单独一张 `navigator_ungrouped_order`（它不是真实分组） | v9 |
| 35 | **分组表单（v9）** | 新建 / 编辑共用一张表单（名称 + 描述）：新建预填**自动去重默认名**；名称必填（空名不提交 + 内联提示）；描述空白归一化为 `None` | v9 |
| 36 | **分组排序（v9）** | 拖分组头到分组头 = 插到它之前；拖到「未分组」头 = 排到最后（未分组永远末位）；另给右键「上移 / 下移分组」（边界置灰）。重排只改 `sort_order`，不动名称 / 描述 | v9 |
| 37 | **连接重排快捷键（v9）** | `Alt+↑/↓` 移动**条目**（区分于 `↑/↓` 移动**光标**）；只对连接行生效（对象顺序由内省给出）；边界为 no-op | v9 |
...
| 4 | 「未分组」头何时渲染 | 未分组非空时**或**已有自定义分组时（空头 = 拖拽移出分组的常驻落点） |
