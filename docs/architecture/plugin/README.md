...
| `sidecar/driver.rs` | ✅ **P1 驱动桥已落地**：`SessionDriver`（会话之上的调用：`driver.describe` / `query.execute` / `query.fetch` / `query.cancel` / `session.ping`）+ `QueryPage` / `PageData`（内联 JSON 或 **Arrow 批**，承载方式对消费方不可见）+ `DriverError`（按错误码分流：`is_cancelled` / `is_capability_denied` / `is_sql_error`）；Arrow 附件在宿主侧解成 `RecordBatch`（可直接喂 DuckDB）；4 条单测 |
