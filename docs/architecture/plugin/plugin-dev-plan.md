...
| 传输与帧 | `crates/plugin/src/sidecar/proto.rs`（✅ 帧/增量解码/async 流读写/版本闸/阈值/错误码）+ `crates/plugin/src/sidecar/conn.rs`（✅ 异步客户端：drive 任务 + 在飞表 + 事件通道 + 超时放弃）+ `crates/plugin/src/sidecar/process.rs`（✅ 三管道接入连接）；`client.rs` 与旧 `sidecar/manager.rs`（HTTP/端口 + 读端口号）是**仅剩的旧路径**，P1 收尾时删（`Registry → Action` 的执行器已落地，见下一行） |
