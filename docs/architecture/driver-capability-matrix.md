...
| 16 | 🟡 | **`list_constraints` 六个驱动一个都没实现**（**2026-09-21 真机发现**）：每个驱动的 `MetadataBrowser::get_constraints` 都写成 `self.list_constraints(...)` 纯转发，而 `list_constraints` 落到 trait **默认空实现**（`traits.rs` 返回 `Ok(vec![])`） | **属性面板的「约束」分区对所有库恒为空白**——主键 / 外键 / 唯一 / CHECK 全不显示；合成 DDL 也拿不到它们。与已修的 §7 #10（序列 / 触发器被空实现遮蔽）**是同一形态**，只是那一次只排查了序列与触发器 | 待办：按库写真实实现——PG `pg_constraint`（`contype` + `conkey` + `confupdtype/confdeltype`）、MySQL `information_schema.TABLE_CONSTRAINTS` + `KEY_COLUMN_USAGE` + `REFERENTIAL_CONSTRAINTS`、SQLite `PRAGMA foreign_key_list` + `pragma_table_info`、DuckDB `duckdb_constraints()`；两个 native 驱动各自再一份。**当下已绕过**：合成 DDL 的主键改走**列上的 `is_primary_key`**（`get_table_detail` 路径是活的，PG 用 `information_schema.table_constraints` 现算）——见 `database/src/sql_gen.rs::create_table_ddl` 的三档回退。真机限制被钉在 `crates/database/tests/ddl_from_real_schema.rs`（补实现后需把断言翻成肯定式）。**同族缺口**：`list_indexes` 未实现在 `postgres` / `postgres_native` / `mysql_native`（另三个有）→ 这三个驱动的「索引」分区为空 |
