...
⁴ **列详情从 2026-09-21 起带三样东西**：`ordinal`（1 基，0 = 驱动没给）、`references`（`{table, column}`，**单列**外键的目标）、`is_foreign_key`（**含复合**外键）。分工是钉死的：`is_foreign_key` 答「有没有」，`references` 答「引到哪」——UI 不要用 `references.is_none()` 读成「不是外键」（见 `traits.rs` 的字段文档）。
...
| trait | 必需性 | 方法数 | 说明 |
| --- | --- | --- | --- |
| `Database` | **必需** | 4 必需 + 其余有默认实现 | `query` / `query_with_cancel` / `begin_transaction` / `meta`；`list_*`、`get_routine_source` 与 `get_table_ddl` 有默认（空 / `None`，2026-09-21 加） |
...
| 14 | ✅ | ~~**`mysql`（sqlx）在明文信道上连不上 MySQL 8+**~~（**已处置 2026-09-21**）：工作区 `sqlx` 的 features 有 `mysql` 但没有 **`mysql-rsa`**，于是 `caching_sha2_password`（MySQL 8+ 默认认证插件）在非 TLS 连接上无法用服务端公钥加密口令，建连报 `RSA auth backend disabled; … or use TLS` | **两个各自合理的默认值相遇成缺陷**：`connection_service::apply_lan_tls_default` 对 LAN 直连注入 `ssl-mode=DISABLED`，`MySqlDatabase::new` 在 URL 无 ssl 键时也补 `ssl-mode=disabled` ⇒ 网段内的 MySQL 8+ 一律连不上（9.7.2 端点实测）。这也解释了「为什么只有 MySQL 挂、PG 没事」：PG 的 SCRAM 不需要 RSA | 已修：workspace `Cargo.toml` 的 sqlx features 补 `mysql-rsa`（只多 `rand` / `rsa` 两个**已在依赖树里**的包，`Cargo.lock` +2 行）；新增真机回归 `crates/engine/tests/mysql_rsa_auth_probe.rs`（明文信道 RSA 认证 + TLS 对照，未设 `RDS_TEST_MYSQL_URL` 时跳过）；`editor_exec_real` 四库 26 项全绿。**教训**：真机套件「未设环境变量即静默跳过」＝ 平时不跑 = 缺 feature 这类缺陷可以活过很多轮 |
| 15 | ✅ | ~~**日期 / 金额 / JSON / UUID 静默变 NULL**~~（**已处置 2026-09-21**）：两个独立成因叠加——① workspace 的 sqlx **没开类型解码 features**（`chrono` / `bigdecimal` / `uuid` / `json`），`tokio-postgres` 也没开 `with-serde_json-1` / `with-uuid-1`；② **四个转换器的解码链里没试这些类型**（探测表只有 `bool/i16/i32/i64/f32/f64/Vec<u8>/String`）⇒ 试探全失败 → 整列声明 Utf8 → 解码再失败 → `.ok().flatten()` 给 `None` | **「读不出来」与「值就是 NULL」在出口层不可区分** —— 界面显示空白格，用户以为是数据本身为空。真机实测修复前：PG(sqlx) **19 列里 12 列**、PG(Official) **11 列**、MySQL(sqlx) **5 列**（含 `DECIMAL` 与 `NOW()`）静默 NULL；MySQL(Official) 因为一律出文本而不受影响。另有「`DECIMAL` 被声明成 `Float64` 而值是 NULL」的类型/值不一致形态 | 已修：① features——`sqlx` 行加 `bigdecimal`，`chrono`/`uuid`/`json` **只能开在后端 crate**（`sqlx-postgres` / `sqlx-mysql`：sqlx 的同名 feature 带 `sqlx-sqlite?/…` 弱引用，会把 sqlx-sqlite 拉进版本解析并与 rusqlite 的 `links = "sqlite3"` 硬冲突，交集为空无解）；`tokio-postgres` 加 `with-serde_json-1` / `with-uuid-1`。② 解码链——新增 `utils.rs::sqlx_cell_as_text!`（**一处定义**，四个转换器共用）+ `sqlx_value_is_null`（区分协议层 NULL）；`postgres.rs` 补 `i16`（`int2`）与文本链，`mysql.rs` 把 `DECIMAL`/`NUMERIC` 移出 f64 档改走 `BigDecimal::to_string()`（**标度保住**：`numeric(10,2)` 出 `"1234.5600"`），`postgres_native.rs` 补 `i16` 与文本链、`NUMERIC` 改走文本档。③ 解不出来时**留痕**（列级 `tracing::warn!`，带列名 / 声明类型 / 计数）。真机回归 `crates/engine/tests/type_decode_fidelity.rs`（4 项断言，含精度与 Binary 不被当文本）。**效果**：PG(sqlx) 12→4、PG(Official) 11→5、MySQL(sqlx) 5→**0**；引擎 493 项单测与真机执行套件全绿。~~**仍不可解码**（有意留白）~~ → **2026-09-21 第二批补上，见 #20**：PG `interval` / 数组 / `inet` / `point` 与 PG(Official) 的 `numeric` 现在都读得出来（做法是自己按 wire 格式解，`driver/native/pg_wire.rs`）；**仍不保证** `range` / 复合类型 / 几何族除 `point` 外 / `hstore` / 位串 / `tsvector` |
| 16 | ✅ | ~~**`list_constraints` 六个驱动一个都没实现**~~（**已处置 2026-09-21**）：每个驱动的 `MetadataBrowser::get_constraints` 都写成 `self.list_constraints(...)` 纯转发，而 `list_constraints` 落到 trait **默认空实现**（返回 `Ok(vec![])`）→ **属性面板的「约束」分区对所有库恒为空白**（主键 / 外键 / 唯一 / CHECK 全不显示）。与已修的 §7 #10（序列 / 触发器被空实现遮蔽）**是同一形态**，那一次只排查了序列与触发器 | ① 属性面板的「约束」分区此前是**死**的；② 合成 DDL 拿不到主键 / 外键 / 唯一（只能靠列上的 `is_primary_key` 兜主键）；③ **同一族还有 `list_indexes` 未实现在 `postgres` / `postgres_native` / `mysql_native`** （另三个有）→ 这三个驱动的「索引」分区为空。**真机还抳出第四个**：DuckDB 的 `list_indexes` 查了 `duckdb_indexes()` 里**不存在**的列（`index_type` / `expression`，真列是 `expressions`）→ 每次 `Binder Error`，而上层把 `get_indexes` 的错误吞成空 → **DuckDB「索引」分区恒空且无痕迹** | ✅ **六个驱动全部补齐**（2026-09-21）：PG 两个驱动走 `pg_constraint`（`unnest ... WITH ORDINALITY` 按列序拼名，`contype` / `confupdtype` / `confdeltype` 映射），MySQL 两个驱动走三张 `information_schema` 表（`GROUP_CONCAT ... ORDER BY ORDINAL_POSITION` 保列序），SQLite 走 `table_info` 的 `pk` 列 + `foreign_key_list` + `index_list` 的 `origin='u'`，DuckDB 走 `duckdb_constraints()`（含 `constraint_name` 与 `referenced_table`）；DuckDB 的 `list_indexes` 一并按真列重写。**SQL 与辅助函数都收敛在 `engine/src/driver/utils.rs`**（`PG_LIST_*_SQL` / `MY_LIST_CONSTRAINTS_SQL`，两个同族驱动共用一份，勿抄第二份）。真机回归 `crates/database/tests/ddl_from_real_schema.rs`（**6 个用例：四个主驱动 + 两个 Official**，每例一个独立表名 —— 并行跑同名探针表会撞 `Table already exists`）。**仍未做**：SQLite 的 `CHECK`（要解析建表 SQL）、DuckDB 的外键动作（`duckdb_constraints()` 不暴露） |
| 17 | ✅ | ~~**列详情缺「引到哪」与「第几列」**~~（**已处置 2026-09-21**）：`ColumnDetail` 只有 `is_foreign_key: bool`，而六个驱动**没有一个**填对——PG / SQLite / DuckDB 硬编码 `false`，MySQL 用 `column_key = 'MUL'` 顶替；列序号则根本没有字段 | ① 属性面板的「外键」列对 PG / SQLite / DuckDB **恒空**、对 MySQL 把**非唯一索引**（`MUL`）误报成外键；② 「引到哪」只能去约束分区反推（也就复合外键才列得全）；③ 序号靠数组下标 ⇒ 列表一旦筛选 / 重排就错位 | ✅ **六个驱动全部接上**：`ColumnDetail` 加 `references: Option<ForeignKeyRef{table, column}>`（**目标挂在列上**，取法照 dbui 的 `Column.references`，MIT）与 `ordinal: u32`（1 基，0 = 没给）；表详情 SQL 收敛成 `utils::{PG,MY,DUCK}_TABLE_DETAIL_SQL`（**列序契约**写在 `PG_TABLE_DETAIL_SQL` 的文档上）+ `utils::columns_from_detail_rows` **一处映射**——此前六个驱动各抄一份 `downcast_ref::<StringArray>` 循环，其中 `postgres.rs` 的字段序与 SQL 列序**对不上**（`r[9]` 会越界 panic，属未引爆的炸弹）；SQLite 走 `PRAGMA foreign_key_list`（`to` 为空 = 隐式引对方主键，**去查** `PRAGMA table_info` 的 pk 列，多列主键取序号最小的）。**复合外键两档一律按同口径处理**：`is_foreign_key` 照标、`references` 留空（配对要按列序做，交给约束分区）。面板：「外键」列改「引用」列（`表.列`；只有「是外键」时写 `FK`，不另开一列——面板是等宽 flex 列）+ `#` 用真序号 + 单列属性页补「序号 / 引用」两行。**顺带修一个潜伏 panic**：`duckdb::query_with_params` 在 `stmt.query()` **之前**取列名（duckdb-rs 的 `column_count` / `column_name` 读的是执行结果 → `The statement was not executed yet`），换用这条参数化路径时立刻炸——`query()` 里是执行后取的，所以至今没暴露。真机回归：`crates/database/tests/ddl_from_real_schema.rs` 六个用例各加 `assert_column_detail_refs`（序号连续 1 基 + `parent_id → 目标表.id` + 非外键列不得带引用；Official 两个探针表为此各补一列自引用外键）+ SQLite 离线单测三档（单列 / 隐式 / 复合） |
| 18 | ✅ | ~~**面板的 DDL 只能合成**~~（**已处置 2026-09-21**）：合成器吃不下的东西（存储引擎 / 字符集与排序规则 / 分区 / 表级选项 / `CHECK` 表达式 / **视图定义**）在**源版原文里本来就有**，而六个驱动里三个给得出原文（MySQL `SHOW CREATE TABLE`、SQLite `sqlite_master.sql`、DuckDB `duckdb_tables().sql`），此前一律走合成 | ① 面板给出的 DDL 与用户库里那份不等价，复制出去执行会丢引擎 / 字符集 / CHECK；② **视图没有 DDL 可看**（合成器要视图定义，目录数据里没有）—— 视图分区干脆不摆 | ✅ **三分落地**：`Database::get_table_ddl`（默认 `None` = 该库给不出，**不是**对象不存在 —— 后者是 `Err`）+ `MetadataService::get_table_ddl`。MySQL 两个驱动走 `SHOW CREATE TABLE`（**对视图同样有效**，返回 `Create View` 列 —— 真机确认，不必再分一套 `SHOW CREATE VIEW`）、SQLite 走 `sqlite_master.sql`、DuckDB 走 `duckdb_tables()` / `duckdb_views()` 的 `sql` 列；**PostgreSQL 没有等价物，恒定走合成**（要原文得自己用 `pg_get_*def` 拼装，那已经是合成）。面板按来源标 `DDL（源版）` / `DDL（合成）`，视图只在有源版时才出分区。顺带把两个 MySQL 驱动里**抄了两遍**的「`SHOW CREATE` 取值」收成 `utils::create_statement`（按列名 `Create …` 定位 —— 恒取列 1 会拿到 `sql_mode`，那是旧缺陷的成因）。真机回归 `crates/database/tests/source_ddl_from_real_schema.rs`（5 例：MySQL / MySQL(Official) / SQLite / DuckDB 各验表与视图的**原文特征**（`ENGINE=InnoDB` / `CHECK` / `DEFAULT 'x'` / `CREATE VIEW`），外加一条 PG 的**否定式**断言钉住「PG 给不出源版」） |
| 19 | ✅ | ~~**错误只给文本、不给对象**~~（**已处置 2026-09-21**）：撞约束时用户拿到的是一整段文本，界面原样糊上去 —— 里面其实有可操作的信息（`UNIQUE constraint failed: orders.tag` 就写着表 `orders`、列 `tag`），但没有任何结构化出口；`DatabaseError` 里那两个结构化变体（`ConstraintViolation` / `ColumnNotFound`）**建了却从来没人构造** | 「约束冲突了」这句话本身不可操作：用户得自己从文本里认出表 / 列 / 约束名，再去导航树里找。日志与历史面板里也只能按文本搜 | ✅ **六个驱动都填上了**（2026-09-21）：`DatabaseError::Query` 加 `location: Option<ErrorLocation>`（`ErrorLocation{schema,table,column,constraint}`，取法照 dbflux 的 `ErrorLocation`，MIT），`Display` 把它接在原因后面（`…（public.orders.tag，约束 uq_tag） (SQL: …)`）。**两条取法**：PG 两个驱动走**协议字段**（`PgDatabaseError` / `DbError` 的 schema / table / column / constraint）——**必须如此**：端点 locale 是中文时消息文本也是中文（`字段 "nope" 不存在`），文本解析当场失效；MySQL / SQLite / DuckDB 协议层没这些字段，走**消息解析**（`engine/src/driver/error_location.rs`，形态按真机样本写，离线单测用同一批样本）。口径：拿不到就不填（**不猜名字**）、复合键只给表不给列、外键冲突填子表（被引用父表不填）。真机回归 `crates/engine/tests/error_location_from_real_errors.rs`（6 例，postgres / postgres_native / mysql / mysql_native / sqlite / duckdb 各一，含 4 条**如实登记的边界**：PG 的未定义列/表不带字段（协议字段只覆盖约束类错误）、sqlx 那条 MySQL 路径真机没报出 FK 错、SQLite 与 DuckDB 的外键报错不点名） |
| 20 | ✅ | ~~**PG 的五类值静默变 NULL**~~（**已处置 2026-09-21，第二批**）：`postgres-types` 0.2.x 对 `NUMERIC` / `INTERVAL` **没有任何 `FromSql`**（也没有 decimal feature），`inet` / `point` 的解码器在 `postgres-protocol` 里但没接上，数组则**从来没试过** —— 于是「金额 · 时间间隔 · 网络地址 · 几何点 · 数组」一律交成空白格，而且 sqlx 与 Official **两个驱动的行为还不一致**（sqlx 有 `BigDecimal` 与 `PgInterval`，Official 什么都没有） | 同一个库两个驱动、同一列两种结果；金额与时间间隔恰恰是用户最常看的两类；`warn` 留痕只说明「解不出来」，不说明「差在哪一步」 | ✅ **五类全部补上**：新增 `crates/engine/src/driver/native/pg_wire.rs` —— 按协议文档自己解 wire 二进制（`NUMERIC` 的 base-10000 组、`INTERVAL` 的三个分量、`INET`/`CIDR` 的地址族+网段、`POINT` 的两个 f64），并**一份解析两边用**（tokio-postgres 侧实现 `FromSql`、sqlx 侧实现 `Type` + `Decode`，OID 一律按 `pg_type` 常量认）；数组在两边解出 `Vec<T>` 后按 **PG 数组字面量**拼（元素含逗号 / 引号 / 空串时按规矩加引号，否则分不回来）。格式化对齐 **PG 自己的文本输出**（`interval` → `1 day 02:03:04`、满位网段不打 `/32`、`point` → `(1.5,-2.25)`）。**一处有意留下的差异**：`numeric` 的 sqlx 那条走 `BigDecimal::to_string()`（按 4 位组拼，`numeric(10,2)` 出 `1234.5600`），Official 那条按 `dscale` 出 `1234.56` —— 值相等、写法差尾零，真机用例各按各的口径断言（不掩盖差异）。回归：`pg_wire` 离线单测 5 项（手拼字节）+ 真机 `type_decode_fidelity.rs` 扩到 20 列（`num` / `iv` / `arr` / `tarr` / `ip` / `pt` 进 `PG_COMMON`，两个 PG 驱动都断言具体文本）。**仍不保证**：`range` / 复合类型 / 几何族除 `point` 外 / `hstore` / 位串 / `tsvector` |
