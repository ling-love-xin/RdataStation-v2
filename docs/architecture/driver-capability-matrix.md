...
| 15 | ✅ | ~~**日期 / 金额 / JSON / UUID 静默变 NULL**~~（**已处置 2026-09-21**）：两个独立成因叠加——① workspace 的 sqlx **没开类型解码 features**（`chrono` / `bigdecimal` / `uuid` / `json`），`tokio-postgres` 也没开 `with-serde_json-1` / `with-uuid-1`；② **四个转换器的解码链里没试这些类型**（探测表只有 `bool/i16/i32/i64/f32/f64/Vec<u8>/String`）⇒ 试探全失败 → 整列声明 Utf8 → 解码再失败 → `.ok().flatten()` 给 `None` | **「读不出来」与「值就是 NULL」在出口层不可区分** —— 界面显示空白格，用户以为是数据本身为空。真机实测修复前：PG(sqlx) **19 列里 12 列**、PG(Official) **11 列**、MySQL(sqlx) **5 列**（含 `DECIMAL` 与 `NOW()`）静默 NULL；MySQL(Official) 因为一律出文本而不受影响。另有「`DECIMAL` 被声明成 `Float64` 而值是 NULL」的类型/值不一致形态 | 已修：① features——`sqlx` 行加 `bigdecimal`，`chrono`/`uuid`/`json` **只能开在后端 crate**（`sqlx-postgres` / `sqlx-mysql`：sqlx 的同名 feature 带 `sqlx-sqlite?/…` 弱引用，会把 sqlx-sqlite 拉进版本解析并与 rusqlite 的 `links = "sqlite3"` 硬冲突，交集为空无解）；`tokio-postgres` 加 `with-serde_json-1` / `with-uuid-1`。② 解码链——新增 `utils.rs::sqlx_cell_as_text!`（**一处定义**，四个转换器共用）+ `sqlx_value_is_null`（区分协议层 NULL）；`postgres.rs` 补 `i16`（`int2`）与文本链，`mysql.rs` 把 `DECIMAL`/`NUMERIC` 移出 f64 档改走 `BigDecimal::to_string()`（**标度保住**：`numeric(10,2)` 出 `"1234.5600"`），`postgres_native.rs` 补 `i16` 与文本链、`NUMERIC` 改走文本档。③ 解不出来时**留痕**（列级 `tracing::warn!`，带列名 / 声明类型 / 计数）。真机回归 `crates/engine/tests/type_decode_fidelity.rs`（4 项断言，含精度与 Binary 不被当文本）。**效果**：PG(sqlx) 12→4、PG(Official) 11→5、MySQL(sqlx) 5→**0**；引擎 493 项单测与真机执行套件全绿。**仍不可解码**（有意留白，见测试文件头）：PG `interval` / 数组 / `inet` / `point`（需 `ipnetwork` / `geo-types` 或逐元素解），以及 **PG(Official) 的 `numeric`** —— `postgres-types` 0.2.13 **没有任何 decimal feature**，要读它得自己按 wire 格式实现 `FromSql` |
