...
| 9 | ✅ | ~~**sqlx 静默忽略未知 URL 参数**（§2.1），而 `driver_properties` 的旧种子值是**MySQL C-API 名**（`connectTimeout` / `useCompression` / `characterEncoding`…）~~ | 驱动属性页里的这些键**写了也不生效**（不报错、无提示）；对 Official 驱动则是**直接报错** | **已收口（2026-09-19，两阶段）**：① 声明里只写各库真认的键（`mysql` 空、`postgres`/`postgres_native` 写 `application_name`、`mysql_native` 写 `max_allowed_packet`）；② 新增**属性规格** `engine/src/driver/property_spec.rs`（每驱动：库认的键清单 + 中文标签 + 未知键的效果），对话框属性页逐行标注**去向**（会下发 / 会被忽略 / 会报错 / 不下发）并列出该驱动的常用键，添加时立即给分级反馈；③ 顺手修掉根因 bug：`DriverConnectionConfig::append_query_params` 硬拼 `?`，已有查询串时会把已有参数并进上一个键的值（真机：`?ssl-mode=DISABLED` + `?ssl_mode=prefer` → sqlx 报 `unknown value`）——现在按有无查询串选 `?` / `&`。**剩余（阶段 3，未做）**：`DriverSide`（由驱动侧落实的键）——文件型驱动的 PRAGMA / 会话参数目前仍**一个都不下发**，现在界面会如实说「当前实现不会用它」，而不是让用户以为配上了 |
