...
### 2026-09-18 — 文档：一页读懂（宣传 / 速览页）

| 项 | 内容 | 落点 |
| --- | --- | --- |
| 新增单文件页 ✅ | `analytics-resource-intro.html`：定位（草稿箱 vs 资产库）+ 三个墩点 + 核心闭环四步 + 七个特点（每个带“为什么”）+ 分层与真相源 + 磁盘布局 + 界面速览（面板 / 详情示意，不靠截图）+ 能力清单（✅ / ○ 分别列清）+ 键鼠速查 + 硬约束六条 + 验证基线。单文件、明暗两套（同 `data-theme` 做法）、零外链、带打印样式 | `analytics-resource-intro.html`（新增） |
| 诚实标注 ✅ | 未落地项（分析表 / 引用档、拖拽到分组头、搜索匹配别名 / 标签 / 来源表、默认分组、`F2`、批量标签 / 移动、内容预览）在页内单独一栏标出，与 §0 进度记录同口径 | 同上 |
| 文档地图 ✅ | 模块入口 §6 加一行；顶部“五件套已齐”补一句指向它 | `README.md` |
