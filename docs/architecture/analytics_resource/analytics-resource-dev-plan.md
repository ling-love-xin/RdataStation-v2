...
>
> **案 A：设置页预设**——在「设置 › 资产库 › 默认分组」里选一个分组（或「未分组」），归档对话框的「分组」下拉打开时预选它。
> - 好处：**可预测**（预选值不会因为上次手滑选错而变），与另外两个设置项（默认排序 / 历史保留）同一形态，实现最简（读设置 → 当下拉初值，一次）。
> - 代价：想“上批都归到报表里”时要每次改设置，或者归档表单里逐次改下拉——**批量归档场景手感差**。
> - 风险：**分组被删后设置里留着悬空 id** —— 要按“认不出当未分组”处理（与 `filter` 的脏数据口径一致），且不必主动清理。
>
> **案 B：记住上次（“上次归到哪，下次就跟哪”）**——归档成功时把实际落点写回设置，下次对话框预选它。
> - 好处：**归档一连串同类产物时手感最好**（一次选好，后面都用它）；与「默认排序 = 上次用的那个」同一口径（仓库已有先例）。
