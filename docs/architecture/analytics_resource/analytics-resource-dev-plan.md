...
| 验证 | `cargo test -p rds-analytics-resource -j 2` → **66 单测 + 6 窗口测试全绿**；`cargo check --workspace --all-targets -j 2` 零告警 | — |

**取舍记录**：行内「打开 / 取回 / 移入回收站」三个按钮**下线**（改写进右键菜单）：原实现只在选中行出现，仍要占一行宽度；它们的正式替代是原型 §2.3 的 hover 版与详情面板的动作按钮，本批先保住“行的信息密度”。

**未落地**：详情面板接入、五个对话框、Action 与按键绑定（`Ctrl+F` / `↑↓` / `Enter` / `F2` / `Delete` / `Ctrl+A` / `Esc`）、行图标、批量多选。
