...
### 2026-09-18 — 文档：一页看懂（宣传 / 速览页，可贴版 + 视觉版）

| 项 | 内容 | 落点 |
| --- | --- | --- |
| 可贴版 ✅ | `analytics-resource-showcase.md`：关键数字 / 三个问题 / 核心闭环与架构分层（`mermaid`）/ 七个特点 / 归档五字段 / 界面骨架与键鼠 / 能力清单（✅ 与 ○ 分列）/ 硬约束六条 / 质量与验证 / 一期路线 + 文档地图。适合贴进 PR / wiki / 聊天 | `analytics-resource-showcase.md`（新增） |
| 视觉版 ✅ | `analytics-resource-showcase.html`：同一套内容的卡片化排版 + 面板 / 详情线框（HTML 模拟，不靠截图）+ 痛点三卡 + 闭环四步 + 分层与真相源 + 磁盘布局。单文件、明暗两套（同 `data-theme` 做法）、零外链、带打印样式 | `analytics-resource-showcase.html`（新增） |
| 纪律 ✅ | 未落地项（分析表 / 引用档、拖拽到分组头、搜索匹配别名 / 标签 / 来源表、默认分组、`F2`、批量标签 / 移动、内容预览）在两版里单独列出，与 §0 同口径，不拿规划当卖点 | 同上 |
| 文档地图 ✅ | 三处登记：模块索引（可贴版 / 视觉版分行）· 模块入口 §6 与顶部一行 · 使用手册顶部「第一次接触先看一页看懂」 | `docs/architecture/README.md`、`README.md`、`analytics-resource-user-guide.md` |
