...
> 状态：**设计定稿（2026-09-15）；Phase 0 三批 + Phase 1 七刀已落地**——归档 → 取回 → 再归档（指纹版本）闭环 + 变更事件 + 索引修复（三类孤儿）已可用（`ArchiveService` / `IndexRepair`）；面板（面板头 / **工具栏搜索·筛选·排序** / **`List` 虚拟化行 + kind 图标 + 右键菜单** / 状态行 / 两种空态）、**Action 与快捷键（`Ctrl+F` / `Esc` / `Delete`）**、**存档详情接入右栏（`RightPanel::Archive`，选中联动）**、呈现层（索引行 → 面板快照，含逐行详情）与 **workbench 接线（面板挂载 + 后台取数 + 端口回执）** 均已落地。逐项证据见 `analytics-resource-dev-plan.md` §0 进度记录。
> 仍待：五个对话框（归档 / 取回 / 版本 / 回收站 / 索引修复）· 详情面板的动作按钮与内容预览 · 编辑器只读打开（P1.6）· 批量多选（含 `F2` / `Ctrl+A`）· 废弃 `recycle.rs` → `ProjectTrash`（跨 crate，P0.8）· 版本保留策略接设置项。
>
...
| 详情属性面板（内容层） | `crates/analytics_resource/src/detail_view.rs`（现状：✅ 只读信息区；动作按钮 / 内容预览 / 危险区随对话框批） |
...
- **当前基线（2026-09-17）**：`69 单测 + 7 窗口测试`全绿（`cargo check --all-targets` 零告警）。
...
| Phase 1 | ✅ 面板骨架 · ✅ 行渲染（含 **kind 图标**）· ✅ 工具栏（搜索 / 筛选 / 排序）· ✅ 两种空态 · ✅ **列表虚拟化（`list::List`）+ 行右键菜单** · ✅ 详情面板内容层 · ✅ 呈现层 · ✅ 术语收尾（`f93d560`）· ✅ **workbench 接线（面板挂载 + 快照桥）** · ✅ **Action 与快捷键（`Ctrl+F` / `Esc` / `Delete`）** · ✅ **存档详情接入右栏（`RightPanel::Archive` + 选中联动）**；⬜ 五个对话框与动作真实现 · ⬜ 详情面板的动作按钮与内容预览 · ⬜ 只读三重守卫（编辑器侧）· ⬜ 批量多选（含 `F2` / `Ctrl+A`） |
