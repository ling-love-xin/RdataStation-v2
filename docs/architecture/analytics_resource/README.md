...
| 详情属性面板（内容层） | `crates/analytics_resource/src/detail_view.rs`（现状：✅ 只读信息区 + **底部动作区（取回…）**；内容预览 / 危险区随各自批次） |
...
| 宿主端口实现 | `crates/workbench/src/components/resource_host.rs`（现状：✅ 归档 = 文件选择 → 对话框 → 入队；取回 = 对话框 → 入队；打开只读 / 移入回收站 / 索引修复仍给明确回执） |
