...
> 状态：**设计定稿（2026-09-15）；Phase 0 三批 + Phase 1 六刀已落地**——归档 → 取回 → 再归档（指纹版本）闭环 + 变更事件 + 索引修复（三类孤儿）已可用（`ArchiveService` / `IndexRepair`）；面板（面板头 / **工具栏搜索·筛选·排序** / **`List` 虚拟化行 + kind 图标 + 右键菜单** / 状态行 / 两种空态）、**Action 与快捷键（`Ctrl+F` / `Esc` / `Delete`）**、详情面板内容层、呈现层（索引行 → 面板快照）与 **workbench 接线（面板挂载 + 后台取数 + 端口回执）** 均已落地。逐项证据见 `analytics-resource-dev-plan.md` §0 进度记录。
> 仍待：详情面板接入 · 五个对话框（归档 / 取回 / 版本 / 回收站 / 索引修复）· 编辑器只读打开（P1.6）· 批量多选（含 `F2` / `Ctrl+A`）· 废弃 `recycle.rs` → `ProjectTrash`（跨 crate，P0.8）· 版本保留策略接设置项。
>
...
| 左 Dock 面板（列表 / 工具栏 / 状态行） | `crates/analytics_resource/src/resource_view.rs`（现状：✅ Phase 1 五刀；`list::List` 虚拟化 + kind 图标 + 行右键菜单已落；详情面板接入 / 对话框 / Action 待做） |
...
| 取数后台任务（列表 + 索引健康） | `crates/workbench/src/services/resource_jobs.rs`（现状：✅ 单工作线程 + 结果槽；指纹扫描在工作线程上） |
| 宿主端口实现 | `crates/workbench/src/components/resource_host.rs`（现状：✅ 动作类请求给明确回执；对话框批接入后换成真实现） |
...
| Phase 1 | ✅ 面板骨架 · ✅ 行渲染（含 **kind 图标**）· ✅ 工具栏（搜索 / 筛选 / 排序）· ✅ 两种空态 · ✅ **列表虚拟化（`list::List`）+ 行右键菜单** · ✅ 详情面板内容层 · ✅ 呈现层 · ✅ 术语收尾（`f93d560`）· ✅ **workbench 接线（面板挂载 + 快照桥）** · ✅ **Action 与快捷键（`Ctrl+F` / `Esc` / `Delete`）**；⬜ 详情面板接入 · ⬜ 五个对话框与动作真实现 · ⬜ 只读三重守卫（编辑器侧）· ⬜ 批量多选（含 `F2` / `Ctrl+A`） |
