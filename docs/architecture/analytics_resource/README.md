...
> 状态：**设计定稿（2026-09-15）；Phase 0 三批 + Phase 1 五刀已落地**——归档 → 取回 → 再归档（指纹版本）闭环 + 变更事件 + 索引修复（三类孤儿）已可用（`ArchiveService` / `IndexRepair`）；面板（面板头 / **工具栏搜索·筛选·排序** / **`List` 虚拟化行 + 右键菜单** / 状态行 / 两种空态）、详情面板内容层、呈现层（索引行 → 面板快照）与 **workbench 接线（面板挂载 + 后台取数 + 端口回执）** 均已落地。逐项证据见 `analytics-resource-dev-plan.md` §0 进度记录。
> 仍待：详情面板接入 · 五个对话框（归档 / 取回 / 版本 / 回收站 / 索引修复）· 编辑器只读打开（P1.6）· Action 与按键绑定 · 批量多选 · 废弃 `recycle.rs` → `ProjectTrash`（跨 crate，P0.8）· 版本保留策略接设置项。
>
...
| Phase 1 | ✅ 面板骨架 · ✅ 行渲染（含 **kind 图标**）· ✅ 工具栏（搜索 / 筛选 / 排序）· ✅ 两种空态 · ✅ **列表虚拟化（`list::List`）+ 行右键菜单** · ✅ 详情面板内容层 · ✅ 呈现层 · ✅ 术语收尾（`f93d560`）· ✅ **workbench 接线（面板挂载 + 快照桥）**；⬜ 详情面板接入 · ⬜ 五个对话框与动作真实现 · ⬜ 只读三重守卫（编辑器侧）· ⬜ Action 与按键绑定 · ⬜ 批量多选 |
