...
> 状态：**设计定稿（2026-09-15）；Phase 0–3 主体与 Phase 2 前八刀已落地**——归档 → 取回 → 再归档（指纹版本）闭环 + 变更事件 + 索引修复（三类孤儿）已可用（`ArchiveService` / `IndexRepair`）；面板（面板头 `＋ ▾` 双入口 + `⋯` 四项 / 工具栏搜索·筛选·**五键排序** / 虚拟化行 + kind 图标 + 右键菜单 / 批量多选 / **分组折叠区** / 状态行 / 加载态 / 撤销栏 / 两种空态）、存档详情（标签 chips + 版本区 + 动作区 + 危险区）、**七个对话框（归档 / 取回 / 草稿选择 / 版本历史 / 索引修复 / 回收站 / 标签 / 分组）**、项目级回收站、只读三重守卫、**三个设置项（历史内容保留 / 默认排序 / 分组折叠记忆）**、**归档表单的别名 / 分组 / 标签真的生效**均已落地。**125 单测 + 27 窗口测试全绿**（逐项证据见 `analytics-resource-dev-plan.md` §0 进度记录）。
> 仍待：**草稿箱右键**入口（面板头 `＋ ▾` 那条已落）· 拖拽到分组头 · 搜索匹配别名 / 标签 / 来源表 · 默认分组 · 批量打标签 / 批量移动（P2.5）· `F2` 重命名 · 内容预览与详情头部可编辑 · 分析表 / 引用两档（Phase 4/5）。
>
...
| `analytics-resource-showcase.md` | **一页看懂（可贴版）**：关键数字 · 三个问题 · 核心闭环与架构分层（mermaid）· 七个特点 · 归档五字段 · 界面骨架 · 能力清单 · 硬约束 · 质量与验证 · 二期路线；适合贴进 PR / wiki / 聊天 |
| `analytics-resource-showcase.html` | **一页看懂（视觉版）**：同一套内容的卡片化排版 + 面板与详情线框（HTML 模拟，不靠截图）+ 明暗切换；单文件、零外链、离线可开、带打印样式 |
