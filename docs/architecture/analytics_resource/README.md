...
> 仍待：**草稿箱右键**入口（面板头 `＋ ▾` 那条已落）· 行尾绝对时间 tooltip（要等工具提示基建）· **Phase 4 采集侧**（DuckDB 探结构；口径与登记已落）· Phase 5（`table_ref`）· 详情版本区的“最近 3 条”明细。
>
...
| Phase 4 | `analysis` 档（DuckDB 表）：🟡 **口径与登记已落（第十七刀：`analysis.rs` 指纹 = 定义 + 结构、行数只作元信息；归档写定义 / 行数×列数；扫描侧不误报）**；⬜ DuckDB 采集侧 · ⬜ M7 Mock / M5 编辑器结果两处上游接入 |
