...
> 仍待：详情面板接入 · 五个对话框（归档 / 取回 / 版本 / 回收站 / 索引修复）· 编辑器只读打开（P1.6）· Action 与按键绑定 · 行图标与批量多选 · 废弃 `recycle.rs` → `ProjectTrash`（跨 crate，P0.8）· 版本保留策略接设置项。
>
...
| 左 Dock 面板（列表 / 工具栏 / 状态行） | `crates/analytics_resource/src/resource_view.rs`（现状：✅ Phase 1 五刀；`list::List` 虚拟化 + 行右键菜单已落；详情面板接入 / 对话框 / Action 待做） |
...
| Phase 1 | ✅ 面板骨架 · ✅ 行渲染 · ✅ 工具栏（搜索 / 筛选 / 排序）· ✅ 两种空态 · ✅ **列表虚拟化（`list::List`）+ 行右键菜单** · ✅ 详情面板内容层 · ✅ 呈现层 · ✅ 术语收尾（`f93d560`）· ✅ **workbench 接线（面板挂载 + 快照桥）**；⬜ 详情面板接入 · ⬜ 五个对话框与动作真实现 · ⬜ 只读三重守卫（编辑器侧）· ⬜ Action 与按键绑定 · ⬜ 行图标 · ⬜ 批量多选 |
