...
> 状态：**设计定稿（2026-09-15）；Phase 0 首切片已落地（2026-09-15，仅 crate 内）**——新增领域类型（`model.rs`）、本体层（`payload.rs`）、迁移 020 与 crate 入口文档，并修掉 7 项继承缺陷（含分页除零、连接嵌套、更新无事务、并发丢版本）。逐项证据见 `analytics-resource-dev-plan.md` §0 进度记录。
> 仍待：服务门面 `service.rs` / 索引修复 `indexer.rs`、store 接入新列、废弃 `recycle.rs`→`ProjectTrash`（跨 crate）、视图四处占位（Phase 1）。视图仍是 `workbench/src/panels.rs::render_resources_placeholder`（文案仍是 v1 语义）。
>
...
| 服务门面（归档 / 取回 / 检索 / 修复编排 + 事件） | `crates/analytics_resource/src/service.rs`（现状：未创建） |
| 索引修复（三类孤儿检测与修复） | `crates/analytics_resource/src/indexer.rs`（现状：未创建） |
...
| Phase 0（先做，无 UI） | ✅ 已落地（仅 crate 内）：crate 入口文档 · workspace 别名 · 迁移 020（+ 库层契约测试）· 领域类型 · 本体层 · 7 项继承缺陷修复 · `mod tests` 接线（此前未编译）｜⬜ 待续（需跨 crate 或后续阶段）：engine 连接池修复（`busy_timeout` / `acquire` 超时）· `.RSmeta` 常量去重 · store 接入新列 · `ProjectTrash` 上提中性化与 `recycle.rs` 废弃 · 版本指纹化重写 · 服务门面 + 事件 · 索引修复 · 测试改走迁移系统（详单见开发方案 §0） |
