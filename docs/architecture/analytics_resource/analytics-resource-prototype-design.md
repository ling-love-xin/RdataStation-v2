...
>
> **落地现状（2026-09-20，第十二刀）**：**拖拽已落**（规格那条「拖动行到分组头 = 移动」）。与规格的两处补充：① 落点**只有分组头**（多选时拖一条 = 拖整个选集；拖到「未分组」头 = 移出全部分组）；② 「全部分组」是聚合头**不接落点**，而「拖到某一行」也不接——资产库没有手工排序，拖到行上会承诺一个做不到的语义。悬停高亮用 `list_active`（与选中底同 token）；只读项目不给拖；拖到当前所在分组不发请求（与菜单里当前项置灰同一口径）。
