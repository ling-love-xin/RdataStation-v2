...
**落地现状**：只读信息区（头部 / 基本信息 / 来源 / 版本 / 标签与分组）已实现，落在**右 Dock 的「存档详情」档位**（`RightPanel::Archive`）而不是本节说的编辑区右侧 20rem 属性面板——`h_resizable` 属性面板那条路要等 M4 接线，宽度口径暂按 `RIGHT_DOCK_WIDTH`（17.5rem）取。尚未做：头部可编辑（改显示名 / 别名）、版本区的"最近 3 条 + 查看全部…"、内容预览、危险区；`TableRef` 的提示条已由 `alert_line` 覆盖（引用型常态提示）。
...
> **落地现状（2026-09-16）**：前三项（打开（只读）/ **取回（检出）…** / 移入回收站）已接在行的右键菜单上，破坏性项用分隔线隔离；其余项随各自批次落地（版本历史 / 重命名 / 标签 / 分组待对话框批，多选待批量批）。本体异常（缺失 / 内容已变）与只读项目下「取回」置灰。
...
| 对话框 | 倍率 | @16px | 说明 |
| --- | --- | --- | --- |
| 归档确认 | 34.0 | 544 | 表单型（显示名 / 分组 / 标签 / 来源 / 历史），六字段 |
