...
> **落地现状（2026-09-17）**：打开（只读）/ **取回（检出）…（已接对话框与真执行）** / 版本历史…（已接对话框与三个真动作）/ 移入回收站 / **复制路径** / **在系统中显示** 已接在行的右键菜单上，破坏性项用分隔线隔离；**多选态已接**：选择集 > 1 时单选项（含打开 / 取回 / 版本历史 / 复制路径 / 在系统中显示）全部置灰，**删除项变“移入回收站（N 项）”并对整个选择集生效**。本体异常（缺失 / 内容已变）与只读项目下「取回」置灰（详情面板的动作区同一判据）；**本体不可定位**（缺失 / 旧行没登记路径）时「复制路径」「在系统中显示」也置灰。重命名 / 标签 / 分组仍待各自批次（多选时它们将变“批量打标签 / 批量移动”）。
