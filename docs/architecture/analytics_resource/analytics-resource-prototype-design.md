...
> **落地现状（2026-09-20，第十四刀）**：已落。与规格的两处补充：① **只有文本型且不太大（≤ 1 MiB）才读**——按本体路径的扩展名判断（显示名改过可能不再带扩展名），二进制（开头带 NUL）与超阀值都给“仅元信息”；② 预览块里**一行一条、不折行**（长行省略号）——折行会让“前 20 行”变成“前 20 行的一部分”；末尾多一句说明（截断时说“仅显示前 20 行”，否则不说）。读数只读开头 8 KiB，截断处切在最后一个换行上；读失败**只记日志、不打断刷新**（预览是附加信息，不是刷新能否成功的前提）。
| 危险区 | 移入回收站（`danger` 文字按钮） |

> `TableRef` 型的详情面板**必须在最顶部显示失效警告条**（`warning` 描边，沿用编辑器只读条样式）："这是引用，源表可能已变更或被删除" + "立即校验"按钮。

**落地现状**：只读信息区（头部 / 基本信息 / **来源** / **版本** / 标签与分组 / **内容预览**）+ **底部动作区「打开（只读）」与「取回（检出）…」** + **危险区「移入回收站」**（P0.8，danger 按钮）已实现，落在**右 Dock 的「存档详情」档位**（`RightPanel::Archive`）而不是本节说的编辑区右侧 20rem 属性面板——`h_resizable` 属性面板那条路要等 M4 接线，宽度口径暂按 `RIGHT_DOCK_WIDTH`（17.5rem）取。**版本分区自 2026-09-17 起总是出现**（值为“N 个历史版本”或“只有当前版本”）并带「查看全部…」入口（打开版本历史对话框）。尚未做：头部可编辑（改显示名 / 别名）、版本区的“最近 3 条”明细（需要一次批量“每存档最近 N 版”取数）；**内容预览已落（第十四刀）**，详情面板的「组织」分区同批把**分组名真的填上**（此前那一段永远不出现）；`TableRef` 的提示条已由 `alert_line` 覆盖（引用型常态提示）。
