...
│  │   ├ 🐘 生产 PG [G]             │  │  ③ 驱动(定宽 11rem，实现短名) · URI(弹性)
...
- **Header 只 3 行（建议布局）**：① 类型徽标（定宽 1.75rem，仅图标）+ 名称（弹性）+ 作用域三态分段；② 备注（弹性）+ 项目（定宽 17rem）；③ 驱动（定宽 11rem）+ URI（弹性）；标签列统一 1.75rem 左对齐。
...
- 顶部信息条（info banner）：`类型 · 驱动实现 · 特性`（如「MySQL · sqlx · 原生连接」；未选类型时为选型提示）
...
| URI | 自绘**可编辑输入框**（直接改文本；无「✎ 编辑 / 完成」两态） |
