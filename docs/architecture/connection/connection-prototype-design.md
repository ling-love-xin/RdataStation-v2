...
│  ├ 搜索框（input）                │  ├ Header：                     │
│  ├ 暂存列表（saved-section）      │  │  ① 名称 + 类型徽标 + 驱动 + 作用域
│  │   ├ 🐬 A · MySQL 演示（未保存）│  │     （类型在左侧选，驱动只列实现短名；
│  │   └ ＋ 新建配置                │  │      未选类型时提示“请先在左侧选择数据库类型”）
│  ├ 分割线                         │  │  ② 备注（宽输入）              │
│  └ 数据库类型（db-section）        │  │  ③ URI 实时预览 + ✎ 编辑      │
│      ▾ 关系型(4) 文件型(2) …      │  │  ④ 作用域语义提示行            │
│      ▸ NoSQL(2) 分析型(2)         │  ├ tabs-bar（tab_bar 底）        │
│                                   │  │  常规 | 网络 | 能力 | 驱动属性 | 高级
└───────────────────────────────────────────────────────────────┘
   底部操作栏： [🔌 测试连接]  ✓ 成功·版本·延迟          [取消] [保存] [保存并关闭]
```

- **类型 × 驱动两层分工**（2026-09-11 去噪）：数据库类型只在左侧栏选，暂存条目最左侧显示缩小的类型 UI（类型 `icon`（emoji），如 🐬 MySQL / 🐘 PostgreSQL）；Header 驱动下拉不再重复类型名，只列当前类型的**驱动实现短名**（`MySQL (sqlx)` → `sqlx`、`MySQL (Official)` → `Official`），同类型下实现名唯一
...
| 作用域 | 全局/项目**可同选**（双轨设计）：仅全局→`G_xxx`；仅项目→`P_xxx`；全局+项目→保存全局并共享至当前项目（`GP_xxx` 快照）；Header 使用**三态分段按钮**（比下拉紧凑）；提示行实时显示保存目标（2026-09-11：项目根由当前项目会话自动预填，已打开项目时仅显示项目名——悬停气泡展示完整路径） |
