...
│  ├ 搜索框（input）                │  ├ Header（建议布局）：            │
│  ├ 暂存列表（+ 添加）              │  │  ① 🐬 类型徽标(仅图标) · 名称(弹性) · 作用域分段
│  │   ▸ 固定高度 7.5rem，超出滚动    │  │     （徽标定宽；未辨识时显示 ?）      │
│  │   ├ 🐬 A · MySQL 演示（未保存）│  │  ② 备注(弹性) + 项目(定宽 17rem，仅全局灰显)
│  ├ 数据库类型（类型树）            │  │     项目为单下拉：项目名 + 路径，末项新增项目            │
│  │   ▸ 占满剩余高度，超出滚动       │  │  ③ 驱动(定宽 11rem) · URI(弹性)
│  │   ▾ 关系型(4) 文件型(2) …      │  ├ tabs-bar（tab_bar 底）        │
│  │   ▾ 关系型(4) 文件型(2) …      │  │  常规 | 网络 | 能力 | 驱动属性 | 高级
│  │   ▸ NoSQL(2) 分析型(2)         │  ├ Tab 内容（固定 20.5rem，超出滚动）
│                                   │  └ 结果提示行（载入 / 保存 / 校验消息）
└───────────────────────────────────────────────────────────────┘
...
- **Header 只 3 行（建议布局）**：① 类型徽标（定宽 6rem）+ 名称（弹性）+ 作用域三态分段；② 备注（弹性）+ 项目（定宽 17rem）；③ 驱动（定宽 11rem）+ URI（弹性）；标签列统一 2.75rem 左对齐。
...
- **Header 元素定宽**：类型徽标 6rem / 驱动 11rem / 项目 17rem / 作用域分段短标签——切换数据库类型时后续元素位置不移动。
...
- 条目结构（v1 v5 `saved-item`）：类型徽标（缩小的数据库类型 UI：类型 emoji，如 🐬；无类型时回退状态点）+ `item-name`（名称）+ `✕`；选中项高亮（`sidebar.accent` + coral 左边条）。
- 本轮增强：已保存条目名称后附**来源短码徽标**（`P` / `G` / `GP`，按 `saved_id` 最长前缀优先解析）；当前条目有未写回修改时名称后显示**脏标记 `●`**（warning 色）；最左侧为缩小的数据库类型 UI（类型 emoji；无类型时回退状态点）。
- 标题行按钮：`导出` / `导入`（C4 模板 JSON 走剪贴板；仅未保存且非空草稿；导入后提醒补填密码）/ `+ 添加`。
...
| 交互 | 行为 |
| --- | --- |
| 类型树 | 分类折叠/展开；搜索过滤；选中类型 → 右侧驱动下拉仅列该类型**启用驱动**（实现短名，如 `sqlx` / `Official`）并默认选中第一个；未选类型时下拉为空且 Header 提示「请先在左侧选择数据库类型」；条目类型徽标取类型 `icon` |
| 作用域 | 全局/项目**可同选**（双轨设计）：仅全局→`G_xxx`；仅项目→`P_xxx`；全局+项目→保存全局并共享至当前项目（`GP_xxx` 快照）；Header 用**三态分段按钮**（短标签 全局 / 项目 / 全局+项目，写库仍用全称标签），右侧小字实时显示落点（`→ 全局库` / `→ 项目库` / `→ 全局库 + 项目快照`） |
...
| 模板导入导出 | 暂存列表标题行 `导出 / 导入`：剪贴板 JSON（`kind`/`version` 校验），**不含密码**；导出仅未保存且非空草稿；导入追加并选中（提示补填密码） |
