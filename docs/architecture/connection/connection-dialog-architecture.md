...
| 100 | **声明跑在功能前面就是假承诺（能力字典加 `Stage`）**：`CapabilitySpec::stage`（`Ready` / `NotBuilt`）——`index_analysis`（索引分析）与 `table_editor`（表编辑器）标 `NotBuilt`：产品里没有这两个入口（全仓无索引分析功能、无表编辑器），却一直被**六个驱动**声明（11 处）；本轮从全部驱动撤掉声明，能力 Tab 把这两个键显示成「索引分析（功能未实现）」（按未声明着色、恒不打勾）；新单测 `capability::tests::not_built_keys_are_claimed_by_nobody` 盯住「未实现的键不得被任何驱动声明、不得标已验收」，`helpers` 侧断言行里带 `not_built` 且**永不**是 `declared` | 字典里留着这两个键是为了记「打算做什么」，但**声明**是给用户看的承诺：D10 口径说未验收不算可用，同一道理——**没实现更不算**（能力 Tab 上每多一行“支持”，就有一个人去找那个不存在的入口）。功能落地时在 `descriptors.rs::capability_keys()` 里加回来即可（该函数的文档已写明这个条件） |
| 101 | **右列按“剩余高度”分配，而不是再给它一个固定高（#94 的收尾）**：`tab_body` 由 `h + min_h + max_h(rems(BODY_H))` 改为 `flex_1 + min_h_0`，右列 `h_full + min_h_0`，侧栏 `h_full + min_h_0`；SideBar 与右列都以**行高**为唯一尺寸来源；新增测试锚点 `conn-body-row`（两列行）/ `conn-result-row`（右列最后一个固定块） | #94 只把“行高”定住了，但内容区又按 `rems(BODY_H)` 写死：右列在行首还有 Header + Tab 条，于是内容区比行底多出 **168px**（实测 1920×1080：行 88.5→608.5，内容区 256.5→776.5），越出部分被 Dialog body 的 `overflow_hidden` 裁掉——**Tab 内容底部一截怎么滚都看不到**。而原有断言只比“侧栏高 == 内容区高”（两边都是 520）所以盖不住。现在断言的是**几何关系**（侧栏高 == 行高；`conn-result-row` 底 ≤ 行底），且实测每帧结果行都恰好贴在行底。**注**：内容区上挂的 `debug_bounds` 落在 Scrollable 重排后的**滚动内容**（`h_auto + min_h_full`）上，它的高度是内容高（可大于视口，属正常滚动行为），不能拿来断言视口 |
| 102 | **在途操作离开 UI 线程（测试 / 保存）**：新增 `BusyOp`（None / Test / Save）挂到对话框状态；`do_test` / `do_save` 拆成「前台取快照 → `background_executor` 跑服务 → `spawn_in` 回前台写结果」；期间按钮 `loading(true)` + 置灰（防并发写库），`取消` 始终可点；「保存并关闭」的关闭动作改到**结果回来之后**（`close_after` 由续体兑现）；`do_save` 增 `close_after: bool` 参数 | 服务层是同步 `block_on` 风格，直接在点击回调里跑会**阻塞 UI 线程**：连不上的主机要等满超时（可达数十秒），窗口会被系统判成“无响应”，连 `loading` 动画都转不起来（UI 线程被占住，渲染不发生）。只带纯数据过线（`DataSourceSaveInput` / `Vec<ConnectionItem>` / `(bool, String)`），`Rc` 状态与窗口一律留在前台；新增 `SaveOutcome` 承载“保存成功但分组未同步”的降级语义 |
| 103 | **缺口的报法：给“第一个缺什么”，不是笼统的一句「请填写…」**：新增纯函数 `helpers::save_blocker`（判据与 `ClonedDialogState::collect` 逐项一致：类型 → 驱动 → 名称 → 地址；文件型地址文案不同），三处消费：拦截文案、决定 Header 哪个标签标红（`header_label_state`，只换色不动行高）、后续若要给按钮置灰；`blocked_hint` 只在**被拦下过一次后**才让标签变红 | 旧文案「请填写名称、驱动与连接 URL」把三个项列一遍，用户得自己判断缺哪个；而置灰按钮（无 tooltip 时）更不能解释原因——“点了才告诉你”与“永远不告诉你”之间，取前者。**未采用“缺项即置灰”**：置灰后无法再解释缺什么，而 0.6.1 里禁用按钮的 tooltip 是否触发依赖组件实现，不拿它当唯一通道 |
| 104 | **凭据默认掩码（密码框与档案字段）**：新增 `SECRET_FIELD_KEYS`（`password` / `passphrase`）+ `is_secret_field` + `secret_input`（`InputState::new(...).masked(true)`）——连接表单的密码框与认证 / 网络管理器按类型展开的凭据行都用掩码输入，渲染处接 `Input::mask_toggle()`（组件自带眼睛按钮）；`helpers::tests::secret_fields_are_the_credential_keys_only` 钉住“只盖凭据”（路径 / 主机不掩）；窗口用例 `connection_render_matrix::password_input_masks_display_without_hiding_the_value` 钉住**掩码只影响渲染**（`value()` 仍是明文） | 全仓此前无任何 `masked` / `mask_toggle` 调用（grep 为空）：密码在输入框里是**明文显示**（编辑回读还会把解密后的密码直接显出来）。为什么必须同时钉住 `value()`：0.6.1 的 `InputState::value()` 文档就写成“masked input returns its masked value”——若真如此，存下去的就是圆点，而错误要到连接时才暴露（看起来像“凭据不对”）。实测：`value()` 返回的是原始文本，掩码只发生在渲染层 |
| 105 | **矮窗口不再把 footer 顶出屏幕 + 侧栏可点行加悬停 + 搜索空态**：① 行高走纯函数 `helpers::dialog_row_height`（设计基准 32.5rem 与「窗口可用高度 × 0.9 − 5.5rem 开销」取小，下限 16rem），调用点只传 `viewport_height / rem_size`；② 类型树行与草稿行加 `hover`（原型 §5 的悬停 = `list.hover`），**选中行不接 hover**（hover 会盖掉选中底色）；③ 搜索无命中 / 目录为空各给一行空态（锚点 `conn-type-empty`），文案分开（“没搜到”与“没加载到”不是同一件事） | ① 对话框总高约 610px、纵向位置是 `viewport/10`：768 高的小屏（再叠 125% 缩放 → 逻辑高 ~614px）会把 footer 顶出屏幕；夹住后由内容区（`flex_1`）与侧栏（`h_full`）自己让位。② “可点但没有任何反馈”与“空白侧栏分不清是搜不到还是坏了”都是可发现性缺口——空态文案区分的依据是 `filter_text` 是否为空 |
| 106 | **键盘选型路径：搜索框回车 = 选中第一个匹配的类型**：新增纯函数 `type_matches_filter`（类型名 / id / 驱动名命中）与 `first_selectable_type`（**优先有启用驱动的**，全无驱动时退回第一个命中项以让 `select_type` 给出原因行），树的渲染与回车路径**共用同一份判据**；容器上的 `InputEnter` 分支只在**焦点确实在搜索框**时生效（其它输入框里的回车不得改选型），占位改为「搜索类型 / 驱动（回车选中）」 | 类型树的行是自绘可点行（Tab 到不了它们），而「选类型」是唯一没有键盘入口的必填步骤。**验证口径**：匹配逻辑由 `helpers` 内嵌单测盯住（含“优先有驱动”“无命中退回”“无命中为 None”）；端口链路（输入框 Enter → `InputEnter` → 容器分支）在 headless 下**拿不到**（app 层的 `Ctrl+Enter` / `Ctrl+T` 绑定也写在 `main.rs`，测试里不存在），所以真机验收写进用户指南 §9.1 F 段，不假称已被自动化盖住。**补充（决策 #107）**：类型列表换成 `List` 后，键盘入口由组件自带（↑↓/Enter，已有窗口用例覆盖）；搜索框回车保留为“过滤后一步选中”的快捷路径，不再是唯一键盘入口 |
| 107 | **侧栏两处列表改用 `list::List` 组件（自绘行退场）+ `✕` / `+ 添加` 换真按钮**：① 类型列表 → `type_tree::TypeTreeDelegate`（**小节 = 分类**，空分类由组件跳过；空态走 `render_empty`；确认走与鼠标同一个入口 `select_type`）；② 草稿列表 → `draft_list::DraftListDelegate`（1 小节；行尾删除用 `ListItem::suffix` 放 `Button::ghost()`；`set_selected_index` 即“切条目”语义，与容器级 `DraftPrev` / `DraftNext` 一致）；③ 行用 `ListItem`——悬停 `list.hover`、选中 `list.active` 底 + `list.active.border` 描边（原型 §5 的映射本来就是这两个 token，配色不变，只是改由组件画，不再自绘色条）；④ `✕` / 「+ 添加」改 `Button::ghost()`（icon + tooltip，键盘可达）；⑤ 删除按钮回调里 `cx.stop_propagation()` | ① 自绘可点行没有键盘导航 / 焦点 / a11y（技能里点名的回归）；组件自带 ↑↓、Enter、Esc（且 `Cancel` 会 `cx.propagate()`，**不会**拦掉对话框自身的 Esc——已核对 0.6.1 源码）、滚动到选中项与虚拟化；② **顺手修一个既有缺陷**：删除草稿的点击会继续冒泡到行的确认，把光标切到被顶掉位置的另一条草稿（gpui 的 `Button` 只在 disabled 时阻断冒泡，启用时不管——所以要自己 `stop_propagation`）；③ 两处列表同源，侧栏不会再一个用组件行、一个用自绘行。**验证**：`connection_render_matrix::{type_list_is_keyboard_navigable, draft_list_is_keyboard_navigable}`——首次给“选类型 / 切草稿”补上键盘自动化覆盖（↑↓ + Enter；无驱动类型被拒且结果行给原因） |
| 108 | **分组折叠改由组件承担：`Collapsible` + 语义 `Button`**：`outline_section` 的标题行从 `cursor_pointer` + `on_click` 的 div 换成 `Button::ghost()`（`h_full` 行高 / `px_0` / `ButtonRounded::None`，底部通栏分隔线不变），展开 / 收起从 `if !collapsed { … }` 换成 `Collapsible::new().motion_id(sec-motion-{id}).open(!collapsed).child(带 `.id()` + `.test_support()` 的行容器).content(body)`；新增测试目标 `conn-sec-{id}`（标题行：kit 观测可 `find` / `click_at`）与 `conn-sec-body-{id}`（内容体：收起时仍在树里、但 `visible()` 为假） | ① 自绘可点 div 没有悬停 / 焦点 / 键盘激活 / a11y，而折叠标题行是 Tab 内容里唯一的“非表单可点区”；② 揭示动效交给组件内的 spring（`motion_id` 给稳定 identity，中断可从当前值反转），不再“啪”地一跳；③ **带 `motion_id` 后内容保持挂载**（`MotionReveal` 按进度夹高度）——收起分组不重建段落、不丢输入焦点与滚动位置（代价：内容体在收起时**仍在元素树里**，所以“真收起了没”的判据要落在**可见性**或后面分组的位置上，不能只看它存不存在——这正是决策 #109 换用 `find` 的理由之一）。**验证**：① `helpers.rs::tests::header_interaction::clicking_the_title_row_collapses_and_restores`（**真点击**，不带对话框的窗口：点标题行**远端的空白处**也能触发回调 → 正文被夹成不可见 → 再点复原；headless 下鼠标事件到不了对话框层，见下文“代价”）；② `connection_render_matrix::outline_section_collapses_from_its_whole_title_row`（对话框内：标题行快照可见 + 整行宽；由 `toggle_section`（与 `on_click` 里同一个方法）驱动，断言收起后**正文不可见但仍挂载**、后面的分组上移，展开后回到原位）。**代价**：headless 下鼠标命中测试到不了对话框层（`Root::render_dialog_layer` 的 deferred 子树）——同样结构的行在**普通窗口**里一点就中（已实测，作为对照写进了上面那个用例）；本仓已有两处同款注记（`analytics_resource/tests/dialog_window.rs`、`editor/src/view/tests.rs`），所以对话框侧的点击 / 悬停 / 键盘观感归真机验收（用户指南 §9.1 F 段） |
| 109 | **测试锚点改用 kit 观测（`find` + `ElementSnapshot`），`debug_selector` 在本模块退场**：对话框里需要断言/交互的元素统一 `.id(...)` + `.test_support()`（`conn-general-guide` / `conn-type-empty` / `conn-side-panel` / `conn-staging-scroll` / `conn-body-row` / `conn-result-row` / `conn-result-{detail,toggle,copy}` / `conn-prop-note-*` / `conn-sec-{id}` / `conn-sec-body-{id}`）；测试侧用 `window.find(id)` / `try_find(id)`（`bounds()` / `visible()`）取代 `cx.debug_bounds("<selector>")` | 旧口径是「只有 `debug_selector` 能进 `debug_bounds` 表，`.id()` 不进」，它能回答的只有“这个节点在不在元素树里”：**隐藏 / 被裁掉的元素也照样被登记**（于是“展开后才出现”这类断言其实是弱断言），而且 id 拼错时只得到一句“没找到”。kit 的观测（`ElementSnapshot`）多给出：**`visible()`**（样式可见 + 未被裁 + 在视口内）、**`bounds()`**、a11y 事实（`role` / `label` / `selected` / `checked` / `disabled` / `value`）、以及取不到时的**已登记路径**；同一批 id 还能直接 `click_at` / `hover` / `press`（真机与测试共用一套标识）。**代价 / 例外**：`test_support()` 在非测试构建是零成本转发（`#[cfg]` 返回原元素，已用 `cargo check -p rds-app` 验证生产路径）；两处继续用 `debug_selector`——`conn-tab-body`（组件 `Scrollable` 会把内层 id 换成 `(caller, "content")`，该元素进不了观测）与 `dialog-layer`（上游 gpui-component 的锚点） |
...
| 窗口 | `connection_render_matrix.rs`（组件化轮） | 降级路径的状态矩阵（空态引导条未渲染 / 可见 / 降级消失的循环、五 Tab 逐一渲染且行高不变、作用域三态、结果行四级、暂存区固定高度、右列不越出行高）；**侧栏两处列表的键盘路径**（↑↓ + Enter，决策 #107）；**分组折叠**（标题行快照可见 + 整行宽；收起后正文**被夹成不可见但仍挂载**、后面的分组上移，展开后回到原位，决策 #108）；**密码掩码只影响渲染**（`value()` 仍是明文，决策 #104） |
| 窗口（内嵌，无对话框层） | `connection_dialog/helpers.rs::tests::header_interaction`（决策 #108） | 分组标题行的**真点击**：点行远端空白也能触发 `on_toggle`（整行都是按钮）→ `Collapsible` 收起后面板变矮 → 再点复原。为什么单独开窗口：headless 下鼠标事件到不了对话框层（deferred 子树），而“点击 → 回调”这段接线必须用真点击钉住 |
...
**窗口断言用哪套锚点（决策 #109）**：默认用 kit 观测——目标元素 `.id(...)` + **`.test_support()`**，测试侧 `window.find(id)` / `try_find(id)` 取 `ElementSnapshot`（`bounds()` / `visible()`）；`find` 取不到会 panic 并列出**已登记路径**，`visible()` 能区分「不在元素树里」与「在树里但被裁掉 / 隐藏」。旧的 `debug_selector` + `cx.debug_bounds(...)` 已在本模块退场，**只保留两处**：① `conn-tab-body`——组件 `Scrollable` 渲染时会把内层元素 id 换成 `(caller, "content")`（`scrollable.rs` 的 `self.element.id(content_id)`），该 div 进不了观测；② `dialog-layer`——上游 gpui-component 的 debug_selector（不是我们的元素）。

两条经验：**（1）首帧 ≠ 稳定帧**——对话框内容会在头几帧继续长高（实测第 5 帧才稳定），拿首帧当几何基线会得到偏差；做前后对比前先空出几帧（`draw_frames`）。**（2）滚动区域里的元素照样能观测**（本节现在就用 `find` 量分组高度与可见性），只是坐标语义与以前一致：量到的是**滚动内容**的坐标。
...
> **UI 组件化轮（2026-09-20）**：两批把对话框里剩下的自绘可点元素换成语义组件——第一批（决策 #104 / #107）侧栏两处列表 → `list::{List, ListItem}` + 委托（并顺手修掉“删草稿的点击冒泡到行确认”）、`✕` / `+ 添加` → `Button::ghost()`、密码框 → `masked(true)`；第二批（决策 #108）分组标题行 → `Button::ghost()` + `Collapsible`（`motion_id` 的 spring 揭示动效，收益：悬停 / 键盘 / a11y / 收起时内容保持挂载）。三处兼修的真缺陷：右列越出行高 168px（#101）、测试/保存阻塞 UI 线程（#102）、矮窗口 footer 出屏（#105）。**测试基建同轮升级（决策 #109）**：窗口断言改用 kit 观测（`.id()` + `.test_support()` → `window.find(...)` 的 `visible()` / `bounds()`），`debug_selector` 在本模块只留两处（`conn-tab-body` 被 `Scrollable` 换掉 id、`dialog-layer` 是上游锚点）。剩余自绘可点元素仅在 `panels/`（另有会话负责）。
...
| 新增（🔴） | **两列不等高 + 侧栏撑高对话框（本轮拖出，决策 #94）**：侧栏自然高 522px、Tab 内容区固定 328px → 右列下方留 194px 空白，且**类型树条目数反向决定对话框高度**（目录增长即变高）。修法：先定行高（`DIALOG_BODY_HEIGHT = 32.5rem`，三向夹住），侧栏与内容区各自同高填满；`DIALOG_TAB_BODY_HEIGHT` 降为内容区最小高参考值。**后续更正（决策 #101）**：“内容区也三向夹住 520px”是错的（右列在内容区之上还有 Header + Tab 条，导致越出行底 168px 被裁）；现已改为右列按剩余高度分配，断言也从“两侧等高”改为几何关系 | `connection_edit_backfill`（目录就绪 → 侧栏 == 行高 520px）、`connection_render_matrix`（降级路径同样成立 + 结果行底 ≤ 行底） |
...
**已关闭（2026-09-20：右列溢出 · 凭据掩码 · 在途操作异步 · 缺口提示 · 折叠组件化）**

| # | 关闭方式 | 验证 |
| --- | --- | --- |
| 新增（🔴） | **右列越出行高 168px → Tab 内容底部被裁（怎么滚都看不到）**：`tab_body` 改 `flex_1 + min_h_0`（不再写死 `rems(BODY_H)`），右列与侧栏改 `h_full + min_h_0`；新增测试锚点 `conn-body-row` / `conn-result-row`（决策 #101） | `connection_render_matrix`（侧栏高 == 行高；`conn-result-row` 底 ≤ 行底；切 Tab 行高不变）、`connection_edit_backfill`（目录就绪路径同断言）。实测：修前 内容区 256.5→776.5 / 行底 608.5；修后每帧结果行恰好贴在行底 |
| 新增（🔴） | **密码框明文显示**（表单密码 + 档案 `password` / `passphrase` 字段）：`masked(true)` + `mask_toggle()`（决策 #104） | `helpers::tests::secret_fields_are_the_credential_keys_only`、`connection_render_matrix::password_input_masks_display_without_hiding_the_value`（掩码 + `value()` 仍是明文） |
| 新增（🟡） | **测试连接 / 保存阻塞 UI 线程**（连不上的主机要等满超时，窗口被判“无响应”，loading 也转不起来）：改「前台取快照 → `background_executor` 跑服务 → `spawn_in` 回前台写结果」，`BusyOp` 挡住重复触发，「保存并关闭」的关闭动作改到结果回来之后（决策 #102） | `ConnectionDialogState` / `do_test` / `do_save` 路径由现有窗口回归（`connection_dialog_ui` / `connection_render_matrix` / `dialog_host_layer` 全绿）覆盖渲染与宿主重绘；**真机待验**：长超时主机的“窗口不卡死 + loading 可见” |
| 新增（⚪） | **缺项提示笼统 + 无字段级指引**：`helpers::save_blocker` 纯函数 + Header 标签标红（决策 #103） | `helpers::tests::save_blocker_reports_the_first_missing_item`（含文件型 / 网络型文案分流、全空格判空） |

| 新增（⚪） | **缺项提示笼统 + 无字段级指引**：`helpers::save_blocker` 纯函数 + Header 标签标红（决策 #103） | `helpers::tests::save_blocker_reports_the_first_missing_item`（含文件型 / 网络型文案分流、全空格判空） |
| 新增（⚪） | **矮窗口 / 高缩放下对话框会把 footer 顶出屏幕**（行高只按设计基准，不夹窗口） | `helpers::tests::row_height_follows_the_viewport_and_has_a_floor`（基准 / 768+125% 夹住 / 极矮下限）；真机：在小窗口里缩放页面看 footer 是否仍可见 |
| 新增（⚪） | **侧栏可点行无悬停反馈 + 搜索无命中只留空白** | 窗口用例：`connection_render_matrix::dialog_state_matrix_renders_on_degraded_path`（`conn-type-empty` 在目录为空时出现）；悬停为视觉反馈，真机看 |
| 新增（🟡） | **选类型没有键盘入口**（类型树行是自绘可点行，Tab 到不了）：搜索框回车选中第一个匹配类型（决策 #106） | `helpers::tests::{type_filter_matches_name_id_and_driver_name, keyboard_type_match_prefers_a_type_with_a_driver}`；**端口链路真机待验**（headless 拿不到 `InputEnter`，见决策 #106） |

| 新增（⚪） | **侧栏两处列表是自绘行**（无键盘导航 / 焦点 / a11y；删除按钮的点击还会冒泡到行的确认） | **已改由 `list::List` + `ListItem` 渲染**（决策 #107）；自动化：`connection_render_matrix::{type_list_is_keyboard_navigable, draft_list_is_keyboard_navigable}`；真机：Tab 进侧栏后 ↑↓ / Enter 的观感 |

| 新增（⚪） | **分组折叠是自绘可点 div**（`cursor_pointer` + `on_click`、`if !collapsed` 手删内容）：无悬停 / 焦点 / 键盘激活，且展开收起是“啪”地一跳 | **已改由 `Collapsible`（`motion_id` → 组件内 spring）+ 语义 `Button::ghost()` 标题行渲染**（决策 #108）；自动化：`helpers::tests::header_interaction::clicking_the_title_row_collapses_and_restores`（真点击）+ `connection_render_matrix::outline_section_collapses_from_its_whole_title_row`（对话框内几何 / 可见性）；真机：点标题行的悬停底色、展开 / 收起动效、键盘 Enter·Space 激活 |

| 新增（⚪） | **窗口断言只会用 `debug_bounds`**（只回答“节点在不在元素树”，隐藏元素也算在；id 拼错只说“没找到”） | **改走 kit 观测**：`.id()` + `.test_support()` + `window.find(...)` → `visible()` / `bounds()` / a11y 事实 / 已登记路径（决策 #109）；对话框 4 个用例文件已迁移（`connection_render_matrix` / `connection_dialog_ui` / `connection_type_driver` / `connection_edit_backfill`）；真机：无需（测试基建） |
