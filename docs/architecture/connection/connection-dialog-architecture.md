...
| 驱动属性初始行 | **`drivers.driver_properties`**（按当前驱动填：换驱动即重填，用户手改过的不覆盖）/ 连接记录 `driver_properties` | ✅（**2026-09-19 修正**：#90——此前初值是 UI 硬编码的 `ssl_mode=prefer` + `connect_timeout=10`，两个键对 Official 驱动都是未知参数，对 sqlx 则静默无效） |
