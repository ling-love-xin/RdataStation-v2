...
| 108 | **分组折叠改由组件承担：`Collapsible` + 语义 `Button`**：`outline_section` 的标题行从 `cursor_pointer` + `on_click` 的 div 换成 `Button::ghost()`（`h_full` 行高 / `px_0` / `ButtonRounded::None`，底部通栏分隔线不变），展开 / 收起从 `if !collapsed { … }` 换成 `Collapsible::new().motion_id(sec-motion-{id}).open(!collapsed).child(带 `.id()` + `.test_support()` 的行容器).content(body)`；新增测试目标 `conn-sec-{id}`（标题行：kit 观测可 `find` / `click_at`）/ `conn-sec-body-{id}`（内容体）/ `conn-sec-panel-{id}`（面板：收起后回落到只剩标题行） | ① 自绘可点 div 没有悬停 / 焦点 / 键盘激活 / a11y，而折叠标题行是 Tab 内容里唯一的“非表单可点区”；② 揭示动效交给组件内的 spring（`motion_id` 给稳定 identity，中断可从当前值反转），不再“啪”地一跳；③ **带 `motion_id` 后内容保持挂载**（`MotionReveal` 按进度夹高度）——收起分组不重建段落、不丢输入焦点与滚动位置（代价：内容体锚点在收起时**仍存在**，高度由进度夹住，所以归档判据放在**面板**上，见下条验证）。**验证**：① `helpers.rs::tests::header_interaction::clicking_the_title_row_collapses_and_restores`（**真点击**，不带对话框的窗口：点标题行**远端的空白处**也能触发回调 → 面板高度回落 → 再点复原；headless 下鼠标事件到不了对话框层，见下文“代价”）；② `connection_render_matrix::outline_section_collapses_from_its_whole_title_row`（对话框内：标题行被 kit 观测登记且可见 / 行宽 == 面板内宽；由 `toggle_section`（与 `on_click` 里同一个方法）驱动，断言收起 / 复原的**几何**与“只动被切换的分组”）。**代价**：headless 下鼠标命中测试到不了对话框层（`Root::render_dialog_layer` 的 deferred 子树）——同样结构的行在**普通窗口**里一点就中（已实测，作为对照写进了上面那个用例）；本仓已有两处同款注记（`analytics_resource/tests/dialog_window.rs`、`editor/src/view/tests.rs`），所以对话框侧的点击 / 悬停 / 键盘观感归真机验收（用户指南 §9.1 F 段） |
...
| 窗口 | `connection_render_matrix.rs`（组件化轮） | 降级路径的状态矩阵（空态引导条 / 五 Tab / 作用域三态 / 结果行四级 / 暂存区固定高度 / 右列不越出行高）；**侧栏两处列表的键盘路径**（↑↓ + Enter，决策 #107）；**分组折叠**（标题行被观测登记且可见 / 行宽 == 面板内宽；`toggle_section` 驱动 → 面板高度回落、只动该分组、再展开复原，决策 #108）；**密码掩码只影响渲染**（`value()` 仍是明文，决策 #104） |
...
| 新增（⚪） | **分组折叠是自绘可点 div**（`cursor_pointer` + `on_click`、`if !collapsed` 手删内容）：无悬停 / 焦点 / 键盘激活，且展开收起是“啪”地一跳 | **已改由 `Collapsible`（`motion_id` → 组件内 spring）+ 语义 `Button::ghost()` 标题行渲染**（决策 #108）；自动化：`helpers::tests::header_interaction::clicking_the_title_row_collapses_and_restores`（真点击）+ `connection_render_matrix::outline_section_collapses_from_its_whole_title_row`（对话框内几何）；真机：点标题行的悬停底色、展开 / 收起动效、键盘 Enter·Space 激活 |
