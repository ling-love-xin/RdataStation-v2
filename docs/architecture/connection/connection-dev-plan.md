...
| C12 | **C4 模板导入导出（不含密码）**：暂存列表标题行「导出 / 导入」走剪贴板 JSON（`kind` / `version` 校验；仅未保存且非空草稿；导入追加并解析驱动短名）｜✅ 2026-09-11 已实现（`connection_template.rs` 2 项） | `connection_dialog/staging.rs`（`ConnectionTemplate` / `TemplateItem` / `templates_{export,import}`）、`render.rs` | 导出 JSON 无密码；非法输入被拒且列表不变 |
