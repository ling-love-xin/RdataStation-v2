...
> 状态：**Phase A/B + 剩余缺口补齐已实现（2026-09-10，cargo check --workspace 通过、零告警）** · Phase C 生态收尾待排期 · 关联文件：`connection-prototype-design.md`（原型）、`connection-dialog-prototype.html`（可交互原型）
...
| 服务编排层（connection crate `service.rs`；应用级 test_connection；模型 `model.rs`） | ❌ 未实现（占位） |
| UI 层（GPUI 对话框 `connection_dialog.rs`；panels.rs 5 字段表单待退役） | ❌ 未实现 |
| v1 蓝本（stores/services/AddDataSourceDialog.vue 53KB） | ✅ 已实现，可参照 |

**缺口即本方案实现对象**：DataSourceService（测试/保存/更新/删除 + Secret 联动）+ GPUI 新增连接对话框（v5 布局）。
...
| A2 | 服务：`DataSourceService`——类型/驱动查询（driver_store）、认证/网络/环境配置读取（stores）、`test_connection`（独立会话，复用 ConnectionFactory + 探测版本）、`save/update/delete`（global_db + AES 凭据 + Secret 联动） | `crates/connection/src/service.rs` | 集成测试：真实 PostgreSQL/DuckDB 新增→测试→保存→回读→更新→删除 |
| A3 | 对话框骨架：标题栏/左侧栏（搜索+暂存+类型树）/Header（名称+驱动类型+作用域+备注+URI 编辑）/常规 Tab（动态表单+认证+**连接安全 SSL**）/测试/保存/取消 | `crates/connection/src/connection_dialog.rs` | UI 手动走通全流程；配置按 config_schema 渲染 |
...
- 每阶段：`cargo check --workspace` 零告警 + 对应集成测试（`crates/connection/tests/`）
