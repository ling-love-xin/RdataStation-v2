...
| I14 | **落地数据之间的引用完整性**：出口以「当前表」为单位，而关系是跨表的 | 只落子表不落父表，项目库里就有悬空值（而 mock 不改已落地数据，也不会替用户补）——未来 JOIN 会落空。**当前处置**：面板在出口区给 warning 行（「引用了 x → y.z：只落这张表，被引用的表不会跟着落库」/「被 … 引用：引用它的一方会落空」），并在场景态说明“场景生成不记入历史” | 可选方向：**批量落库**（按关系闭包一起落，需定义部分失败怎么办）；或落库时给目标表加 `FOREIGN KEY` DDL（但 DuckDB 要求父表先存在、插入顺序受限，与现在的自由落库/追加相冲）。**未拍板** |
