...
| I11 | **「列依赖 / 外键」这套模型全仓库空转**：`ColumnDependency`（含 `ref_table` / `ref_column`）与 `DependencyType` 都在，`resolve_dependencies` 会读 `dependency` 做 Kahn 拓扑排序——但**没有任何生产代码写过 `Some(_)`**（全部构造点 16 处皆 `None`，模板宏 `col!` 也没有能设依赖的分支），且**生成路径从不调用 `resolve_dependencies`**；`GeneratorConfig::ForeignKey { values }` 是「从给定字符串集合随机取」（`generators.rs:728`），与跨表引用无关 | 场景模板产出的多表**之间没有引用完整性**（`order_items.order_id` 不会取到 `orders.id`）。原型容易让人误以为已经做到：v1 原型的生成器条目写着 `example: '引用目标表主键'`，模拟实现却是 `return 'FK_' + Math.floor(r * 999)`（`v1/prototype/mock-data-generator.html:1032, 1627`）；v1 引擎的同名分支也是 `#[allow(dead_code)]` 且只取**同一行**的源列值 | 两条路选一，**先拍板再改代码**：**(a) 真做**——同模板内先给被引用表建主键池，引用列从中采样（`ref_table` / `ref_column` 是现成挂点，不必新造模型）；**(b) 明确降级**——把 `DependencyType` 与 `ForeignKey` 的文案改成「取值集合」，不再暗示跨表引用 |
