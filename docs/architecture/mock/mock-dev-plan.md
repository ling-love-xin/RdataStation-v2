...
| C3 | 列依赖编辑器（`Expression` / `Template` / `Weighted`） | 若做：Mock 面板的列编辑 + 引擎求值（模板 → 加权 → 算术三条路成本递增） | **先拍板**：要么实现求值，要么把四个空壳变体与三个空字段从模型里删掉（`resolve_dependencies` 拓扑排序已随死代码删除，现状见架构 §9-I6） |
