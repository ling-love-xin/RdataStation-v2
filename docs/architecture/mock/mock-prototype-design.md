...
- 场景表之间的**外键引用完整性**：原型里没有可画的东西（v1 也没实现），先拍板再画，见 §9-I11；
...
| 生成器清单里的「🔗 外键」条目与筛选标签 | **降级** | v1 的 `foreign_key` 是「值集合」而非跨表引用（模拟实现为 `'FK_' + 随机数`）；v2 不设筛选标签（改分类子菜单 + 搜索），也不在文案上暗示跨表引用——真做与否见 `mock-architecture.md` §9-I11 |
