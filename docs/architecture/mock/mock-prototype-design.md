...
- **业务日历**：**已落地为列级参数**（架构 D42）——`sequential_date` / `sequential_date_with_gaps` / `date_time_between` 各有
  「仅工作日 + 工作周掩码 + 跳过日期（节假日）+ 上班日期（调休）」，`date_time_between` 另有「仅工作时段（09:00~18:00）」。
  未做的三件：① 工作时段窗口自定义（现固定 09:00~18:00）；② 把日历参数扩到 `date` / `date_time` / `date_time_before` / `date_time_after`（助手现成）；
  ③ 内置节假日表——**刻意不做**：法定节假日与调休每年发通知，内置表必然静默过期。
...
`date_time_between` 则只采工作日，另有「仅工作时段（09:00~18:00）」；工作周掩码 `1111100` = 周一~周五，
「跳过日期」（节假日）与「上班日期」（调休）两个列表表达年度例外，面板把掩码翻成人话（「周一~周五」）并**只在启用时**把它们写进字段摘要。
...
| 工作日历参数非法 | 列编辑对话框里改「工作周掩码 / 跳过日期 / 上班日期」（日期串、掩码佋看） | 日历列表与取值集合不同：**输入时不校验**（可先粘一半）；生成时在**生成前**拦住，中央表头 / 面板 danger 行写清哪一列、哪个字段、期望格式（`YYYY-MM-DD` / 7 位 0/1），改完重试即可 |
...
| 8 | ~~数值 / 时序生成器覆盖面~~ | ✅ 部分已定：补上分布族（泊松 / 指数 / 帕累托 / Beta / 二项）与 `time_series`（趋势 + 周期 + 噪声），共 143 变体；**业务日历**已落地为**列级参数**（工作周掩码 / 节假日 / 调休 / 工作时段，不做表级共享与内置节假日表）；**仍未做**：「跟随另一列取值」（跨列求值，与第 6 项一起立项）、工作时段窗口自定义 |
