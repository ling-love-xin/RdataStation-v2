...
| 生成中 | —— （当前同步执行，无中间态） | 待办：后台任务 + 进度 + 取消（引擎已有 `cancel()` 与批次回调） |
...
| 生成 / 应用 / 取消 / 导入 | `Button`（`primary` / `secondary`） | 不手搓 `div + on_click` |
...
| 进度条 / 取消（`mock:generate-progress`） | 待办（Phase B） | 事件通道退役 → 批次回调 + 面板状态位；引擎 `cancel()` 已就绪 |
...
| 5 | 生成中态 | 建议引入：后台任务 + 进度 + 取消（引擎 `cancel()` 与批次回调均已就绪，当前是同步调用，大行数会卡界面） |
