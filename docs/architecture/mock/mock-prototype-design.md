...
| 预览可排序、不筛选、行数可选 | 表头**右端的箭头**点一下 = **按该列重查临时表**（`ORDER BY … LIMIT N`，见 §8；表头正文那一拍是组件的「列选择」，本表未开）；右键菜单里有同一套（升 / 降 / 取消排序）；**行数档在预览标题行右侧**（10 / 25 / 50 / 100 / 200，生成时只读了 10 行，选更多即重查）；箭头画的是**生效中的排序**（以面板回推的快照为准，被拒时箭头会被纠回去）。筛选仍不做：它把「前 N 行取样」变成长度未知的子集；排序与扩行数做的都是**全局前 N 行**，与标题里的「前 N 行」同一个口径 |
| 列名也看不全时 | 表头文字截断后同样给悬停全文（`render_th` 自画：截断 + 悬停；行号槽 `#` 不给） |
| 拖过的列宽不会被重建打回 | delegate 按**列名**记宽度（订阅 `TableEvent::ColumnWidthsChanged`，组件在拖完 mouse-up 时报全部列宽）；重建表头（重查 / 改行数 / 重新生成）时 `column()` 交回记下的宽度 |
...
> 预览重建表头时的列宽：**拖过的宽度会活下来**（delegate 按列名记，订阅组件的 `TableEvent::ColumnWidthsChanged`）；重建表头（重查 / 重新生成）不再把用户拖好的宽度打回默认——只有列集合真的换了才回默认档。
...
| 预览行数档 | 预览标题行右侧的 `Button::ghost().xsmall()` + `dropdown_menu`（与语言 / 场景模板同一套菜单） | 档位固定（`PREVIEW_LIMIT_CHOICES`）：行数是**重查代价**的旋钮（`LIMIT N`），不必用输入框去解析与拦非法值 |
| 宿主能力 | `MockHost`（生成 / 出口 / 来源 / 只读 / 打开详情 / 重绘 / **预览重查取样**） | 与 `project::ui::ProjectUiHost` 同范式；workbench 侧桥接见 `components/mock_host.rs`（`preview_sample` 是全端口里唯一**非阻塞**的：拿不到内存库锁就回落 `None`，因为它是随手动作，不值得等一个不可取消的出口任务） |
...
| 生成 → 预览 | **已迁** | 预览表落中央 tab（组件库 `DataTable`：列宽可拖 + 横向滚动 + `#` 行号槽），生成时取前 `PREVIEW_ROWS`（10）行；**重查取样（2026-09-20 补）**：排序（表头右端箭头 / 右键菜单）与**行数档**（10 / 25 / 50 / 100 / 200）走 `MockEngine::try_preview_sample` → `SqlEngine::build_select_ordered` |
