...
> 交互稿：`mock-prototype.html`（v2 原生，RDS Light/Dark + 15 场景可切）。
...
│ users 1,000 行 / products 500 行 / …     │ ← 本次要生成的表清单（每行可删，删表连带清入边）
│ [＋ 加表（当前草稿）]                     │ ← 自定义多表：把单表态调好的表加进来
│ 表间关系（3 条）                          │
│ orders.user_id → users.id        [删除]   │ ← 关系是派生视图（真身在列的 dependency）
│ order_items.order_id → orders.id [删除]   │
│ [＋ 加关系]                               │ ← 四个下拉：子表.列 → 父表.列
...
| 生成器清单里的「🔗 外键」条目与筛选标签 | **降级** | v1 的 `foreign_key` 是「值集合」而非跨表引用（模拟实现为 `'FK_' + 随机数`）；v2 不设筛选标签（改分类子菜单 + 搜索），也不在文案上暗示跨表引用。**但模板里的真实引用不靠它表达**——靠「范围对齐父表行数」，见 `mock-architecture.md` §9-I11 |
