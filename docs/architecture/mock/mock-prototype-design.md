...
| 场景模板生成成功 | 「场景模板 ▾」选一套内置模板 | `已按「电商系统」生成 4 张表（合计 21,500 行）：出口作用于当前选中的那张`；结果区出现「结果表（4 张）」+「当前表」下拉（`{表名} ▾`）+ 来源说明；出口作用于选中那张；**草稿不被改写** |
| 结果表切换 | 结果区 / 详情标题栏的「当前表」下拉 | `gen_info` 跟着换（结果摘要 / 预览 / 详情都作用于它）；越界下标**不改状态**；只有一张时不摆选择器（单表生成后自动消失） |
...
| 跨表后果 | 当前结果表参与了关系（引用了别表 / 被别表引用） | 出口区一行 **warning**：「引用了 `user_id → users.id`：只落这张表，被引用的表不会跟着落库」/「被 `order_items.order_id` 引用：只落这张表，引用它的一方会落空」——出口只作用于当前表，关系却是跨表的（架构 §9-I14） |
