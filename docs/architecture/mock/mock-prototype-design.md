...
> 交互稿：`mock-prototype.html`（v2 原生，RDS Light/Dark + 13 场景可切）。
...
### 4.4 场景模板（`run_scenario`）

两条状态：**选模板**（下拉展开）与 **多结果 / 当前表**。
...
│           社交平台       4 张表 · 161,000 行 ┘
├─────────────────────────────────────────┤
│ 结果表（4 张）              [orders ▾]   │ ← 只在结果多于一张时出现
│ 来自场景模板「电商系统」：出口只作用于当前选中的那张表 │
│ orders · 临时表 temp_mock_orders · 5,000 行 · 18 ms │
└─────────────────────────────────────────┘
```

- **一选即生成**：选模板就提交后台任务（`MockJobKind::Scenario`），**不经「生成」按钮**、也不改草稿；
- **菜单不现场重算**：模板清单只留名称与规模（`ScenarioChoice`），渲染期不构造模板本体；
- **结果表是一组**：单表生成 1 张，场景模板 N 张；「当前表」下拉的项目是 `{表名}（{行数} 行）`，
  出口 / 详情 / 预览三处都跟它走（越界下标不改状态）；
- **量纲按张表**：进度行显示 `{done} / {total} 张表`（`rows_total` 为 0，不显示行数与百分比文案），
  与单表生成的 `{done} / {total} 批（≈N / M 行）` 是同一组件的两种量纲；
- **草稿与结果解耦**：中央 tab 的字段表仍是草稿的，与场景表无关 → 预览上方要给一句来源说明，
  否则用户会拿草稿的列去对预览的表头（这是场景态**必须**有的一句话）。

### 4.5 尚未落地（本稿不画）

- 列依赖编辑（`dependency` / `DependencyType`）：模型已就位但生成器不解释表达式，见 `mock-architecture.md` §9-I6；
- 表间引用的**模型化**（现在是把子表范围手写成父表行数；已是能用的关系，缺的是可校验与自动跟随），见 §9-I11；
