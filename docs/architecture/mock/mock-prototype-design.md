...
| 场景模板生成成功 | 「场景模板 ▾」选一套内置模板 | `已按「电商系统」生成 4 张表（合计 21,500 行）：出口作用于当前 tab 那张`；中央**开出 4 个 tab**（每表一个，标题 = 表名 · 行数，首个为当前）；右 Dock 结果区出现「结果表（4 张）· 点一行切到它的 tab」+ 来源说明；出口作用于当前 tab；**草稿不被改写** |
