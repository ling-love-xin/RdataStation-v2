...
| 场景模板选择与一键生成 | **已迁** | 面板「场景模板 ▾」菜单（6 套内置）→ `MockJobKind::Scenario` 后台任务 → 装配层 `generate_scenario_at`；一次产多张临时表（**不写库**），结果区与详情用「当前表」下拉切换，出口作用于选中那张；进度按「张表」报（见 `mock-architecture.md` §4.3） |
