...
| 列依赖编辑器 | 待拍板 | 引擎**不解释依赖表达式**（`Expression` / `Template` / `Sequence` / `Weighted` 是 v1 遗留空壳），`dependency` 现只用于跨表引用（架构 §9-I6 / I11）；要做还是删空壳先定一个 |
...
| 6 | 依赖表达式 | `Expression` / `Template` / `Weighted` 的取值计算：要么实现求值（模板 / 加权 / 算术三条路成本递增），要么**把空壳从模型里删掉**（`dependency` 只留跨表引用）。当前是「既不求值、也没删」——四个变体与三个字段零使用（架构 §9-I6） |
