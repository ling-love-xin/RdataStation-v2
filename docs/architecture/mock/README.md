...
cargo test  -p rds-mock -j 2                                   # 130 单元（含 58 视图）+ 32 引擎 + 5 持久化往返 + 4 历史/模板 + 2 清理 集成
...
| `cargo test -p rds-mock` | 130 单元（21 纯逻辑 + 37 窗口 + 72 其他）+ 32 引擎集成 + 5 持久化往返 + 4 历史/模板集成 + 2 清理集成全过 |
