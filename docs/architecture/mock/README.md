...
| 工作日历就在列上 | **列级参数**（不与别的列共享、不内置节假日表）：仅工作日 + 工作周掩码（`1111100` = 周一~周五）+ 跳过日期（节假日）+ 上班日期（调休，优先）；顺序日期按**工作日数**推进，`date_time_between` 落在休息日就重抽（不成则顺延），另有「仅工作时段（09:00~18:00）」。判定顺序与预算：架构 D42 |
