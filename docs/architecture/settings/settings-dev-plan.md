...
### 2026-09-18 — M6 接线（再续）：`resources.collapsed_groups`

| 项 | 内容 | 落点 |
| --- | --- | --- |
| 新项 | `resources.collapsed_groups`（复合值：**项目根路径 → 折叠的分组 key 列表**；`entry = Module`，不上页） | `src/{model,registry,lib}.rs` |
| 为何按项目分桶 | 分组 id 是每项目独立生成的：平铺一份列表会在面板丢掉“不认识的 id”（分组被删）时把别的项目的记录一起抹掉；空列表 = 删掉该项目的记录 | 同上 |
| 作用域声明 | 它是**项目专属**的视图状态，严格按 §2.2 判据属项目级；开新迁移 + 后台往返的代价不抵“一屏展开形状”的收益，故暂住 `settings.json` 并登记待确认 | `settings-architecture.md` §2.2 边界示例 + §14 Q7 |
| 消费方 | 面板构造期读（`set_collapsed`）+ 点分组头时写回**当前全集**（宿主按项目分桶）；`slot_is_scalar` 把两个复合槽都排除在页面之外 | `workbench/src/{components/resource_host.rs,panels/resources.rs}` |
| 验证 | `cargo test -p rds-settings -j 1` **21 项全绿**（登记表契约测试自动覆盖：复合值不上页 / `composite` 标记与形态一致 / 无标量读写路径）；`cargo test -p rds-analytics-resource -j 1` **124 + 27 全绿**（+1 面板窗口：注入折叠态当场生效且不回写） | §5 |

### 2026-09-18 — M6 接线（续）：`resources.default_sort`

| 项 | 内容 | 落点 |
| --- | --- | --- |
| 新项 | `resources.default_sort`（Enum 五列，默认 `name`；入口 = 设置页 + 模块内；生效 = 下次操作）：面板里点一次排序就写回，“默认排序” = 上次用的那个（不另给一份会与实际脱节的设置） | `src/{model,registry,lib}.rs` |
| 消费方 | 宿主构造期读 → `SortField::from_key`（不认识的 key 回退默认）→ `ResourcesPanel::set_sort` 注入；面板侧新增端口 `ResourcesHost::remember_sort`（用户动作才写，注入不写——否则默认值会被当成动作回显） | `workbench/src/{components/resource_host.rs,panels/resources.rs}` |
| 验证 | `cargo test -p rds-settings -j 1` **21 项全绿**（登记表契约测试自动覆盖新项）；`cargo test -p rds-analytics-resource -j 1` **124 + 26 全绿**（+1 面板窗口：注入只重排不回写） | §5 |

**一处刻意的取舍**：**只存字段不存方向**——同一列两个方向存成两个 key 会让设置页多出只能二选一的行；方向按字段惯例（名称升序，时间 / 大小 / 版本降序），当次的翻转不入库。
...
| # | 任务 | 依赖 | 验收 |
| --- | --- | --- | --- |
| P4.1 | `resources.keep_versions` 落地（模型 + 登记 + 页面行 + 归档调用接线） | M6 dev-plan P2.4 | ✅ **已落（2026-09-18）**：页面上是预设档行（含 `-1` = 全部保留），消费方是 M6 的归档 / 版本还原作业（宿主读设置→转领域类型→随作业带入）。同批还落了 `resources.default_sort`（两个入口：设置页 + 面板内点排序）与 `resources.collapsed_groups`（复合值、不上页）。**余**：M6 的"默认分组"需先拍板语义再登记 |
