...
| # | 任务 | 依赖 | 验收 |
| --- | --- | --- | --- |
| P4.1 | `resources.keep_versions` 落地（模型 + 登记 + 页面行 + 归档调用接线） | M6 dev-plan P2.4 | ✅ **已落（2026-09-18）**：页面上是预设档行（含 `-1` = 全部保留），消费方是 M6 的归档 / 版本还原作业（宿主读设置→转领域类型→随作业带入）。**余**：M6 侧的"默认分组 / 分组折叠态"两项还需过准入五条再登记（后者按架构 §2.2 属项目级状态，不进 `settings.json`） |
