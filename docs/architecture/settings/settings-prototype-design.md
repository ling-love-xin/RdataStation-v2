...
| 数据源导航 | 来源标识 | `navigator.source_short_code` | segmented 短码 / 文字 | 即时 | `database/src/nav_view.rs::render_connection_row` |
| 数据源导航 | 显示标签 | `navigator.show_tags` | Switch | 即时 | `database/src/nav_view.rs::render_connection_row` |
| 数据源导航 | 显示归属域 | `navigator.show_scope` | Switch | 即时 | `database/src/nav_view.rs::render_connection_row` |
...
| # | 状态 | 项 | 说明 |
| --- | --- | --- | --- |
| 1 | ✅ | 两栏页面 | 已实现并**接到工作台**（分节导航 + 内容区 + 搜索 + 恢复默认）；本表 #3/#4/#5 同批完成 |
| 2 | ✅ | 分段控件改用组件 | 枚举 / 两态 / 数值预设档统一走 `TabBar::segmented`；旧视图里的手搓按钮组随旧视图退役 |
| 3 | ✅ | Quick Open 与设置页互斥 | 四处入口都改（⚙×2、标题栏搜索、两个 action） |
| 4 | ✅ | `Esc` 关闭 / `Ctrl+F` 聚焦搜索 | 页面挂 `key_context("settings")` + 焦点句柄，app 层绑键；**打开即接管焦点**（否则刚打开按 `Esc` 没反应） |
| 5 | ✅ | 尺寸契约扫描 | `ui_contract` 尺寸 + 颜色扫描均含 `settings_page.rs`；8 个 `SETTINGS_*` 数值进契约 1 |
| 6 | 🟡 | `ToggleThemeMode` 未接线 | Action 已定义但无键位、无 `on_action` 处理；**要么接线、要么删除**（见架构 §14 Q2） |
| 7 | ✅ | 写盘失败静默 | 已关闭：原子写 + 错误槽 + 页面危险色横幅（架构 §13 K2） |
| 8 | ✅ | 搜索 | 搜索行 + 结果列表（节 › 行 面包屑）+ 无结果空态；只搜登记项、动作行不参与 |
| 9 | ⬜ | 「恢复默认」的 hover 卡未接 | 现用 `IconName::RotateCw` 图标按钮；默认值文本无处看（接 `HoverCard` 后补） |
| 10 | 🟡 | 页面窗口测试 | 已有冒烟（渲染一帧 / 默认节 / 程序性改词）；键盘输入路径无自动化覆盖（架构 §13 K10） |
| 11 | ⬜ | 打开时的焦点落点 | 现为**页面根**（`Esc` 立刻可用）；是否直接交给搜索框待定（与索引焦点冲突时要额外判断） |
